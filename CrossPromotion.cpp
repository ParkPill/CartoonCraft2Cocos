...
void CrossPromotion::update(float dt){
    if(isInfoArrived){
        ...
        sptFromUrl->setPosition(Vec2(size.width/2, size.height + 500));
    ...
    if(isWaitingForImage && sptFromUrl->isReady){
        ...
        btnFrame->setCapInsets(cocos2d::Rect(30, 30, 30, 30));
        float scale = 1;
        btnFrame->setScale(scale);
        btnFrame->setScale9Enabled(true);
        float margin = 52;
        btnFrame->setContentSize(cocos2d::Size((sptFromUrl->imgSize.width + margin)/scale, (sptFromUrl->imgSize.height + margin)/scale));
