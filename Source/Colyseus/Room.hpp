...
#include "../Source/Colyseus/Serializer/schema.h"
...
public:
    ...
    void leave(bool consented = true)
    {
        if (!this->id.empty())
        {
            if (consented)
            {
                this->connection->send((int)ColyseusProtocol::LEAVE_ROOM);
            }
            else
            {
    ...
    {
        this->connection->send((int)ColyseusProtocol::ROOM_DATA, data);
    }
...
protected:
    ...
    void _onMessage(const WebSocket::Data &data)
    {
        ...
        if (this->previousCode == 0)
        {
            ...
            case ColyseusProtocol::JOIN_ERROR:
            {
        ...
        {
            switch ((ColyseusProtocol)this->previousCode)
            {
