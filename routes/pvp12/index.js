// routes/Pvp12league/index.js

module.exports = function(app, Pvp12league, Ccuser)
{
    var apiName = 'p12';
    ...
    var resultLeague = function (){ // in midnight reward by rank
        console.log("result league pvp12");
        Pvp12league.find({}).sort({trophy : -1}) // sort -1 for ascending
...
//                setRank(users[i].userid, i, users[i].trophy, users.length);
                ...
                Ccuser.findOne({_id: users[i].userid}, function(err, userWithID){
                    ...
                    if(userWithID){
                        userWithID.pvp_rwd_12 = i+"_"+total+"_"+trophy;
//                        console.error("user id: " + userWithID.id + " reward: " + userWithID.pvp_rwd_12); // test 
                ...
                if(toRemoveCount > 0){
                    toRemoveCount--;
                    Pvp12league.deleteOne({ _id: users[i]._id }, function (err) {
                        if (err) {
                            console.log("err: " + err);
                        }
...
//            Pvp12league.deleteMany({}, function (err, _) {}); // delete all
        });
    }
    Pvp12league.resultLeague = resultLeague;
...
//            console.log("sv score: " + user.score);
...
//                    console.log("saveuser success data:" +  JSON.stringify(user));
        ...
        Pvp12league.findOne(param, callback);
    });
    
    // get rank
    app.post('/cc/get'+apiName+'rank', function(req,res){
//        console.log("rank req id: " + req.body);
        Pvp12league.find().sort({trophy : -1}) // sort -1 for ascending
                                            .exec(function(err, users){
...
//        console.log("find match for trophy: " + req.body.trophy);
        Pvp12league.find({trophy:{$gt:min}}
                    , function(err, users){
            if(err) return res.status(500).send({error: 'database failure'});
            if(users.length == 0){
                var newUser = new Pvp12league();
