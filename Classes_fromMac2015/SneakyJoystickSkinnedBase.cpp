...
void SneakyJoystickSkinnedBase::setBackgroundSprite(cocos2d::ui::ImageView *aSprite)
{
	if(backgroundSprite){
...
void SneakyJoystickSkinnedBase::setEnabled(bool enable){
    if (enable) {
        if(joystick->touchListener == nullptr){
            joystick->addListener();
        }
    }else{
        if(joystick->touchListener != nullptr){
            joystick->removeListener();
        }
    }
}
