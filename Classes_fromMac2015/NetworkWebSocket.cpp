//
//  NetworkWebSocket.cpp
//  CartoonCraftNV
//
//  Created by PACKSUNG PILL on 2020/05/19.
//
...
//        log("protocol %s", _socket->getProtocol().c_str());
        vector<string> *list = new vector<string>(1);
        list->push_back("echo-protocol");
        bool result = this->_socket->init(*this, this->_url, list);
...
bool NetworkWebSocket::send(string message)
{
    ...
    {
//        log("exception on network serer");
        throw;
    }
