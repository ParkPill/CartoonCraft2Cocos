...
bool HudLayer::init() {
  ...
  //    sptRect->setCapInsets(cocos2d::Rect(3, 3,
  //    sptRect->getContentSize().width - 6, sptRect->getContentSize().height -
  //    6)); sptRect->setScale9Enabled(true); this->addChild(sptRect, 9);
  //    sptRect->runAction(Sequence::create(DelayTime::create(0.5f),
  //    EaseIn::create(MoveBy::create(0.5f, Vec2(0, -750)), 2),
  //    CallFunc::create(CC_CALLBACK_0(ImageView::removeFromParent, sptRect)),
  //    NULL));
  //
...
//        UserDefault::getInstance()->setBoolForKey(KEY_AUTO_TARGET_SELECTED,
//        false);
//    }
#endif
  ...
  bool lastJewelCollected = UserDefault::getInstance()->getBoolForKey(
      __String::createWithFormat(KEY_JEWEL_COLLECTED_FORMAT, 3)->getCString());
  ...
  //    lblCommand->setPosition(Vec2(size.width/2,
  //    lblCommand->getBoundingBox().size.height/2 + 50));

  Button *btn;
  for (int i = 0; i < 6; i++) {
    //        Vec2 pos = Vec2(size.width - 600 + (i%3)*200, 400 - 200*(i/3)); //
    //        two rows Vec2 pos = Vec2(size.width - 10 - 200*(5-i), 10); //
    //        reverse
    ...
    //        PPLabel* lbl = PPLabel::create("", 30, Color3B::WHITE, true, true,
    //        TextHAlignment::CENTER, true); btn->addChild(lbl, 10);
    //        lbl->setPosition(Vec2(btn->getContentSize().width/2,
    //        btn->getContentSize().height*5/6)); lbl->setName("lbl");
  ...
  PPLabel *lbl = PPLabel::create(
      "0", 40, Color3B::WHITE, false, true, TextHAlignment::LEFT,
      false); // LM->getLocalizedLabel("0", Color4B::WHITE);
  ...
  lbl = PPLabel::create("0", 40, Color3B::WHITE, false, true,
  ...
  lbl = PPLabel::create("not enough food", 50, Color3B::WHITE, true, true,
  ...
  if (isRaid) {
    ...
    for (auto info : deckUnitList) {
      ...
      btn->addClickEventListener(
          CC_CALLBACK_1(HudLayer::onUnitInBottomDeckClick, this));
      btn->setTag(counter);
      btn->setPosition(Vec2(137.05f + counter * 267.3f, 122.38f));
      Sprite *spt = Sprite::createWithSpriteFrameName(
          WORLD->getSpriteNameForUnit(info->unitType));
      btn->addChild(spt);
      spt->setScale(2);
    ...
    for (auto info : heroDeckUnitList) {
      ...
      btn->setTag(counter);
      btn->setPosition(Vec2(137.05f + counter * 267.3f, 122.38f));
      EnemyBase *unit = EnemyBase::createEnemy(
          info->unitType, 0, 0,
          WORLD->getSpriteNameForUnit(info->unitType).c_str());
      btn->addChild(unit);
    ...
    sv->setInnerContainerSize(
        cocos2d::Size(counter * 267.3f, sv->getContentSize().height));
  }
  if (GM->nextScene == STAGE_FIELD || isRaid) {
    ...
    if (WORLD->isMultiplay) {
      Label *lblOpponentID =
          LM->getLocalizedLabel(MM->enemyID.c_str(), Color4B::WHITE);
      ...
      Label *lblOpponentName =
          LM->getLocalizedLabel(MM->enemyName.c_str(), Color4B::WHITE);
    ...
    lblTimer = PPLabel::create("00:00:00", 50, DARK_GRAY_3B, false, false,
    ...
    btnSelectAll->addClickEventListener(
        CC_CALLBACK_0(HudLayer::onSelectAllForces, this));

    btn = (Button *)ndPanel->getChildByName("btnSelectDrag");
    btn->addClickEventListener(
        CC_CALLBACK_1(HudLayer::onDragSelectClick, this));
    ...
    btn->addClickEventListener(
        CC_CALLBACK_0(HudLayer::onSelectForcesInScreen, this));

    Button *btnShow = (Button *)ndPanel->getChildByName("btnShowHide");
    btnShow->addClickEventListener(
        CC_CALLBACK_1(HudLayer::onShowMoreMenuClick, this));
#if (CC_TARGET_PLATFORM == CC_PLATFORM_WIN32)
    ndPanel->setPosition(Vec2(size.width - offsetRight, 567.30f));
    btnShow->setTag(1);
    btnShow->setVisible(false);
    ndPanel->getChildByName("btnSelectDrag")->setVisible(false);
    ndPanel->getChildByName("btnSelectScreen")->setVisible(false);
#endif
    rightBottomPanel =
    ...
    lblGold->setString(
        Value((int)(10 * pow(2, GM->matchFindCount + 1))).asString());
    GM->alignToCenter(btn->getChildByName("imgGold"), lblGold, 20,
                      btn->getContentSize().width / 2, 0);
    ...
    Label *lblDescription =
        LM->getLocalizedLabel("", Color4B(77, 77, 77, 255), 30);
    ...
    if (GM->nextScene == STAGE_FIELD && !isRaid) {
      ...
      imgDragSelected =
          rightBottomPanelForCampaign->getChildByName("imgDrawSelected");
  ...
  if (WORLD->stageIndex == 0 && GM->nextScene == STAGE_FIELD &&
      !GM->isColosseum && WORLD->difficultyMode != DIFFICULTY_MODE_HELL) {
    tutorialIndex = 0;
    ...
    img->addClickEventListener(
        CC_CALLBACK_0(HudLayer::onTutorialBoxClick, this));
...
void HudLayer::onDisassembleCancelClick() { hideDisassembleConfirmButtons(); }
void HudLayer::showDisassembleConfirmButtons() {
  ...
  if (btn == nullptr) {
    ...
    btn->addClickEventListener(
        CC_CALLBACK_0(HudLayer::onDisassembleConfirmClick, this));
    this->addChild(btn);
#if (CC_TARGET_PLATFORM == CC_PLATFORM_WIN32)
    btn->setScale(0.65f);
  ...
  {
    float halfH = btn->getContentSize().height * btn->getScale() * 2 / 3;
    float halfW = btn->getContentSize().width * btn->getScale() / 2;
    btn->runAction(MoveTo::create(
        0.3f, Vec2(size.width / 2 - halfW - gap / 2, halfH + gap / 2)));
  }
  ...
  if (btn == nullptr) {
    ...
    btn->addClickEventListener(
        CC_CALLBACK_0(HudLayer::onDisassembleCancelClick, this));
    this->addChild(btn);
#if (CC_TARGET_PLATFORM == CC_PLATFORM_WIN32)
    btn->setScale(0.65f);
...
void HudLayer::hideDisassembleConfirmButtons() {
  ...
  if (btn != nullptr) {
    btn->stopAllActions();
    btn->runAction(MoveTo::create(
        0.3f, Vec2(size.width / 2, -btn->getContentSize().height)));
  }
...
bool HudLayer::tryHandleHudClick(const Vec2 &glPos) {
  ...
  const HudClickTarget targets[] = {
      {"btnDisassembleConfirm", &HudLayer::onDisassembleConfirmClick},
      {"btnDisassembleCancel", &HudLayer::onDisassembleCancelClick},
      {"btnDisassemble", &HudLayer::onDisassembleClick},
      {"btnCancelBuilding", &HudLayer::onCancelBuildingClick},
  };

  for (const auto &target : targets) {
    Node *btnNode = getChildByName(target.name);
    if (btnNode == nullptr || !btnNode->isVisible()) {
      continue;
    }
    Rect box = btnNode->getBoundingBox();
    box.origin.x -= padding;
    box.origin.y -= padding;
    box.size.width += padding * 2;
    box.size.height += padding * 2;
...
void HudLayer::initShortcuts(Vec2 startPos) {
  ...
  for (int i = 0; i < shortcutCount; i++) {
    ...
    btn->setTag(i);
    ((Text *)shortcutButton->getChildByName("lblIndex"))
        ->setString(Value(i + 1).asString());
  ...
  btn->addClickEventListener(
      CC_CALLBACK_1(HudLayer::onShortcutDeleteClick, this));
...
void HudLayer::onShortcutClick(Ref *ref) {
  ...
  if (identifiedIDs.size() == 0) { // register
    if (shortcutList[index].length() == 0 &&
        WORLD->selectedArray.size() > 0) { // no history
      str = "";
      ...
      ((Text *)btn->getParent()->getChildByName("lblCount"))
          ->setString(Value((int)WORLD->selectedArray.size()).asString());
      ...
      if (WORLD->isHero(unitIndex)) {
        ...
        img->loadTexture(WORLD->getSpriteNameForUnit(unitIndex),
      ...
      for (int i = 0; i < shortcutCount; i++) {
        ((Button *)getChildByName(strmake("btnShortcut%d", i))
             ->getChildByName("btn"))
            ->setColor(i == index ? Color3B::GREEN : Color3B::WHITE);
      }
    ...
    if (selectedShortcut == index ||
        true) { // remove true if screen move should happen in the second time
    ...
    btn->getParent()->getChildByName("lblPlus")->setVisible(false);
    ((Text *)btn->getParent()->getChildByName("lblCount"))
        ->setString(Value((int)identifiedIDs.size()).asString());
  }
...
void HudLayer::onShortcutDeleteClick(Ref *ref) {
  ...
  Button *btn =
      (Button *)getChildByName(strmake("btnShortcut%d", selectedShortcut))
          ->getChildByName("btn");
  btn->setColor(Color3B::WHITE);
  if (btn) {
    btn->getParent()->getChildByName("lblPlus")->setVisible(true);
...
void HudLayer::on2xFastClick() {
  rightBottomPanelForCampaign->getChildByName("img3xSelected")
      ->setVisible(false);
  if (WORLD->gameSpeed == 2) {
    WORLD->gameSpeed = 1;
    rightBottomPanelForCampaign->getChildByName("img2xSelected")
        ->setVisible(false);
  } else {
    WORLD->gameSpeed = 2;
    rightBottomPanelForCampaign->getChildByName("img2xSelected")
...
void HudLayer::onShowMoreMenuClick(Ref *ref) {
  ...
  if (shouldShow) {
    btn->getParent()->runAction(EaseOut::create(
        MoveTo::create(0.3f, Vec2(size.width - offsetRight, 567.30f)), 2));
  } else {
    btn->getParent()->runAction(EaseOut::create(
        MoveTo::create(0.3f, Vec2(size.width - offsetRight, 0)), 2));
  }
...
void HudLayer::setRaid() {
  std::string strEquipped =
      GM->raidEnemyBuildings; // UDGetStr(KEY_BUILDINGS, "");
  ...
  for (int i = 0; i < units.size(); i++) {
    ...
    if (datas.size() > 1) {
      ...
      if (x > 100 || y > 100) {
        ...
        float extraX = 0;
        //                if
        //                ((int)WORLD->getBuildingOccupySize(unitType).width%2
        //                == 1) {
        //                    extraX = 50;
        //                }
        float extraY = 0;
        //                if
        //                ((int)WORLD->getBuildingOccupySize(unitType).height%2
        //                == 1) {
        //                    extraY = 50;
        //                }
        sptPos = Vec2(x * TILE_SIZE + extraX, y * TILE_SIZE + extraY);
      }
      Vec2 pos = sptPos - Vec2(occupySize.width * TILE_SIZE / 2,
                               occupySize.height * TILE_SIZE / 2);
      Vec2 occupyPos = pos + Vec2(TILE_SIZE * (((int)occupySize.width) % 2),
                                  TILE_SIZE * occupySize.height);
      EnemyBase *unit = WORLD->createUnit(
          unitType,
          info->unitType == UNIT_TREE_FOR_BATTLE ? WHICH_SIDE_MUTUAL
                                                 : WHICH_SIDE_ENEMY,
          ITS_BUILDING, sptPos, "name", 1,
          WORLD->getSpriteNameForUnit(unitType));
      ...
      if (unit->isBuilding) {
        unit->setPosition(
            unit->getPosition() +
            Vec2(TILE_SIZE * 0.5f * (((int)occupySize.width) % 2),
                 TILE_SIZE * 0.5f * (((int)occupySize.height) % 2)));
        //                setupBuilding(unit);
        unit->level = info->level;
        WORLD->setOccupy(occupyPos, occupySize.width, occupySize.height, true,
                         unit);

        unit->buildingCompleteTimeLeft = info->endTime - BSM->getCurrentTimeT();
        unit->unitState = info->unitState;
        //                buildings.pushBack(unit);
        if (info->unitState == UNIT_STATE_BUILDING ||
            info->unitState == UNIT_STATE_UPGRADE ||
            unit->unitState == UNIT_STATE_REMOVING) {
          unit->energy = 1;
          unit->maxEnergy = 100;
          unit->updateEnergy();
          ...
          if (unit->unitState == UNIT_STATE_UPGRADE) {
            ...
            //                        unit->buildingCompleteTime =
            //                        getUnitCreatingTime(unit->unitType,
            //                        unit->level+1);
          } else if (unit->unitState == UNIT_STATE_BUILDING) {
            if (unit->unitType == UNIT_TREE_FOR_BATTLE) {
              unit->setScale(0.5f);
              unit->runAction(
                  ScaleTo::create(unit->buildingCompleteTimeLeft, 1));
              GM->makeItSiluk(unit);
            } else {
  ...
  for (int i = 0; i < units.size(); i++) {
    if (units.at(i).asString().length() > 0) {
      ...
      //                    unit->setPosition(building->getPosition() + Vec2(0,
      //                    -200)); info->x = unit->getPositionX(); info->y =
      //                    unit->getPositionY(); break;
      //                }
      //            }
  ...
  if (str.length() > 0) {
    ...
    for (int i = 0; i < posList.size(); i++) {
      ValueVector xAndY =
          GameManager::getInstance()->split(posList.at(i).asString(), "/");
      if (xAndY.size() > 1) {
  ...
  for (int i = 0; i < units.size(); i++) {
    if (units.at(i).asString().length() > 0) {
      ...
      if (raidHeroPosXList.size() > index && raidHeroPosYList.size() > index) {
        pos = WORLD->getPositionFromTileCoordinate(raidHeroPosXList.at(index),
                                                   raidHeroPosYList.at(index));
      }
      unit->setPosition(pos);
      unit->setLocalZOrder(-unit->getBoundingBox().getMinY());
...
void HudLayer::showStarForRaid(int starIndex) {
  ImageView *imgStar = (ImageView *)rightBottomPanel->getChildByName("ndBattle")
                           ->getChildByName(strmake("imgStar%d", starIndex));
...
void HudLayer::showRaidResultWithSaveRecord() { showRaidResult(true); }
void HudLayer::showRaidResult(bool saveRecord) {
  ...
  if (GM->currentStageIndex == STAGE_RAID && saveRecord) {
    ...
    if (isWin) {
      ...
      //            if (!isWin) {
      //                trophy = 0; // no bonus trophy for opponent when player
      //                lose
      //            }
    }
    if (trophy < -100) {
    ...
    BSM->sendPost(
        "adddefencerecord",
        "id=" + GM->raidEnemyID + "&enemyid=" + UDGetStr(KEY_SAVED_ID, "-1") +
            "&name=" + UDGetStr(KEY_NAME, "noname") +
            "&trophy=" + Value(isWin ? -1 : 0).asString() + "&gold=" +
            Value(gold).asString() + "&tree=" + Value(tree).asString() +
            "&star=" + Value(WORLD->starCountForRaid).asString(),
        httpresponse_selector(BuggyServerManager::onActionNotNeededComplete));
    ...
    if (isWin) {
      bool isRewardGiven = UDGetBool(
          strmake(KEY_SINGLE_PLAY_CLEAR_FORMAT, GM->singlePlayStageIndex)
              .c_str(),
          false);
      trophy = 0;
      if (!isRewardGiven) {
        ...
        BSM->saveUserData("gold=" + Value(GM->getCoin()).asString() +
                          "&tree=" + Value(GM->getTree()).asString());
      }

      UDSetBool(strmake(KEY_SINGLE_PLAY_CLEAR_FORMAT, GM->singlePlayStageIndex)
                    .c_str(),
                true);
      UDSetInt(strmake(KEY_SINGLE_PLAY_STAR_FORMAT, GM->singlePlayStageIndex)
                   .c_str(),
               WORLD->starCountForRaid);
    }
  }
  ...
  btn->addClickEventListener(
      CC_CALLBACK_1(HudLayer::onOkFromRaidWinPopup, this));
  Node *imgTitle = layer->getChildByName("imgTitle");
  ImageView *img;
  float delayTime = 0.5f;

  Node *imgSwordL =
      layer->getChildByName("imgTitle")->getChildByName("imgSwordL");
  Node *imgSwordR =
      layer->getChildByName("imgTitle")->getChildByName("imgSwordR");
  if (!isWin) {
...
void HudLayer::showCancelBuildingButton() {
  ...
  btn->addClickEventListener(
      CC_CALLBACK_0(HudLayer::onCancelBuildingClick, this));
  this->addChild(btn);
#if (CC_TARGET_PLATFORM == CC_PLATFORM_WIN32)
  btn->setScale(0.65f);
...
void HudLayer::onCancelBuildingClick() {
  ...
  WORLD->isBuildingTemplateMoved = false;
...
#if (CC_TARGET_PLATFORM == CC_PLATFORM_WIN32)
  WORLD->win32HudButtonClicked = true;
#endif
}
...
void HudLayer::showSupportOffer() {
  ...
  btn->setZoomScale(0);
  btn->runAction(
      EaseInOut::create(ResizeTo::create(0.3, cocos2d::Size(2098, 1270)), 2));

  PPLabel *lbl =
      PPLabel::create(LM->getText("choose support"), 60, Color3B(45, 45, 45),
                      LM->getLanguageType() != LanguageType::ENGLISH, false,
                      TextHAlignment::CENTER, false);
  ...
  for (int i = 0; i < 3; i++) {
    ...
    btn->setScale9Enabled(true);
    btn->setPosition(
        Vec2(size.width / 2 + (i - 1) * 560, size.height / 2 + 60));

    btn->addClickEventListener(
        CC_CALLBACK_1(HudLayer::onSupportSelected, this));
    ...
    if (i == 0) {
      ...
      for (int j = 0; j < 4; j++) {
        ...
        btn->addChild(spt);
        spt->setPosition(Vec2(btn->getContentSize().width / 2 -
                                  (unitGap * 3 / 2) + unitGap * j,
      ...
      for (int j = 0; j < 3; j++) {
        ...
        btn->addChild(spt);
        spt->setPosition(
            Vec2(btn->getContentSize().width / 2 - (unitGap) + unitGap * j,
                 btn->getContentSize().height / 2 + 70));
      }
    ...
    if (GM->isColosseum) {
      lbl = PPLabel::create(LM->getText("select support"),
                            btn->getContentSize().height * 2 / 6, DARK_GRAY_3B,
                            LM->getLanguageType() != LanguageType::ENGLISH,
                            false, TextHAlignment::CENTER, false);
      lbl->setPosition(Vec2(size.width / 2 + (i - 1) * 560 - 520 / 2,
                            size.height / 2 - 320));
      layer->addChild(lbl);
      lbl->setAnchorPoint(Vec2(0, 0.5));
      ...
      btnVideo->setPosition(Vec2(size.width / 2 + (i - 1) * 560 + 150 + xOffset,
                                 size.height / 2 - 320));
      btnVideo->addClickEventListener(
          CC_CALLBACK_1(HudLayer::onSupportSelected, this));
      btnVideo->setTag(i);
      ...
      btnTicket->setPosition(
          Vec2(size.width / 2 + (i - 1) * 560 + 320 + xOffset,
               size.height / 2 - 320));
      btnTicket->addClickEventListener(
          CC_CALLBACK_1(HudLayer::onSupportSelectedByTicket, this));
      btnTicket->setTag(i);
      ...
      lbl = PPLabel::create(Value(ticketCount).asString(), 50, Color3B::WHITE,
                            false, true, TextHAlignment::RIGHT, false);
      ...
      spt->setPosition(
          lbl->getPosition() +
          Vec2(-lbl->totalWidth / 2 - spt->getContentSize().width / 2 - 50, 0));
    } else {
      ...
      btn->setScale9Enabled(true);
      btn->setPosition(
          Vec2(size.width / 2 + (i - 1) * 560, size.height / 2 - 320));
      btn->addClickEventListener(
          CC_CALLBACK_1(HudLayer::onSupportSelected, this));
      btn->setTag(i);
      ...
      lbl = PPLabel::create(LM->getText("select support"),
                            ...
                            LM->getLanguageType() != LanguageType::ENGLISH,
                            false, TextHAlignment::CENTER, false);
      lbl->setPosition(Vec2(btn->getContentSize().width / 2 + 70,
                            btn->getContentSize().height / 2));
      btn->addChild(lbl);
...
void HudLayer::onMenuClick() {
  ...
  //    btnBlock->setColor(Color3B::BLACK);
  //    btnBlock->setPressedActionEnabled(false);
  //    btnBlock->addClickEventListener(CC_CALLBACK_0(HudLayer::onResumeClick,
  //    this));
  //
  //    ImageView* img = ImageView::create("uiBox.png");
  ...
  //    }else{
  //        btn->addClickEventListener(CC_CALLBACK_0(HudLayer::onSaveClick,
  //        this));
  //    }
  ...
  //        btn->setColor(Color3B::GRAY);
  //    }else{
  //        btn->addClickEventListener(CC_CALLBACK_0(HudLayer::onLoadClick,
  //        this));
  //    }
  //    btn->setPosition(Vec2(size.width/2, y));
  ...
  //    btn->setContentSize(cocos2d::Size(520, 150));
  //    btn->setScale9Enabled(true);
  //    btn->addClickEventListener(CC_CALLBACK_0(HudLayer::onOptionClick,
  //    this)); btn->setPosition(Vec2(size.width/2, y));
  ...
  //    btn->setContentSize(cocos2d::Size(520, 150));
  //    btn->setScale9Enabled(true);
  //    btn->addClickEventListener(CC_CALLBACK_0(HudLayer::onResumeClick,
  //    this)); btn->setPosition(Vec2(size.width/2, y));
...
void HudLayer::onSaveClick() {
  ...
  float gapY = 260;
  PPLabel *lblSave =
      PPLabel::create(LM->getText("save"), 100, DARK_GRAY_3B, true, false,
                      TextHAlignment::CENTER, false);
...
void HudLayer::onSaveSlotClick(Ref *ref) {
  ...
  if (savedStage >= 0) {
    ...
    PPLabel *lbl =
        PPLabel::create(LM->getText("overwrite save data"), 50, DARK_GRAY_3B,
                        true, false, TextHAlignment::CENTER, false);
    layer->addChild(lbl);
    lbl->setPosition(Vec2(size.width / 2, size.height / 2 + 300));
    lbl->setWidth(1000);
    ...
    btn->setContentSize(cocos2d::Size(500, 200));
    btn->addClickEventListener(
        CC_CALLBACK_0(HudLayer::onOkFromSaveOverwrite, this));
...
void HudLayer::saveCurrentStageData(int slot) {
  ...
  for (auto enemy : WORLD->heroArray) {
    ...
    data.append(Value(enemy->btns.size() > 0 ? enemy->btns.at(0)->getTag() : -1)
                    .asString() +
                ",");
    data.append(Value(enemy->btns.size() > 1 ? enemy->btns.at(1)->getTag() : -1)
                    .asString() +
                ",");
    data.append(Value(enemy->btns.size() > 2 ? enemy->btns.at(2)->getTag() : -1)
                    .asString() +
                ",");
    data.append(Value(enemy->btns.size() > 3 ? enemy->btns.at(3)->getTag() : -1)
                    .asString() +
                ",");
    data.append(Value(enemy->btns.size() > 4 ? enemy->btns.at(4)->getTag() : -1)
                    .asString() +
                ",");
...
void HudLayer::onOkFromLoadData() {
  ...
  this->runAction(Sequence::create(
      ...
      CallFunc::create(CC_CALLBACK_0(HudLayer::goToLoadedStage, this)), NULL));
}
...
void HudLayer::showPriceInfo(std::string msg, int gold, int lumber, int food) {
  ...
  priceInfo->runAction(Sequence::create(
      DelayTime::create(3),
      CallFunc::create(CC_CALLBACK_0(HudLayer::hidePriceInfo, this)), NULL));
...
void HudLayer::setMenu(int index, int btnType) {
  ...
  btnMenu->getChildByName("lblGold")->setVisible(btnType >= BTN_TYPE_WORKER &&
                                                 btnType < BTN_TYPE_CANCEL);
  btnMenu->getChildByName("lblLumber")
      ->setVisible(btnType >= BTN_TYPE_WORKER && btnType < BTN_TYPE_CANCEL);
  int unitIndex = -1;
  ...
  if (btnType == BTN_TYPE_MOVE) {
    ...
    //        LM->setLocalizedString((Label*)btn->getChildByName("lblName"),
    //        "move");
...
#if (CC_TARGET_PLATFORM == CC_PLATFORM_WIN32)
  {
    ...
    static const SC shortcuts[] = {
        ...
        {BTN_TYPE_ARCHER, "A", nullptr},
        {BTN_TYPE_HELICOPTER, "H", nullptr},
        {BTN_TYPE_CATAPULT, "C", nullptr},
        {BTN_TYPE_CASTLE, "C", nullptr},
        {BTN_TYPE_FARM, "F", nullptr},
        {BTN_TYPE_BARRACKS, "R", nullptr},
        {BTN_TYPE_LUMBER, "L", nullptr},
        {BTN_TYPE_WATCHER_TOWER, "W", nullptr},
        {BTN_TYPE_FACTORY, "T", nullptr},
        {BTN_TYPE_AIRPORT, "A", nullptr},
        {BTN_TYPE_HQ, "H", nullptr},
        {BTN_TYPE_BUNKER, "U", nullptr},
        {BTN_TYPE_ORC_BARRACKS, "R", nullptr},
        ...
        {BTN_TYPE_ORC_SPEAR, "S", nullptr},
        {BTN_TYPE_TROLL, "T", nullptr},
        {BTN_TYPE_MAGE, "M", nullptr},
    };
  ...
  ImageView *imgTitleBack =
      (ImageView *)btnMenu->getChildByName("imgTitleBack");
#if (CC_TARGET_PLATFORM == CC_PLATFORM_WIN32)
  imgTitleBack->setVisible(isToBuy || lbl->getString().length() > 0);
#else
  ...
  if (isToBuy) {
    //        LM->setLocalizedString((Label*)btn->getChildByName("lblName"),
    //        WORLD->getUnitName(unitIndex));
    ((Text *)btn->getParent()->getChildByName("lblGold"))
        ->setString(Value(WORLD->getGoldPriceForUnit(unitIndex)).asString());
    ((Text *)btn->getParent()->getChildByName("lblLumber"))
        ->setString(Value(WORLD->getLumberPriceForUnit(unitIndex)).asString());
...
void HudLayer::onCommandClick(Ref *ref) {
  ...
  if (index != BTN_TYPE_BUILD && index != BTN_TYPE_BUILD_BETTER &&
      index != BTN_TYPE_CANCEL) {
    //        if(selectedMenuName.compare(name) != 0){
    //            if(selectedMenuName.compare("-1") != 0){
    //                Button* btnPrevious =
    //                (Button*)WORLD->getChildByName(selectedMenuName);
...
void HudLayer::hideInfo() { lblInfo->setVisible(false); }
...
void HudLayer::addLabelToButton(std::string text, Button *btn, bool bordered,
                                Color3B color, bool isSystemLabel) {
  PPLabel *lbl = PPLabel::create(
      LM->getText(text.c_str()), btn->getContentSize().height * 2 / 6, color,
      isSystemLabel, bordered, TextHAlignment::CENTER, false);
  lbl->setPosition(
      Vec2(btn->getContentSize().width / 2, btn->getContentSize().height / 2));
  btn->addChild(lbl);
}
Sprite *HudLayer::addSpriteToButton(std::string imgFileName, Button *btn,
                                    Button::TextureResType type) {
  Sprite *spt;
...
Sprite *HudLayer::getIconSprite(std::string itemName) {
  ...
  Sprite *spt =
      Sprite::createWithSpriteFrameName(WORLD->getItemStat(itemName, "sprite"));
  spt->setName("sprite");
  cocos2d::Rect textureRect = spt->getTextureRect();
  if (textureRect.size.width > TILE_SIZE ||
      textureRect.size.height > TILE_SIZE) {
    ...
    spt->setTextureRect(cocos2d::Rect(
        textureRect.origin.x + spt->getContentSize().width / 2 -
            textureWidth / 2,
        textureRect.getMaxY() - textureHeight, textureWidth, textureHeight));
  }
...
void HudLayer::toggleAuto(bool showMsg) {
  ...
  //    Sprite* spt =
  //    Sprite::create(isAuto?"UI/btnToggleManual.png":"UI/btnToggleAuto.png");
  ...
  if (showMsg) {
    GameManager::getInstance()->showDisposableMessage(
        LanguageManager::getInstance()
            ->getText(isAuto ? "auto targeting" : "manual targeting")
            .c_str(),
        this);
  }
}
...
//    }
  ...
  if (okay) {
    ...
    _listener->onDisconnected =
        CC_CALLBACK_2(HudLayer::onDisconnectedController, this);
    ...
    // bind onAxis event call function, onAxis will be called when analog stick
    // is changed
...
// Controller is the obejects of the Controller，keyCode means the keycode of
// the controller you click down
void HudLayer::onKeyDown(cocos2d::Controller *controller, int keyCode,
                         cocos2d::Event *event) {
  CCLOG("KeyDown:%d", keyCode);
}

void HudLayer::onKeyUp(cocos2d::Controller *controller, int keyCode,
                       cocos2d::Event *event) {
  // You can get the controller by tag, deviceId or devicename if there are
  // multiple controllers
  CCLOG("tag:%d DeviceId:%d DeviceName:%s", controller->getTag(),
        controller->getDeviceId(), controller->getDeviceName().c_str());
  CCLOG("KeyUp:%d", keyCode);
}

// The axis includes X-axis and Y-axis and its range is from -1 to 1. X-axis is
// start from left to right and Y-axis is bottom to top.
void HudLayer::onAxisEvent(cocos2d::Controller *controller, int keyCode,
                           cocos2d::Event *event) {
...
void HudLayer::addHeartAnimated(Vec2 pos) {
  ...
  spt->runAction(Sequence::create(
      EaseIn::create(MoveTo::create(1, heart->getPosition() + Vec2(50, 0)), 3),
      CallFuncN::create(CC_CALLBACK_1(HudLayer::addHeartDone, this)),
...
void HudLayer::addHeartDone(Ref *obj) {
  ...
  spt->setScale(0.5);
  spt->runAction(Sequence::create(
      RotateBy::create(1, 300),
      CallFuncN::create(CC_CALLBACK_1(HudLayer::spriteMoveDone, this)), NULL));

  heart->addChild(spt);
  spt->setPosition(Vec2(heart->getContentSize().width / 2,
                        heart->getContentSize().height / 2));
}
...
void HudLayer::onKeyPressed(EventKeyboard::KeyCode keyCode, Event *event) {
  ...
  if (WORLD != nullptr && WORLD->usesPcRtsControls()) {
    ...
    if (keyCode == EventKeyboard::KeyCode::KEY_ESCAPE) {
      ...
      Node *disassembleCancelBtn = this->getChildByName("btnDisassembleCancel");
      if (disassembleCancelBtn != nullptr &&
          disassembleCancelBtn->getPositionY() > 0) {
        onDisassembleCancelClick();
        return;
      }
    }
    ...
    // D: demolish selected building
    if (keyCode == EventKeyboard::KeyCode::KEY_D) {
      Node *demolishBtn = this->getChildByName("btnDisassemble");
      if (demolishBtn != nullptr && demolishBtn->isVisible()) {
        onDisassembleClick();
        return;
      }
    }

    // Menu shortcuts: check visible menu buttons first.
    // If a matching button is found, fire it and skip general hotkeys.
    auto getMenuKey = [](int btnType) -> EventKeyboard::KeyCode {
      ...
      case BTN_TYPE_STOP:
        return KC::KEY_S;
      case BTN_TYPE_ATTACK:
        return KC::KEY_A;
      case BTN_TYPE_GATHER:
        return KC::KEY_G;
      case BTN_TYPE_BUILD:
        return KC::KEY_B;
      case BTN_TYPE_BUILD_BETTER:
        return KC::KEY_V;
      case BTN_TYPE_CANCEL:
        return KC::KEY_ESCAPE;
      case BTN_TYPE_WORKER:
        return KC::KEY_W;
      case BTN_TYPE_GOBLIN_WORKER:
        return KC::KEY_W;
      case BTN_TYPE_SWORDMAN:
        return KC::KEY_S;
      case BTN_TYPE_ARCHER:
        return KC::KEY_A;
      case BTN_TYPE_HELICOPTER:
        return KC::KEY_H;
      case BTN_TYPE_CATAPULT:
        return KC::KEY_C;
      case BTN_TYPE_CASTLE:
        return KC::KEY_C;
      case BTN_TYPE_FARM:
        return KC::KEY_F;
      case BTN_TYPE_BARRACKS:
        return KC::KEY_R;
      case BTN_TYPE_LUMBER:
        return KC::KEY_L;
      case BTN_TYPE_WATCHER_TOWER:
        return KC::KEY_W;
      case BTN_TYPE_FACTORY:
        return KC::KEY_T;
      case BTN_TYPE_AIRPORT:
        return KC::KEY_A;
      case BTN_TYPE_HQ:
        return KC::KEY_H;
      case BTN_TYPE_BUNKER:
        return KC::KEY_U;
      case BTN_TYPE_ORC_BARRACKS:
        return KC::KEY_R;
      case BTN_TYPE_TROLL_HOUSE:
        return KC::KEY_T;
      case BTN_TYPE_TEMPLE:
        return KC::KEY_E;
      case BTN_TYPE_BARBECUE:
        return KC::KEY_B;
      case BTN_TYPE_GOBLIN:
        return KC::KEY_G;
      case BTN_TYPE_GOBLIN_BOMB:
        return KC::KEY_B;
      case BTN_TYPE_ORC_AXE:
        return KC::KEY_A;
      case BTN_TYPE_ORC_SPEAR:
        return KC::KEY_S;
      case BTN_TYPE_TROLL:
        return KC::KEY_T;
      case BTN_TYPE_MAGE:
    ...
    for (int i = 0; i < 6; i++) {
      Node *btnMenu = WORLD->getChildByName(strmake("btnMenu%d", i));
      if (btnMenu == nullptr || !btnMenu->isVisible())
        continue;
      Button *btn = (Button *)btnMenu->getChildByName("btn");
      if (getMenuKey(btn->getTag()) == keyCode) {
        onCommandClick(btn);
        return;
      }
    }
...
void HudLayer::showStageTitle() {
  ...
  int stageIndex =
      GameManager::getInstance()
          ->getWorld()
          ->indexToWarp; // GameManager::getInstance()->currentStageIndex;
  if (stageIndex == -1) {
    sprintf(bufStage, "%s",
            LanguageManager::getInstance()->getText("stageLobby").c_str());
  } else {
    if (GameManager::getInstance()->getWorld()->stageIndex >= 0 &&
        !GameManager::getInstance()->getWorld()->isBossMap) {
      sprintf(bufStage, "BOSS");
    } else {
      sprintf(bufStage, "%s %d",
              LanguageManager::getInstance()->getText("stage").c_str(),
              stageIndex + 1);
    }
  }
  ...
  if (stageIndex >= TUTORIAL_STAGE_START && stageIndex <= TUTORIAL_STAGE_END) {
    sprintf(bufStage, "%s %d",
            LanguageManager::getInstance()->getText("lesson").c_str(),
            stageIndex - TUTORIAL_STAGE_START + 1);
    if (stageIndex == TUTORIAL_STAGE_END) {
      sprintf(bufTitle, "");
      sprintf(bufStage, "%s",
              LanguageManager::getInstance()->getText("theLastLesson").c_str());
    } else {
    }
    sprintf(
        bufTitle, "%s",
        LanguageManager::getInstance()
            ->getText(__String::createWithFormat(
                          "tutorialTitle%d", stageIndex - TUTORIAL_STAGE_START)
                          ->getCString())
            .c_str());
  } else if (stageIndex == STAGE_NAMING) {
    sprintf(bufTitle, "%s",
            LanguageManager::getInstance()
                ->getText(__String::createWithFormat("stageTitle%d", stageIndex)
                              ->getCString())
                .c_str());
    sprintf(
        bufStage, "%s",
        LanguageManager::getInstance()->getText("stageTypeYourName").c_str());
    showTyping();
  } else {
    if (GameManager::getInstance()->getWorld()->stageIndex >= 0 &&
        !GameManager::getInstance()->getWorld()->isBossMap) {
      sprintf(
          bufTitle, "%s",
          LanguageManager::getInstance()
              ->getText(__String::createWithFormat("bossTitle%d", stageIndex)
                            ->getCString())
              .c_str());
    } else {
      sprintf(
  ...
  Label *lblTitle = LanguageManager::getInstance()->getLocalizedLabel(
      bufTitle, Color4B(235, 235, 235, 255));
  ...
  Label *lblStage = LanguageManager::getInstance()->getLocalizedLabel(
      bufStage, Color4B(235, 235, 235, 255));
  lblStage->setAnchorPoint(Vec2(0.5, 0.5));
  lblStage->setPosition(
      Vec2(size.width / 2, y - lblTitle->getBoundingBox().size.height - 2));
  this->addChild(lblStage, 10000);
  lblStage->setOpacity(0);

  lineLength = lblTitle->getBoundingBox().size.width >
                       lblStage->getBoundingBox().size.width
                   ? lblTitle->getBoundingBox().size.width
                   : lblStage->getBoundingBox().size.width;
  currentLineLength = 0;
  ...
  sptWhiteGun->runAction(Sequence::create(
      RotateBy::create(dur, 360),
      CallFunc::create(CC_CALLBACK_0(HudLayer::whiteGunFired, this)),
      EaseOut::create(RotateBy::create(0.1, -30), 2), RotateBy::create(0.4, 30),
      DelayTime::create(2.0f), FadeOut::create(1.0f), NULL));
  ...
  sptGunEffect->runAction(
      Sequence::create(DelayTime::create(dur), FadeIn::create(0.05f),
                       DelayTime::create(0.1f), FadeOut::create(0.05f),
                       CallFuncN::create(CC_CALLBACK_1(
                           Sprite::removeFromParentAndCleanup, sptGunEffect)),
                       NULL));
  this->addChild(sptGunEffect, 10000);

  lblTitle->runAction(
      Sequence::create(DelayTime::create(dur + 0.5f), FadeIn::create(0.5f),
                       DelayTime::create(1.5f), FadeOut::create(1),
                       CallFuncN::create(CC_CALLBACK_1(
                           Node::removeFromParentAndCleanup, lblTitle)),
                       NULL));
  lblStage->runAction(
      Sequence::create(DelayTime::create(dur + 0.5f), FadeIn::create(0.5f),
                       DelayTime::create(1.5f), FadeOut::create(1),
                       CallFuncN::create(CC_CALLBACK_1(
                           Node::removeFromParentAndCleanup, lblStage)),
                       NULL));
}
void HudLayer::stageTitleLineUpdate(float dt) {
  ...
  if (currentLineLength > 10) {
    dnTitleLine->drawSolidRect(Vec2::ZERO, Vec2(currentLineLength, 4),
                               Color4F(235.0f / 255, 235.0f / 255, 235.0f / 255,
                                       sptWhiteGun->getOpacity() / 255.0f));
  }
  ...
  dnTitleLine->drawSolidRect(Vec2(currentLineLength + gap, 0),
                             Vec2(currentLineLength + bulletWidth + gap, 4),
                             Color4F(235.0f / 255, 235.0f / 255, 235.0f / 255,
                                     sptWhiteGun->getOpacity() / 255.0f));
...
void HudLayer::showDialog(std::string str) {
  ...
  CallFunc *call =
      CallFunc::create(CC_CALLBACK_0(HudLayer::onDialogDone, this));
  call->retain();
...
void HudLayer::onDialogDone() {
  ...
  if (UserDefault::getInstance()->getBoolForKey(
          __String::createWithFormat(KEY_DOES_IT_TALKED_FORMAT, "story4")
              ->getCString(),
          false)) {
    //        showEnding();
  }
...
void HudLayer::onKeyReleased(EventKeyboard::KeyCode keyCode, Event *event) {
  ...
  if (WORLD != nullptr && WORLD->usesPcRtsControls()) {
    if (keyCode == EventKeyboard::KeyCode::KEY_ESCAPE) {
      WORLD->cancelWin32AttackMoveMode();
      return;
  ...
  if (keyCode == EventKeyboard::KeyCode::KEY_Z ||
      ...
      keyCode == EventKeyboard::KeyCode::KEY_DPAD_CENTER) {
             ...
             keyCode == EventKeyboard::KeyCode::KEY_X) {
    ...
    if (popupArray.size() == 0) {
      if (isRaid) {
        ...
        if (WORLD->gameMode == GAME_MODE_PVP6 ||
            WORLD->gameMode == GAME_MODE_PVP12) {
...
void HudLayer::showTalk(const char *talk) {
  if (talkLabel == NULL) {
    ...
    talkLabel = Label::createWithSystemFont(
        talk, GameManager::getInstance()->getFont(FONT_DEFAULT),
        30); // Label::createWithSystemFont(talk,
             // GameManager::getInstance()->getFont(FONT_ARIAL), 30);
  ...
  //    talkLabel->runAction(Sequence::create(CCFadeIn::create(0.4),
  //    DelayTime::create(2), CCFadeOut::create(0.4), NULL));
  talkLabel->runAction(Sequence::create(
      EaseOut::create(ScaleTo::create(0.4, 1), 8), DelayTime::create(2),
      EaseIn::create(ScaleTo::create(0.4, 0.1), 8), NULL));
  talkLabel->runAction(
      Sequence::create(EaseOut::create(MoveBy::create(0.4, Vec2(0, 50)), 8),
                       DelayTime::create(2),
                       EaseIn::create(MoveBy::create(0.4, Vec2(0, -50)), 8),
                       MoveBy::create(0.01, Vec2(0, 1000)), NULL));
  //    talkLabel->runAction(Sequence::create(MoveBy::create(0.4, Vec2(0, 50)),
  //    DelayTime::create(2), MoveBy::create(0.4, Vec2(0,
  //    50)),MoveBy::create(0.01, Vec2(0, 500)), NULL));
}
...
void HudLayer::showAchievementComplete(const char *text) {
  ...
  lblText->setPosition(Vec2(spt->getPosition().x + 80,
                            achieveNode->getContentSize().height / 2));
  lblText->setTag(722);
  achieveNode->addChild(lblText);

  ((Label *)achieveNode->getChildByTag(722))->setString(text);
  achieveNode->setPosition(
      Vec2(size.width / 2 + 15,
           size.height + (achieveNode->getContentSize().height / 2)));

  achieveNode->runAction(Sequence::create(
      EaseIn::create(
          MoveBy::create(0.5f, Vec2(0, -achieveNode->getContentSize().height)),
          0.2f),
      ...
      EaseIn::create(
          MoveBy::create(0.5f, Vec2(0, achieveNode->getContentSize().height)),
          4),
      CallFuncN::create(CC_CALLBACK_1(HudLayer::nodeMoveDone, this)), NULL));
}

void HudLayer::achievementEffect(Node *node) {
  Sprite *achieveNode = (Sprite *)node;
  Sprite *checkSprite = Sprite::create("checkImage.png");
  checkSprite->setPosition(Vec2(achieveNode->getContentSize().width - 80,
                                achieveNode->getContentSize().height / 2));
...
void HudLayer::addEnergyEffect() {
  ...
  for (int i = 0; i < effectCount; i++) {
    ...
    spt->setPosition(
        ...
        Vec2(rand() % spread - spread / 2, rand() % spread - spread / 2));
    Vec2 pos = spt->getPosition();
    spt->setScale(3 + (rand() % 10) * 0.4);
    this->addChild(spt, 100);
    spt->runAction(FadeOut::create(0));
    spt->runAction(Sequence::create(DelayTime::create(i * interval),
                                    FadeIn::create(interval), NULL));
    spt->runAction(Sequence::create(
        DelayTime::create(i * interval),
        EaseIn::create(MoveTo::create(dur, Vec2(x, y)), 3),
        CallFuncN::create(CC_CALLBACK_1(HudLayer::spriteMoveDone, this)),
        NULL));
  }
  this->scheduleOnce(schedule_selector(HudLayer::addEnergyDone),
                     dur + interval * effectCount);
}
...
//    recordItem->setVisible(true);w
  ...
  //    recordItem->setPosition(pauseItem->getPosition() +
  //    Vec2(-pauseItem->getContentSize().width - 15, 0));
}
...
void HudLayer::setTotalBulletCount(int count, int maxCount) {
  ...
  //    TextBMFont* lblCount =
  //    (TextBMFont*)checkBoard->getChildByName("lblBulletLeft"); if (maxCount <
  //    0) {
  //        lblCount->setString("00");
  //    }else{
  //        lblCount->setString(__String::createWithFormat("%d/%d", count,
  //        maxCount)->getCString());
  //    }
}
...
void HudLayer::setBulletCount(int count, int maxCount) {
  ...
  //    int countLeft = count;
  //    int counter = 0;
  //    float x = checkBoard->getPosition().x +
  //    checkBoard->getContentSize().width*checkBoard->getScale()/2 + 2; float y
  //    = checkBoard->getPosition().y +
  //    checkBoard->getContentSize().height*checkBoard->getScale()/2;
  //    while(countLeft > 0){
  //        Sprite* sptBullet =
  //        Sprite::createWithSpriteFrameName("bulletPi.png");
  //        sptBullet->setAnchorPoint(Vec2(0, 1));
  //        sptBullet->setPosition(x, y);
  //        sptBullet->setScale((checkBoard->getContentSize().height*checkBoard->getScale()
  //        -
  //        1*bulletCountInColumn)/(sptBullet->getContentSize().height*bulletCountInColumn));
  ...
  //            x += sptBullet->getContentSize().width*sptBullet->getScale() +
  //            2; y = checkBoard->getPosition().y +
  //            checkBoard->getContentSize().height*checkBoard->getScale()/2;
  //            counter = 0;
  //        }
  //    }
...
void HudLayer::keyCollected(int count) {
  lblKeyCollected->setString(
      __String::createWithFormat("x %d", count)->getCString());
  GameManager::getInstance()->showParticleExplosion(
      this, "blueKey.png", lblKeyCollected->getPosition(), 2);
}
void HudLayer::starCollected(int count) {
  Vec2 pos;

  GameManager::getInstance()->showParticleExplosion(this, "goldStar.png", pos,
                                                    2);
...
void HudLayer::showPauseLayer() {
  ...
  pauseLayer = dynamic_cast<Layout *>(
      GUIReader::getInstance()->widgetFromJsonFile("LegendDary_UI_Pause.json"));
  pauseLayer->setPosition(
      Vec2(size.width / 2 - pauseLayer->getContentSize().width / 2, 0));
  this->addChild(pauseLayer);

  Button *btnResume =
      dynamic_cast<Button *>(Helper::seekWidgetByName(pauseLayer, "Button_14"));
  btnResume->addTouchEventListener(CC_CALLBACK_2(HudLayer::resumeGame, this));

  Button *btnRestart = dynamic_cast<Button *>(
      Helper::seekWidgetByName(pauseLayer, "Button_14_0"));
  btnRestart->addTouchEventListener(CC_CALLBACK_2(HudLayer::retryStage, this));

  Button *btnStages = dynamic_cast<Button *>(
      Helper::seekWidgetByName(pauseLayer, "Button_14_1"));
  btnStages->addTouchEventListener(
      CC_CALLBACK_2(HudLayer::toStageSelect, this));
...
void HudLayer::showBuyLayer(Button *btn, std::string itemName) {
  ...
  //        Sprite* sptIcon =
  //        Sprite::createWithSpriteFrameName(WORLD->getItemSpriteName(itemType,
  //        itemName)); btn->addChild(sptIcon); sptIcon->setName("sptIcon");
...
void HudLayer::onFreeCoin(Ref *pSender, ui::Widget::TouchEventType eEventType) {
  if (TouchEventType::TOUCH_EVENT_ENDED != (TouchEventType)eEventType)
    return;
  if (GameManager::getInstance()->market == MARKET_CHINA_NO_IAP) {
...
void HudLayer::videoDone() {
  ...
  //            GameManager::getInstance()->getHudLayer()->scheduleOnce(schedule_selector(HudLayer::closeVideoDone),
  //            0.05);
  //        }else if(GameManager::getInstance()->videoIndex ==
  //        VIDEO_WEAPON_RENT){
...
void HudLayer::videoFailed() {
  if (GameManager::getInstance()->getHudLayer()) {
    GameManager::getInstance()->getHudLayer()->scheduleOnce(
        schedule_selector(HudLayer::closeVideoFailed), 0.1);
  }
}
void HudLayer::closeVideoDone(float dt) {
  ...
  Label *lbl = Label::createWithSystemFont(
      __String::createWithFormat("+%d", coinCount)->getCString(),
      GameManager::getInstance()->getFont(FONT_BITDUST_ONE), 30);
  sptCoin->addChild(lbl);
  lbl->setPosition(Vec2(45, 19));
  lbl->setAnchorPoint(Vec2(0, 0.5));

  lbl = Label::createWithSystemFont(
      __String::createWithFormat("EXP %d", expCount)->getCString(),
      GameManager::getInstance()->getFont(FONT_BITDUST_ONE), 30);
  sptCoin->addChild(lbl);
...
void HudLayer::closeVideoFailed(float dt) {
  ...
  Label *lbl = Label::createWithSystemFont(
      "ADS FAILED", GameManager::getInstance()->getFont(FONT_BITDUST_ONE), 30);
  ...
  /*Label* lbl =
  Label::createWithSystemFont(LanguageManager::getInstance()->getText(STR_VIDEO_FAILED),
  GameManager::getInstance()->getFont(FONT_DEFAULT), 24);
...
void HudLayer::coinLabelScheduler(float dt) {
  ...
  if (UserDefault::getInstance()->getBoolForKey(KEY_REMOVE_ADS_GET, false)) {
    if (increasingCounter == 0) {
      lblCoinCount->setString(
          __String::createWithFormat("%d\n+%d", increasingCoinCount / 2,
                                     increasingCoinCount / 2)
              ->getCString());
    } else {
...
void HudLayer::expLabelScheduler(float dt) {
  ...
  bool isDoubleExp =
      UserDefault::getInstance()->getBoolForKey(KEY_DOUBLE_EXP_GET, false);
  if (isDoubleExp) {
  ...
  if (increasingCounter >= increasingCounterMax) {
    ...
    increasingCounter = 0;
    if (isDoubleExp) {
      lblExpCount->setString(
          __String::createWithFormat("%d\n+%d", expCount / 2, expCount / 2)
              ->getCString());
    } else {
      lblExpCount->setString(
          __String::createWithFormat("%d", expCount)->getCString());
    }
    pbExp->setPercent(increasingExpCount * 100.0f / maxExpCount);
    this->unschedule(schedule_selector(HudLayer::expLabelScheduler));

    if (expCount >= maxExpCount) {
      sptMaxTalkBalloon->setPosition(sptMaxTalkBalloon->getPosition() +
                                     Vec2(0, 1000));
      sptMaxTalkBalloon->runAction(Sequence::create(
          ScaleTo::create(0.001, 0.5), ScaleTo::create(0.1, 1.1),
          ScaleTo::create(0.05, 1.0f), NULL));
    }

    btnNext->setVisible(true);
    btnNext->runAction(Sequence::create(ScaleTo::create(0.01, 0.5),
                                        ScaleTo::create(0.1, 1.1),
                                        ScaleTo::create(0.06, 1.0), NULL));
    btnRetry->setVisible(true);
    btnRetry->runAction(Sequence::create(ScaleTo::create(0.01, 0.5),
                                         ScaleTo::create(0.1, 1.1),
                                         ScaleTo::create(0.06, 1.0), NULL));
    btnHome->setVisible(true);
    btnHome->runAction(Sequence::create(ScaleTo::create(0.01, 0.5),
                                        ScaleTo::create(0.1, 1.1),
                                        ScaleTo::create(0.06, 1.0), NULL));
  }
...
void HudLayer::tryGameContinue(Ref *pSender,
  ...
  if (GameManager::getInstance()->getCoin() >= price) {
    ...
    float duration = 3;
    ImageView *darySleep = dynamic_cast<ImageView *>(
        Helper::seekWidgetByName(reviveLayer, "sptDary"));
    darySleep->runAction(
        EaseOut::create(MoveBy::create(duration, Vec2(0, 50)), 3));
    darySleep->loadTexture("backLight.png");
    darySleep->runAction(
        RepeatForever::create(RotateBy::create(duration, 360)));
    ...
    this->scheduleOnce(schedule_selector(HudLayer::reviveLater),
                       duration * 2 / 3);

    Button *btnRevive = dynamic_cast<Button *>(
        Helper::seekWidgetByName(reviveLayer, "btnContinue"));
    btnRevive->removeFromParentAndCleanup(true);
    Button *btnClose = dynamic_cast<Button *>(
        Helper::seekWidgetByName(reviveLayer, "btnClose"));
    btnClose->removeFromParentAndCleanup(true);
  } else {
    CallFuncN *call = CallFuncN::create(
        CC_CALLBACK_1(HudLayer::recommandCoinShopClosed, this));
  }
}
...
void HudLayer::closeWaitMessageBoxLaterAndShowSucess() {
  this->scheduleOnce(
      schedule_selector(HudLayer::closeWaitMessageBoxAndShowSuccess), 0.01);
}
...
void HudLayer::reviveFromVideo() {
  ...
  // #endif
  //    this->scheduleOnce(schedule_selector(HudLayer::playBackgroundMusicLater),
  //    2); GameManager::getInstance()->getWorld()->revive();
  //
  //    int count = reviveCount;
  //    count++;
  //    reviveCount = count;
...
void HudLayer::toStageSelect(Ref *pSender,
                             ui::Widget::TouchEventType eEventType) {
  if (TouchEventType::TOUCH_EVENT_ENDED != (TouchEventType)eEventType)
    return;
...
void HudLayer::closeSchedule(float dt) {
  ...
  if (toWhat == CLOSE_TO_NEXT) {
    ...
    if (GameManager::getInstance()->theme * 24 +
            GameManager::getInstance()->currentStageIndex ==
        GameManager::getInstance()->totalStage - 1) {
...
void HudLayer::nextStage(Ref *pSender, ui::Widget::TouchEventType eEventType) {
  ...
  if (GameManager::getInstance()->theme == 3 &&
      GameManager::getInstance()->currentStageIndex == 23) {
  ...
  if (GameManager::getInstance()->life > 0 ||
    ...
    if (!UserDefault::getInstance()->getBoolForKey(KEY_INFINITY_LIFE_GET,
      ...
      //                UserDefault::getInstance()->setIntegerForKey(KEY_LIFE_SPENT_TIME,
      //                ServerManager::getInstance()->getCurrentTime());
      //            }
    }
...
void HudLayer::onReplayVideo(Ref *pSender,
                             ui::Widget::TouchEventType eEventType) {
  if (TouchEventType::TOUCH_EVENT_ENDED != (TouchEventType)eEventType)
    return;
  if (GameManager::getInstance()->market == MARKET_CHINA_NO_IAP) {
...
void HudLayer::displayBossEnergy(const char *name) {
  ...
  //    bossEnergyBackground->setPosition(Vec2(size.width/2, size.height/2 -
  //    60)); bossEnergyBackground->setScaleY(0.5);
  bossEnergyBarWidth = 600;

  bossName = Label::createWithSystemFont(
      name, GameManager::getInstance()->getFont(FONT_VISITOR), 30);
  ...
  // bossEnergyBarYellow->setPosition(Vec2(size.width/2 - bossEnergyBarWidth/2,
  // size.height/2 + 180)); bossEnergyBarYellow->setAnchorPoint(Vec2(0, 0.5f));
...
void HudLayer::setBossEnergy(int percent) {
  ...
  bossEnergyBarBack->drawSolidRect(
      Vec2(size.width / 6 - 1, size.height * 2 / 3 - 3),
      Vec2(size.width / 6 + width + 1, size.height * 2 / 3 + 3),
      Color4F(235.0f / 255, 235.0f / 255, 235.0f / 255, 100 / 255.0f));
  Color4F color;
  if (percent < 20) {
  ...
  bossEnergyBar->drawSolidRect(
      Vec2(size.width / 6, size.height * 2 / 3 - 2),
      Vec2(size.width / 6 + energyLeft, size.height * 2 / 3 + 2), color);
  if (percent <= 0) {
  ...
  if (talkIndex < talkText.size()) {
    ...
    if (tutorialHowTo == nullptr) {
      ...
      if (lbl->lblNormal != nullptr) {
        lbl->lblNormal->enableShadow(Color4B(DARK_GRAY_3B, 255),
                                     cocos2d::Size(4, -4));
      }
    ...
    if (tutorialIndex < 11) {
      ...
      ImageView *imgContent =
          (ImageView *)tutorialHowTo->getChildByName("imgContent");
      ...
      lbl->setString(
          LM->getText(strmake("tutorialHowTo%d", tutorialIndex).c_str()));
    }
  }
}
void HudLayer::addListener() {
  touchListener = EventListenerTouchAllAtOnce::create();
  touchListener->onTouchesBegan = [&](const std::vector<Touch *> &touches,
                                      Event *evt) {
    log("onTouchesBegan");
    ...
    //        WORLD->showTalkText("what? asdf sadf dsfa dsfdasf asdf sadf dsaf
    //        ad fasd fasd f adsf dsaf adsf asd fsad fadsf asdf ",
    //        WHOSE_TALK_HERO);
  };
  touchListener->onTouchesMoved = [&](const std::vector<Touch *> &touches,
                                      Event *evt) {
    //        log("onTouchesMoved");
    //        log("x: %f, y: %f", touches.at(0)->getLocation().x,
    //        touches.at(0)->getLocation().y); chopad->TouchesMoved(touches,
    //        evt);
  };
  touchListener->onTouchesEnded = [&](const std::vector<Touch *> &touches,
                                      Event *evt) {
    //        log("onTouchesEnded");

    //        chopad->TouchesEnded(touches, evt);
  };
  touchListener->onTouchesCancelled = [&](const std::vector<Touch *> &touches,
                                          Event *evt) {
...
void HudLayer::showDialog(const char *message, const char *btn1,
                          const char *btn2) {
  CallFuncN *call =
      CallFuncN::create(CC_CALLBACK_1(HudLayer::messageBoxClosed, this));
...
void HudLayer::onEvent(float dt) {
  ...
  if (talkState == TALK_STATE_ASKING) {
    ...
    if (text.size() == 0 || text.compare(key) == 0) {
      ...
      GameManager::getInstance()->getWorld()->showTalkText(text.substr(5),
                                                           WHOSE_TALK_HERO);
    } else if (text.find(">") != std::string::npos) {
      std::string strText = text.substr((int)text.find(">") + 1);
      WORLD->showTalkText(strText, WHOSE_TALK_NPC,
                          text.substr(0, (int)text.find(">")));
    }
    //        else if(text.find("fairy>") != std::string::npos){
    //            GameManager::getInstance()->getWorld()->showTalkText(text.substr(6),
    //            WHOSE_TALK_NPC);
    //        }else if(text.find("fairy>") != std::string::npos){
    //            GameManager::getInstance()->getWorld()->showTalkText(text.substr(5),
    //            WHOSE_TALK_HERO);
    ...
    showOptions(
        LanguageManager::getInstance()->getText(
            StringUtils::format("event%d_option_0", eventIndex).c_str()),
        LanguageManager::getInstance()->getText(
            StringUtils::format("event%d_option_1", eventIndex).c_str()));
  } else if (talkState == TALK_STATE_CHOOSED) {
    std::string key = StringUtils::format("event%d_choose_%d_%d", eventIndex,
                                          answer, talkIndex);
    text = LanguageManager::getInstance()->getText(key.c_str());
    if (text.size() == 0) {
      endEvent();
...
void HudLayer::showBlackTopAndBottom() {
  ...
  blackBottom->runAction(
      Sequence::create(MoveBy::create(0.6f, Vec2(0, barHeight)), nullptr));
  blackBottom->setColor(Color3B::BLACK);

  if (eventIndex != 100 || true) {
    //        Sprite* sptBSkip =
    //        Sprite::createWithSpriteFrameName("bSkipPressed.png");
    //        blackBottom->addChild(sptBSkip);
    //        sptBSkip->setScale(4);
    //        sptBSkip->setPosition(Vec2(blackBottom->getContentSize().width -
    //        sptBSkip->getContentSize().width*sptBSkip->getScale()/2 - 60,
    //        blackBottom->getContentSize().height/2 +
    //        sptBSkip->getContentSize().height*sptBSkip->getScale()/2 + 4));
    ...
    lblSkip->setPosition(Vec2(
        blackBottom->getContentSize().width -
            lblSkip->getContentSize().width * lblSkip->getScale() / 2 - 60,
        blackBottom->getContentSize().height / 2 +
            lblSkip->getContentSize().height * lblSkip->getScale() / 2 - 50));
...
void HudLayer::hideBlackTopAndBottom() {
  if (blackTop == nullptr)
    return;
  blackTop->runAction(Sequence::create(
      MoveBy::create(1, Vec2(0, 80)),
      CallFunc::create(CC_CALLBACK_0(Sprite::removeFromParent, blackTop)),
      nullptr));
  blackBottom->runAction(Sequence::create(
      MoveBy::create(1, Vec2(0, -80)),
      CallFunc::create(CC_CALLBACK_0(Sprite::removeFromParent, blackBottom)),
      nullptr));
  showBtns();
}
...
void HudLayer::updateTalkBoxRope(float dt) {
  ...
  int y = 0;
  // horizontal
  for (int i = 0; i < sptTalkBox->getContentSize().width + ropeWidth;
       i += ropeWidth) {
    bool topExist = false;
  ...
  //        if(!leftExist){
  //            Sprite* sptRope =
  //            Sprite::createWithSpriteFrameName("ropeParticle.png");
  //            sptTalkBox->addChild(sptRope);
...
void HudLayer::readyToShowNextTalk() {
  ...
  if (stage->isInEvent) {
    if (talkState == TALK_STATE_QUESTIONING) {
      ...
      talkIndex = 0;
      sptTalkBox->getChildByTag(0)->runAction(Sequence::create(
          Blink::create(0.8, 4), DelayTime::create(1),
          CallFunc::create(CC_CALLBACK_0(HudLayer::readyToShowNextTalk, this)),
          NULL));
    } else {
      if (stage->imgTalkBox != nullptr &&
          stage->imgTalkBox->getChildByName("TOUCH") == nullptr) {
...
void HudLayer::onLeftRelease() { isCursorReadyToMove = true; }
void HudLayer::onRightRelease() { isCursorReadyToMove = true; }
void HudLayer::onUpRelease() { isCursorReadyToMove = true; }
void HudLayer::onDownRelease() { isCursorReadyToMove = true; }
void HudLayer::onLeftPress() {
  if (!isCursorReadyToMove)
    return;
...
void HudLayer::selectABCOption(Node *node) {
  ...
  if (selectedAbcOption != node && node != nullptr) {
    ...
    sptCursor->setPosition(node->getBoundingBox().origin +
                           Vec2(-sptCursor->getContentSize().width / 2 - 10,
                                node->getBoundingBox().size.height / 2));
    ...
    if (actionType == ACTION_TYPE_EXCHANGE) {
      ...
      ingredientCount =
          (ingredientCount / samShopExchangeRate) * samShopExchangeRate;
      if (inventory->isSlotAvailableForThisItem(
              samShopResultStoneName,
              getInventoryTypeForItemType(ITEM_TYPE_ETC),
              ingredientCount / samShopExchangeRate)) {
        ...
        if (GM->getCoin() >= totalFee) {
          ...
          inventory->addItem(samShopResultStoneName,
                             getInventoryTypeForItemType(ITEM_TYPE_ETC),
                             ingredientCount / samShopExchangeRate);
          ...
          abcLayer->getChildByName("lblOption1")
              ->runAction(Blink::create(0.5, 2));
          this->scheduleOnce(schedule_selector(HudLayer::closeABCLayerLater),
                             1);
        } else {
    ...
    int page =
        selectedInventorySlot->getParent()->getChildByName("lblPage")->getTag();
    ...
    if (index == 0) {
      ...
      if (itemType == ITEM_TYPE_ETC) {
        int ability =
            WORLD->itemStatTable[name].asValueMap().at("ability").asInt();
        if (ability > 0) {
...
void HudLayer::ddiyong(Node *node) {
  node->setScale(0.1f);
  node->runAction(Sequence::create(ScaleTo::create(0.2, 1.1),
...
void HudLayer::showOptions(std::string option0, std::string option1) {
  ...
  float height = 257;
  float scale = 0.3f;
  Label *lblOption0 = LanguageManager::getInstance()->getLocalizedLabel(
      StringUtils::format("A. %s", option0.c_str()).c_str(),
      Color4B(78, 78, 78, 255));
  ...
  Label *lblOption1 = LanguageManager::getInstance()->getLocalizedLabel(
      StringUtils::format("B. %s", option1.c_str()).c_str(),
      Color4B(78, 78, 78, 255));
  ...
  sptTalkBox->runAction(Sequence::create(
      ResizeTo::create(0.2f, cocos2d::Size(8, 257)),
      ResizeTo::create(0.5f, cocos2d::Size(640, height)),
      CallFunc::create(CC_CALLBACK_0(HudLayer::onTalkBoxResizeDone, this)),
      nullptr));
  inputShouldWait = true;
  //    sptTalkBox->runAction(Sequence::create(ScaleTo::create(0.2f, 0.1f,
  //    1),ScaleTo::create(0.5f, 1),
  //    CallFunc::create(CC_CALLBACK_0(HudLayer::onTalkBoxResizeDone, this)),
  //    nullptr));
...
void HudLayer::showEnding(int index) {
  ...
  sptBack->setColor(Color3B::BLACK);
  sptBack->setCapInsets(cocos2d::Rect(3, 3, sptBack->getContentSize().width - 6,
                                      sptBack->getContentSize().height - 6));
...
void HudLayer::updateEnding(float dt) {
  ...
  int padding = 10;
  std::string text = LM->getText(
      StringUtils::format("ending%d_%d", endingIndex, endingTalkIndex).c_str());
  bool isLeft = true;
  std::string profile;
  std::string name;
  if (text.find("system>") != std::string::npos) {
    ...
    if (text.size() > 0) {
      ...
      endingTalkIndex++;
      lastEndingTalkY -=
          utils::getCascadeBoundingBox(notice).size.height + padding;
    } else {
  ...
  log("box width before: %f",
      utils::getCascadeBoundingBox(profilePic).size.width);
  Label *lbl = LM->getLocalizedLabel(name.c_str(), Color4B::WHITE);
  lbl->setAnchorPoint(Vec2(isLeft ? 0 : 1, 1));
  lbl->setPosition(
      Vec2(isLeft ? profilePic->getContentSize().width + padding : -padding,
           profilePic->getContentSize().height));
  ...
  if (text.find("shyface") != std::string::npos) {
    ...
    sptFace->runAction(
        Sequence::create(DelayTime::create(0.3f), FlipX::create(true),
                         DelayTime::create(0.3f), FlipX::create(false),
                         DelayTime::create(0.3f), FlipX::create(true), NULL));
    sptFace->setPosition(
        Vec2(lbl->getPositionX() +
                 sptFace->getContentSize().width / 2 * (isLeft ? 1 : -1),
             lbl->getPositionY() -
                 lbl->getContentSize().height * lbl->getScaleY() - padding));
  } else {
    ImageView *imgTalkBox = ImageView::create("talkBoxFrame.png");
    imgTalkBox->setAnchorPoint(Vec2(isLeft ? 0 : 1, 1));
    imgTalkBox->setPosition(Vec2(
        lbl->getPositionX(),
        lbl->getPositionY() - lbl->getContentSize().height * lbl->getScaleY()));
    imgTalkBox->setCapInsets(
        cocos2d::Rect(3, 3, imgTalkBox->getContentSize().width - 6,
                      imgTalkBox->getContentSize().height - 6));
    ...
    cocos2d::Size talkSize = cocos2d::Size(
        lblTalkShadow->getContentSize().width * lblTalkShadow->getScale(),
        lblTalkShadow->getContentSize().height * lblTalkShadow->getScale());
    float width = 66;
    ...
    imgTalkBox->setContentSize(
        cocos2d::Size(talkSize.width + padding, talkSize.height + padding));
    lblTalkShadow->setPosition(
        imgTalkBox->getPosition() +
        Vec2((isLeft ? 1 : -1) * imgTalkBox->getContentSize().width / 2,
             -padding / 2 - talkSize.height / 2));
    // resize talkbox end

    sptPointer->setPosition(
        imgTalkBox->getPosition() +
        Vec2((isLeft ? -1 : 1) * sptPointer->getContentSize().height / 2 +
                 (isLeft ? 3 : -3),
             -5));
    sptPointer->setRotation(isLeft ? 90 : -90);
  }
  log("box width after: %f",
      utils::getCascadeBoundingBox(profilePic).size.width);
  endingTalkIndex++;
  lastEndingTalkY -=
      utils::getCascadeBoundingBox(profilePic).size.height + padding;
...
PPLabel *HudLayer::showInstanceMessage(std::string msg) {
  ...
  lbl->runAction(Sequence::create(
      MoveBy::create(dur, Vec2(0, distanceToMove)), DelayTime::create(2),
      MoveBy::create(dur, Vec2(0, -distanceToMove)),
...
//         return false;
//     }
//     UD->setStringForKey(strmake(KEY_INVENTORY_ITEM_FORMAT, index).c_str(),
//     name); addItemCountForInventorySlot(index);
//
//     return true;
// }
// int HudLayer::inventory->getItemCount(int slot){
//     return UD->getIntegerForKey(strmake(KEY_INVENTORY_ITEM_COUNT_FORMAT,
//     slot).c_str(), 0);
// }
...
//     if(count == 0){
//         UD->setStringForKey(strmake(KEY_INVENTORY_ITEM_FORMAT, slot).c_str(),
//         "deleted"); rearrangeInventoryItems();
//     }
// }
// void HudLayer::addItemCountForInventorySlot(int slot){
//     int count = inventory->getItemCount(slot);
//     count++;
//     UD->setIntegerForKey(strmake(KEY_INVENTORY_ITEM_COUNT_FORMAT,
//     slot).c_str(), count);
// }
...
//     int count = 0;
//     while(true){
//         if(UD->getStringForKey(strmake(KEY_INVENTORY_ITEM_FORMAT,
//         count).c_str(), "").size() == 0){
//             break;
//         }
//         count++;
...
//     }
//     UD->setStringForKey(strmake(KEY_INVENTORY_ITEM_FORMAT, index).c_str(),
//     itemName); addItemCountForInventorySlot(index);
//
//     return true;
// }
//
...
//             std::string item =
//             UD->getStringForKey(strmake(KEY_INVENTORY_ITEM_FORMAT,
//             count).c_str(), "");
//             UD->setStringForKey(strmake(KEY_INVENTORY_ITEM_FORMAT,
//             count-1).c_str(), item); int itemCount =
//             inventory->getItemCount(count);
//             UD->setIntegerForKey(strmake(KEY_INVENTORY_ITEM_COUNT_FORMAT,
//             count-1).c_str(), itemCount);
//         }else{
//             if(UD->getStringForKey(strmake(KEY_INVENTORY_ITEM_FORMAT,
//             count).c_str(), "").compare("deleted") == 0){
...
//     }
//     if(slot < 0){
//         assert(strmake("not slot found for the item name: %s",
//         itemName.c_str()).c_str());
//     }
//
//     int count = inventory->getItemCount(slot);
//     count--;
...
Label *HudLayer::changeTextWithLabel(Text *txt, float width) {
  ...
  log("alignment: %d, %d", (int)txt->getTextHorizontalAlignment(),
      (int)txt->getTextVerticalAlignment());
...
Sprite *HudLayer::changeCoinToAnimation(Node *imgCoin) { return nullptr; }
Sprite *HudLayer::changeGemToAnimation(Node *imgCoin) { return nullptr; }
void HudLayer::showInventory() {}
Sprite *HudLayer::addBToCloseToLayer(Node *layer) {
  Sprite *sptBToClose =
      Sprite::createWithSpriteFrameName("bToCloseReleased.png");
...
//     return UD->getStringForKey(strmake(KEY_INVENTORY_ITEM_FORMAT,
//     slot).c_str(), "");
// }
...
bool HudLayer::isPopupAvailable() { return true; }
...
void HudLayer::ropeAround(Node *node) {
  ...
  int ropeScale = 4;
  // horizontal
  for (int i = 0; i < node->getContentSize().width + ropeWidth;
       i += ropeWidth) {
    x = i;
...
void HudLayer::showIntro() {
  ...
  sptFrame->runAction(
      Sequence::create(DelayTime::create(1), FadeTo::create(2, 150), NULL));
  ...
  lbl->runAction(MoveBy::create(
      dur, Vec2(0, lbl->getBoundingBox().size.height + size.width)));

  GM->nextScene = STAGE_TITLE;
  //    this->runAction(Sequence::create(DelayTime::create(dur - 2),
  //    CallFunc::create(CC_CALLBACK_0(HudLayer::showBIAndReplaceScene, this)),
  //    NULL));
...
void HudLayer::showAlchyShop() {
  ...
  lbl->setString(strmake("A.%s   B.%s", LM->getText("combine").c_str(),
                         LM->getText("cancel").c_str()));
...
void HudLayer::showDoctorShop() {
  ...
  lbl->setString(strmake("A.%s   B.%s", LM->getText("evolution").c_str(),
                         LM->getText("cancel").c_str()));
...
void HudLayer::selectDoctorShopItem(Ref *ref) {
  ...
  std::string ingredient0 =
      WORLD->getEvolutionStat(tier, strmake("ingredient %d", btn->getTag()));
...
void HudLayer::showCredit() {
  ...
  //    sptRect->setCapInsets(cocos2d::Rect(3, 3,
  //    sptRect->getContentSize().width - 6, sptRect->getContentSize().height -
  //    6)); sptRect->setScale9Enabled(true); this->addChild(sptRect, 8);
  ...
  for (int i = 0; i < 5; i++) {
    ...
    if (i == 1 || i == 2) {
      lbl->setString(strmake("%s\n%s",
                             LMText(strmake("credit%d", i).c_str()).c_str(),
                             LMText(strmake("credit%d_1", i).c_str()).c_str()));
    } else {
      LM->setLocalizedString(lbl, strmake("credit%d", i));
    }
    lbl->setOpacity(0);
    lbl->runAction(Sequence::create(
        DelayTime::create(1), DelayTime::create(i * 3), FadeIn::create(1),
        DelayTime::create(1), FadeOut::create(1), NULL));
    this->addChild(lbl, 9);
    lbl->setPosition(Vec2(size.width / 2, size.height - 714 / 2 - 100));
  }
  GM->nextScene = STAGE_FIELD;
  //    this->runAction(Sequence::create(DelayTime::create(3*6 + 1),
  //    CallFunc::create(CC_CALLBACK_0(HudLayer::showBIAndReplaceScene, this)),
  //    NULL));
}
...
void HudLayer::selectPeterInsurancePlan(int index) {
  Node *newlySelectedPlan =
      peterShopLayer->getChildByName(strmake("lblOption%d", index));
  if (newlySelectedPlan == selectedPeterItem) {
    ...
    newlySelectedPlan->runAction(
        Sequence::create(Blink::create(0.5, 2),
                         CallFuncN::create(CC_CALLBACK_1(
                             HudLayer::onConfirmInsurancePlan, this)),
                         nullptr));
  } else {
...
void HudLayer::showPotSoul(cocos2d::Vec2 pos) {
  ...
  spt->runAction(Sequence::create(
      EaseInOut::create(MoveBy::create(1, Vec2(0, 100)), 2),
      EaseInOut::create(MoveTo::create(0.7f, targetPos), 2),
      CallFunc::create(CC_CALLBACK_0(HudLayer::onPotSoulMoveDone, this)),
      SPT_REMOVE_FUNC, NULL));
  Node *counter = hudLayer->getChildByName("potCounter");
  counter->setPosition(Vec2(900, 1166));
  counter->runAction(Sequence::create(
      DelayTime::create(1), MoveTo::create(0.4f, Vec2(710, 1166)),
      DelayTime::create(1), MoveTo::create(0.3f, Vec2(900, 1166)), NULL));
  counter->runAction(
      Sequence::create(DelayTime::create(1), FadeIn::create(0.4f),
                       DelayTime::create(1), FadeOut::create(0.3f), NULL));
...
void HudLayer::onPotSoulMoveDone() {
  ...
  int potCount = UDGetInt(KEY_POT_SOUL_COUNT, 0);
  lbl->setString(Value(potCount).asString());
  lbl->runAction(Sequence::create(ScaleTo::create(0.1f, 1.2f),
                                  ScaleTo::create(0.1f, 1), NULL));
}

void HudLayer::onReviveByGem() {
  int gemPrice =
      Value(((Text *)deadPopup->getChildByName("lblGemPrice"))->getString())
          .asInt();
...
void HudLayer::showWinPopup(bool win) {
  ...
  if (WORLD->gameMode == GAME_MODE_PVP6 || WORLD->gameMode == GAME_MODE_PVP12) {
    ...
    btn->runAction(Sequence::create(
        DelayTime::create(1), EaseBackOut::create(ScaleTo::create(0.3f, 1)),
        nullptr));
    btn->runAction(
        Sequence::create(DelayTime::create(1), FadeIn::create(0), nullptr));
    ImageView *img;
    int rank;
    Text *lbl;
    ...
    int playerTrophy = UDGetInt(
        WORLD->gameMode == GAME_MODE_PVP6 ? KEY_PVP6_TROPHY : KEY_PVP12_TROPHY,
        1000);

    int timeLeftToSeasonOff = BSM->getTimeLeftToSunday();
    int lastTimeLeftToSeaonOff =
        UDGetInt(WORLD->gameMode == GAME_MODE_PVP6 ? KEY_PVP6_LAST_TIME_LEFT
                                                   : KEY_PVP12_LAST_TIME_LEFT,
                 -1);
    if (lastTimeLeftToSeaonOff < 0) {
      UDSetInt(WORLD->gameMode == GAME_MODE_PVP6 ? KEY_PVP6_LAST_TIME_LEFT
                                                 : KEY_PVP12_LAST_TIME_LEFT,
               timeLeftToSeasonOff);
    } else if (lastTimeLeftToSeaonOff < timeLeftToSeasonOff) { // season changed
      playerTrophy = 1000;
      UDSetInt(WORLD->gameMode == GAME_MODE_PVP6 ? KEY_PVP6_TROPHY
                                                 : KEY_PVP12_TROPHY,
               1000);
      UDSetInt(WORLD->gameMode == GAME_MODE_PVP6 ? KEY_PVP6_LAST_TIME_LEFT
                                                 : KEY_PVP12_LAST_TIME_LEFT,
    ...
    for (int i = 0; i < units.size(); i++) {
      ...
      if (info != nullptr && info) {
        ...
        if (unitType >= 0) {
          //                    EnemyBase* unit =
          //                    EnemyBase::createEnemy(unitType, 1, 0,
          //                    GM->getUnitName(unitType).c_str());
          ...
          unit->setPosition(img->getPosition() +
                            Vec2(0, -img->getContentSize().height / 4));
          ...
          if (!isAlive) {
            ...
            spt->runAction(Sequence::create(
                DelayTime::create(0.5f), FadeIn::create(0), animate,
    ...
    for (int i = 0; i < units.size(); i++) {
      ...
      if (info != nullptr && info) {
        ...
        if (unitType >= 0) {
          ...
          Node *unit = GM->getHeroSpine(unitType);
          unit->setScale(0.5f);
          img = (ImageView *)ndResult->getChildByName(
              strmake("imgEnemy_%d", index));
          info = GM->getUnitInfoFromString(units.at(i).asString());
          rank = info->rank;
    ...
    GameSharing::logFB(
        strmake("STAGE %d %s", GM->currentStageIndex, win ? "WIN" : "LOSE")
            .c_str());
    ...
    //    Label* lbl = LM->getLocalizedLabel(strResult.c_str(), Color4B::BLACK,
    //    70);
    Node *title = PPLabel::create(strResult, 70, DARK_GRAY_3B, true, false,
                                  TextHAlignment::CENTER, false);
    ...
    for (int i = 0; i < 5; i++) {
      if (i == 0) {
        customLabel =
            PPLabel::create(LM->getText("time"), fontSize, DARK_GRAY_3B, true,
                            false, TextHAlignment::LEFT, false);
        customLabel->setName("lblTimeTitle");
        spt = Sprite::create("iconTime.png");
        spt->setName("sptTime");
        //            lbl = LM->getLocalizedLabel("Time", Color4B::BLACK,
        //            fontSize); lbl->setName("lblTimeTitle");
      } else if (i == 1) {
        customLabel = PPLabel::create(
            strmake("%s (%s/%s)", LM->getText("gold text").c_str(),
                    LM->getText("used").c_str(), LM->getText("earned").c_str()),
            fontSize, DARK_GRAY_3B, true, false, TextHAlignment::LEFT, false);
        customLabel->setName("lblGoldTitle");
        spt = Sprite::create("iconGold.png");
        spt->setName("sptGold");
        //            lbl = LM->getLocalizedLabel("Gold (Used/Earnd)",
        //            Color4B::BLACK, fontSize); lbl->setName("lblGoldTitle");
      } else if (i == 2) {
        customLabel = PPLabel::create(
            strmake("%s (%s/%s)", LM->getText("lumber text").c_str(),
                    LM->getText("used").c_str(), LM->getText("earned").c_str()),
            fontSize, DARK_GRAY_3B, true, false, TextHAlignment::LEFT, false);
        customLabel->setName("lblLumberTitle");
        spt = Sprite::create("iconLumber.png");
        spt->setName("sptLumber");
        //            lbl = LM->getLocalizedLabel("Lumber (Used/Earnd)",
        //            Color4B::BLACK, fontSize); lbl->setName("lblLumberTitle");
      } else if (i == 3) {
        customLabel = PPLabel::create(LM->getText("produced unit"), fontSize,
                                      DARK_GRAY_3B, true, false,
                                      TextHAlignment::LEFT, false);
        customLabel->setName("lblProduceTitle");
        spt = Sprite::create("iconPopulation.png");
        spt->setName("sptPopulation");
        //            lbl = LM->getLocalizedLabel("Produced Unit",
        //            Color4B::BLACK, fontSize);
        //            lbl->setName("lblProduceTitle");
      } else if (i == 4) {
        customLabel =
            PPLabel::create(LM->getText("killed unit"), fontSize, DARK_GRAY_3B,
        ...
        //            lbl = LM->getLocalizedLabel("Killed Unit", Color4B::BLACK,
        //            fontSize); lbl->setName("lblKillTitle");
      }
      ...
      if (i == 0) {
        customLabel = PPLabel::create("00:00:00", fontSize, DARK_GRAY_3B, true,
                                      false, TextHAlignment::RIGHT, false);
        customLabel->setName("lblTime");
        //            lbl = LM->getLocalizedLabel("00:00:00", Color4B::BLACK,
        //            fontSize); lbl->setName("lblTime");
      } else if (i == 1) {
        customLabel = PPLabel::create("0/0", fontSize, DARK_GRAY_3B, true,
                                      false, TextHAlignment::RIGHT, false);
        customLabel->setName("lblGold");
        //            lbl = LM->getLocalizedLabel("0/0", Color4B::BLACK,
        //            fontSize); lbl->setName("lblGold");
      } else if (i == 2) {
        customLabel = PPLabel::create("0/0", fontSize, DARK_GRAY_3B, true,
                                      false, TextHAlignment::RIGHT, false);
        customLabel->setName("lblLumber");
        //            lbl = LM->getLocalizedLabel("0/0", Color4B::BLACK,
        //            fontSize); lbl->setName("lblLumber");
      } else if (i == 3) {
        customLabel = PPLabel::create("0", fontSize, DARK_GRAY_3B, true, false,
                                      TextHAlignment::RIGHT, false);
        customLabel->setName("lblProduce");
        //            lbl = LM->getLocalizedLabel("0", Color4B::BLACK,
        //            fontSize); lbl->setName("lblProduce");
      } else if (i == 4) {
        customLabel = PPLabel::create("0", fontSize, DARK_GRAY_3B, true, false,
                                      TextHAlignment::RIGHT, false);
        customLabel->setName("lblKill");
        //            lbl = LM->getLocalizedLabel("0", Color4B::BLACK,
        //            fontSize); lbl->setName("lblKill");
      }
    ...
    if (GM->playedMapID.length() > 0) {
      ...
      //            btn->setContentSize(cocos2d::Size(300, 200));
      //            btn->setScale9Enabled(true);
      //            btn->addClickEventListener(CC_CALLBACK_0(HudLayer::onDislikeClick,
      //            this)); btn->setPosition(Vec2(size.width/2 - 180,
      //            size.height/2 - 450));
      //
    ...
    spt->setPosition(
        Vec2(size.width / 2, size.height / 2 + 850 / 2 + 110 -
                                 spt->getContentSize().height / 2));
    ...
    if (!win) {
      spt->runAction(Sequence::create(
          ...
          EaseIn::create(RotateBy::create(0.5f, -90), 2), NULL));
    }

    this->schedule(schedule_selector(HudLayer::updateResultPopup), 0.1f);

    int chapter = WORLD->stageIndex / 12;
    //        UDSetInt(strmake(KEY_`_STAGE_INDEX_FORMAT, chapter).c_str(), 1);
    //        // test
    ...
    if (win && timeLeft <= 0 && stageIndex == chestIndex && !isArena) {
      ...
      std::string strTimeKey =
          strmake(KEY_CAMPAIGN_CHEST_GET_TIME_FORMAT, chapter).c_str();
      ...
      btn->addClickEventListener(
          CC_CALLBACK_1(HudLayer::onOpenCampaignChestClick, this));
      btn->setOpacity(0);
      ...
      btn->addClickEventListener(
          CC_CALLBACK_0(HudLayer::onOkCampaignChestClick, this));
    }
#endif
  }
}
void HudLayer::onOpenCampaignChestClick(Ref *ref) {
  ...
  imgChest->runAction(Sequence::create(
      ...
      CallFunc::create(CC_CALLBACK_0(HudLayer::onOpenCampaignChest, this)),
      NULL));
}
void HudLayer::onOpenCampaignChest() {
  ...
  btn->runAction(Sequence::create(DelayTime::create(1.2f),
                                  JumpBy::create(0.5f, Vec2(0, 650), 200, 1),
                                  NULL));
}
...
void HudLayer::showPremiumRetry() {
  ...
  imgBack->runAction(
      EaseInOut::create(ResizeTo::create(0.3, cocos2d::Size(1334, 1038)), 2));

  ImageView *contentBox = (ImageView *)layer->getChildByName("contentBox");
  contentBox->runAction(
  ...
  btn->addClickEventListener(
      CC_CALLBACK_0(HudLayer::onBuyPremiumRetryClick, this));
  btn->runAction(
      Sequence::create(DelayTime::create(0.3), FadeIn::create(0.5f), NULL));
...
#if (CC_TARGET_PLATFORM != CC_PLATFORM_WIN32)
  if (WORLD->isGameOver && !WORLD->isMultiplay &&
      !UDGetBool(KEY_PREMIUM_START, false) &&
      (WORLD->gameMode == GAME_MODE_NORMAL ||
       WORLD->gameMode == GAME_MODE_HARD) &&
      GM->currentStageIndex != STAGE_CUSTOM) {
    showPremiumRetry();
  }
#endif
...
void HudLayer::onOkFromWinPopup(Ref *ref) {
  ...
  if (GM->playedMapID.length() > 0) {
    Node *layer = this->getChildByName("winPopup");
    Node *btnLike = layer->getChildByName("btnLike");
    int tag = btnLike->getTag();
    std::string mapId = GM->playedMapID;
    GM->playedMapID = "";
    if (tag == 1) {
      GM->pendingMapLikeMapId = mapId;
      GM->pendingMapLike = 1;
      GM->pendingMapDislike = 0;
      GM->pendingMapSuccess = isWon ? 1 : 0;
      GM->pendingMapFail = isWon ? 0 : 1;
      GM->requestOnlineAccount(this, PENDING_ONLINE_ACTION_MAP_LIKE);
    } else {
      BSM->uploadCustomMapResult(mapId, 0, 0, isWon ? 1 : 0, isWon ? 0 : 1);
    }
  }
  if (!UDGetBool(KEY_RATE_POPUP_NEVER_SHOW, false) &&
      (GM->market == MARKET_PAID || GM->market == MARKET_FREE)) { // review test
    //    if(true){ // test
    Node *popup = Node::create();
    this->addChild(popup, 200);
    ImageView *imgBack = ImageView::create("uiBox.png");
    ...
    Node *title = PPLabel::create(LM->getText("enjoy game"), 70, DARK_GRAY_3B,
                                  true, false, TextHAlignment::CENTER, false);
    ...
    title = PPLabel::create(LM->getText("rate game"), 50, DARK_GRAY_3B, true,
                            false, TextHAlignment::CENTER, false);
    title->setPosition(Vec2(size.width / 2, size.height / 2 - 150));
    popup->addChild(title);

    EnemyBase *unit =
        EnemyBase::createEnemy(UNIT_TROLL, 0, 0, "trollStand0.png");
    popup->addChild(unit);
    ...
    btn->setScale9Enabled(true);
    btn->addClickEventListener(
        CC_CALLBACK_1(HudLayer::onReivewPopupButtonClick, this));
    btn->setPosition(Vec2(size.width / 2 + 500, size.height / 2 - 450));
    addLabelToButton("ok", btn, false, DARK_GRAY_3B);
  ...
  if (GM->currentStageIndex == 11 && isWin) {
    ...
    if (isWin) {
      stage++;
      if (stage <
          13) { // test  5 for google indie festival 13(12 + bonus) for launch
                //                Node* temp = Node::create();
                //                temp->setTag(stage);
  ...
  if (btn->getTag() == 0) {
    if (GM->market == MARKET_PAID) {
      ...
      Application::getInstance()->openURL(
          "itms-apps://itunes.apple.com/app/id1389133177?action=write-review");
      ...
      Application::getInstance()->openURL(
          ...
          "details?id=com.magmon.cartooncraftf");
#endif
    }
...
void HudLayer::arrangeMenu(cocos2d::Vec2 pos) {
  if (GM->currentStageIndex == STAGE_LOBBY)
    return;
  const float menuScale = WORLD->getWorkerMenuScale();
  const float radius = WORLD->getWorkerMenuRadius();
  const float centerOffsetY = WORLD->getWorkerMenuCenterOffsetY();
  for (int i = 0; i < 6; i++) {
    Button *btn = (Button *)WORLD->getChildByName(strmake("btnMenu%d", i));
    btn->setPosition(pos + Vec2(0, centerOffsetY));
    btn->stopAllActions();
    btn->setOpacity(0);
    btn->runAction(FadeIn::create(0.16f));
    btn->setScale(menuScale);
    float angle = i * 60 - 120;
    //        btn->setEnabled(false);
    btn->runAction(Sequence::create(
        EaseOut::create(
            MoveBy::create(0.16f, Vec2(cos(angle * 3.14f / -180) * radius,
                                       sin(angle * 3.14f / -180) * radius)),
            2),
        nullptr));
...
void HudLayer::dismissMenu() {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    if (btnMenu == nullptr || !btnMenu->isVisible())
      continue;
    anyVisible = true;
    Button *btn = (Button *)btnMenu;
    float angle = i * 60 - 120;
    Vec2 inward = Vec2(-cos(angle * 3.14f / -180) * radius,
                       -sin(angle * 3.14f / -180) * radius);
    btn->stopAllActions();
    int menuIndex = i;
    btn->runAction(Sequence::create(
        Spawn::create(EaseIn::create(MoveBy::create(duration, inward), 2),
                      FadeOut::create(duration), nullptr),
        CallFunc::create(
            [this, menuIndex]() { setMenu(menuIndex, BTN_TYPE_NONE); }),
        nullptr));
  }
  if (anyVisible) {
    WORLD->selectedCommand = COMMAND_NOTHING;
...
void HudLayer::update(float dt) {
  ...
  if (lblVs != nullptr) {
    ...
    if (timeLeft < 0) {
      lblVs->setString("0");
      GM->getWorld()->endGame(imgPvpPlayerEnergy->getContentSize().width >=
                              imgPvpEnemyEnergy->getContentSize().width);
    } else if (timeLeft < 60) {
...
void HudLayer::setPvpUI(int playerHPMax, int enemyHPMax) {
  ...
  lbl->setString(
      Value(UDGetInt(WORLD->gameMode == GAME_MODE_PVP6 ? KEY_PVP6_TROPHY
                                                       : KEY_PVP12_TROPHY,
                     1000))
          .asString());
...
void HudLayer::updatePvpUI(int playerHP, int enemyHP) {
  imgPvpPlayerEnergy->setContentSize(
      cocos2d::Size(1000.0f * playerHP / playerHpMax, 115.82f));
  if (playerHP / playerHpMax < 0.3) {
    imgPvpPlayerEnergy->setColor(Color3B(248, 59, 7));
  }
  imgPvpEnemyEnergy->setContentSize(
      cocos2d::Size(1000.0f * enemyHP / enemyHpMax, 115.82f));
...
void HudLayer::handlePcControlGroupRecall(int groupIndex, bool addToSelection) {
  ...
  ValueVector list = GM->split(groupStr, "|");
  std::vector<int> aliveIDs;
  for (int i = 0; i < (int)list.size(); i++) {
    std::string s = list.at(i).asString();
    if (s.empty())
      continue;
    int unitID = list.at(i).asInt();
    for (auto unit : WORLD->heroArray) {
      if (unit->unitID == unitID) {
        aliveIDs.push_back(unitID);
        break;
      }
    }
  }
  ...
  // Rebuild group string filtering dead units
  std::string newStr = "";
  for (int id : aliveIDs) {
    newStr = strmake("%s%d|", newStr.c_str(), id);
  }
  groupStr = newStr.substr(0, newStr.length() - 1);

  // Double-press within 0.5s → center camera
  DWORD now = GetTickCount();
  bool doCenter = false;
  if (pcControlGroupLastKey == groupIndex &&
      (now - pcControlGroupLastPressTime) < 500) {
    doCenter = true;
  }
  pcControlGroupLastKey = groupIndex;
  pcControlGroupLastPressTime = now;
  ...
  if (doCenter) {
    ...
    WORLD->moveScreen(-centerPos * WORLD->layerScale + size / 2);
  }
}
#endif
