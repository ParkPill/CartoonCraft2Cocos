...
//         theRoom->onStateChange = CC_CALLBACK_1(HelloWorld::onRoomStateChange,
//         this);
//
...
void HelloWorld::updateUnitMoveNew(float dt) {
  for (auto unit : heroArray) {
    if (unit->unitAct == UNIT_ACT_RESTING_FOR_NEXT_TARGET_SEARCH) {
      ...
      //            if (unit->unitAct == UNIT_ACT_NONE || unit->unitActDetail ==
      //            UNIT_ACT_DETAIL_IDLE || unit->unitAct ==
      //            UNIT_ACT_ATTACK_DDANG) {
...
void HelloWorld::createMissile(std::string strMsName,
  ...
  ms->runAction(Sequence::create(
      ...
      CallFuncN::create(CC_CALLBACK_1(HelloWorld::checkMissileHit, this)),
      NULL));
...
Movable *HelloWorld::createMissile(int missileType, int dmg, bool visible,
  ...
  sptMissile->runAction(Sequence::create(
      ...
      CallFuncN::create(CC_CALLBACK_1(HelloWorld::missileMoveDone, this)),
      nullptr));
...
void HelloWorld::onKeyReleased(EventKeyboard::KeyCode keyCode,
                               Event *unused_event) {
...
void HelloWorld::showWarpAnimation() {
  ...
  sptBeam->runAction(Sequence::create(
      ...
      CallFunc::create(CC_CALLBACK_0(HelloWorld::checkEnding, this)), NULL));
  Vec2 pos;
  ...
  this->runAction(Sequence::create(
      ...
      CallFunc::create(CC_CALLBACK_0(HelloWorld::stopWoongSound, this)),
      nullptr));
  shakeScreen(6);

  //    this->runAction(Sequence::create(DelayTime::create(1),
  //    CallFunc::create(CC_CALLBACK_0(HelloWorld::testFunction, this)), NULL));
}
...
void HelloWorld::warp() {
  ...
  this->runAction(Sequence::create(
      DelayTime::create(4),
      CallFunc::create(CC_CALLBACK_0(HelloWorld::goToNextScene, this)), NULL));
}
...
void HelloWorld::destroyDestructable(Movable *drop) {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    part->runAction(Sequence::create(
        DelayTime::create(3),
        CallFuncN::create(CC_CALLBACK_1(HelloWorld::coinWaitDone, this)),
        NULL));
...
void HelloWorld::attackEnemy(EnemyBase *drop, int damage) {
  ...
  this->runAction(Sequence::create(
      ...
      CallFunc::create(CC_CALLBACK_0(HelloWorld::resumeLayer, this)), NULL));
}
          ...
          if (!drop->immortal) {
            ...
            if (false && rand() % 100 > 100 - DEFAULT_CRITICAL - extraCritical -
              ...
              critical->runAction(
                  Sequence::create(DelayTime::create(0.7), FadeOut::create(0.3),
                                   CallFuncN::create(CC_CALLBACK_1(
                                       HelloWorld::spriteMoveDone, this)),
                                   NULL));
...
void HelloWorld::shakeScreenOnce() { shakeScreen(2); }
void HelloWorld::shakeScreen(int count) {
  Vector<FiniteTimeAction *> array;
  for (int i = 0; i < count; i++) {
    //        array.pushBack(CallFunc::create(CC_CALLBACK_0(HelloWorld::shakeScreenFirst,
    //        this))); array.pushBack(DelayTime::create(0.03));
    //        array.pushBack(CallFunc::create(CC_CALLBACK_0(HelloWorld::shakeScreenSecond,
    //        this))); array.pushBack(DelayTime::create(0.05));
    array.pushBack(MoveBy::create(0.07, Vec2(4, 0)));
    array.pushBack(MoveBy::create(0.07, Vec2(-4, 0)));
  }
  //    array.pushBack(CallFunc::create(CC_CALLBACK_0(HelloWorld::shakeScreenEnd,
  //    this))); array.pushBack(DelayTime::create(0.03));
...
void HelloWorld::shakeScreenEnd() { extraCameraPos = Vec2::ZERO; }
...
void HelloWorld::removeIncompleteBuilding(EnemyBase *building) {
  if (building == nullptr) {
    return;
  }
...
bool HelloWorld::compareFourTiles(int t, int l, int r, int b, int x, int y,
                                  TMXLayer *stageLayer) {
  bool result = true;
...
bool HelloWorld::compareNineTiles(int lt, int t, int rt, int l, int r, int lb,
...
Label *HelloWorld::showLabelFromPool(Node *parent, cocos2d::Vec2 pos,
  ...
  lbl->runAction(Sequence::create(
      ...
      CallFuncN::create(CC_CALLBACK_1(HelloWorld::backToLabelPool, this)),
      NULL));
  labelPoolIndex++;
...
void HelloWorld::runEffect(int effect, Vec2 point, float angle) {
  ...
  if (effect == EFFECT_EXPLODE_SMALL) {
    ...
    spt->runAction(Sequence::create(
        ...
        CallFuncN::create(CC_CALLBACK_1(HelloWorld::enemyBaseMoveDone, this)),
        NULL));
    spt->runAction(
...
void HelloWorld::healHeroesNearPoint(Movable *healer, Vec2 point, int hp,
                                     int heroCount) {
  int index = 0;
  if (healer->alliSide == WHICH_SIDE_HERO) {
    for (auto unit : heroArray) {
...
/*
void HelloWorld::addGlowEffect(Sprite* sprite,const Color3B& colour, const
cocos2d::Size& size)
{
...
void HelloWorld::setEntireMap(int stage) {
  ...
  if (gameMode == GAME_MODE_PVP6 || gameMode == GAME_MODE_PVP12) {
    ...
    theMap = cocos2d::TMXTiledMap::create(
        strmake("battleSingle%d.tmx", GM->singlePlayStageIndex));
    ...
    if (stage == STAGE_CUSTOM) {
      ...
      theMap = cocos2d::TMXTiledMap::create(
          StringUtils::format("colosseum%d.tmx", stage));
      log("sm map %d ", stage);
    } else {
      theMap = cocos2d::TMXTiledMap::create(
          StringUtils::format("stage%d.tmx", stage));
    }
  } else if (GM->nextScene == STAGE_INTRO) {
    theMap = cocos2d::TMXTiledMap::create(
        StringUtils::format("stage%dIntro.tmx", stage));
    isGameOver = true;
    if (stage == 0) {
      showNPCEvent(0);
    } else if (stage == 12) {
  ...
  if (GM->currentStageIndex != STAGE_LOBBY) {
    ...
    drawMiniMapFrame->drawSolidRect(miniMapDrawStartPos,
                                    miniMapDrawStartPos +
                                        Vec2(miniMapWidth, miniMapHeight),
                                    Color4F(0.4f, 0.4f, 0.4f, 1));
    miniMapViewRect =
...
void HelloWorld::addEnemiesToMap(TMXTiledMap *map, int levelScore,
                                 bool blueKey) {
  Vec2 pos;
  ...
  //    CallFunc::create(CC_CALLBACK_0(HelloWorld::loadEnemies, this)), NULL));
...
FireableBase *HelloWorld::addEnemyToLoadStack(TMXTiledMap *map, int levelScore,
    ...
    for (int j = 0; j < mapSize.height; j++) {
      ...
      if (fogAboveUnit->appliedState > FOG_SEEN_NOT &&
          (getTileGIDAt(decoLayer, Vec2(i, j)) == 49 ||
           isSoilBlock(getTileGIDAt(soilLayer, Vec2(i, j))))) {
...
float HelloWorld::getMin(float src, float dst) { return src < dst ? src : dst; }
float HelloWorld::getMax(float src, float dst) { return src > dst ? src : dst; }
void HelloWorld::setStage(TMXTiledMap *tileMap) {
  ...
  this->runAction(Sequence::create(
      ...
      CallFunc::create(CC_CALLBACK_0(HelloWorld::checkBirdFly, this)),
      nullptr));
...
UnitInfo *HelloWorld::getHeroAt(int index) { return nullptr; }
...
void HelloWorld::setOccupy(cocos2d::Vec2 pos, int width, int height,
...
void HelloWorld::addDecoBlock(Vec2 coordinate, Vec2 position,
...
void HelloWorld::addTalkBalloon(TMXTiledMap *map, std::string imgName,
                                cocos2d::Vec2 pos) {
  Sprite *spt = Sprite::createWithSpriteFrameName(imgName);
  map->addChild(spt, 10);
...
EnemyBase *HelloWorld::createUnit(int index, int whichSide, bool isBuilding,
...
int HelloWorld::getUnitMaxHP(int unit) { return GM->getUnitMaxHP(unit); }
void HelloWorld::stopWoongSound() { readyToMove = true; }
...
void HelloWorld::splashDamage(Vec2 pos, int radius, int damage,
...
void HelloWorld::doTheBombToMissiles(float dt) {
  ...
  if (bombTargetMissileArray.size() <= 0) {
    ...
    this->schedule(schedule_selector(HelloWorld::doTheBombToEnemies), 0.1,
...
void HelloWorld::releaseCustomMovingCamera() { isCameraInCustomMoving = false; }
// gravityupdate
...
cocos2d::Rect HelloWorld::tileRectFromTileCoords(Vec2 tileCoords,
                                                 TMXTiledMap *map) {
...
#if (CC_TARGET_PLATFORM == CC_PLATFORM_WIN32)
bool HelloWorld::usesPcRtsControls() { return true; }

bool HelloWorld::isShiftHeldForSelection() {
  if (!usesPcRtsControls()) {
    return false;
  }
...
bool HelloWorld::isCtrlHeldForSelection() {
  if (!usesPcRtsControls()) {
    return false;
  }
...
void HelloWorld::updateWin32EdgeScroll(const Vec2 &viewPos) {
...
Vec2 HelloWorld::getWin32MouseWorldPosFromView(const Vec2 &viewPos) {
  Size visibleSize = Director::getInstance()->getVisibleSize();
  return Director::getInstance()->convertToGL(
      Vec2(viewPos.x, visibleSize.height - viewPos.y));
}

Vec2 HelloWorld::getWin32MouseWorldPos(EventMouse *evt) {
  return getWin32MouseWorldPosFromView(evt->getLocationInView());
}

bool HelloWorld::isClickOnWorkerMenu(const Vec2 &glPos) {
  return getWorkerMenuButtonAtGlPos(glPos) != nullptr;
}

Button *HelloWorld::getWorkerMenuButtonAtGlPos(const Vec2 &glPos) {
  if (GM->currentStageIndex == STAGE_LOBBY) {
    return nullptr;
  }
...
void HelloWorld::handleWorkerMenuClick(const Vec2 &glPos) {
  if (HUD == nullptr) {
    return;
  }
...
void HelloWorld::setWin32AttackMoveMode(bool enabled) {
...
void HelloWorld::cancelWin32AttackMoveMode() { setWin32AttackMoveMode(false); }

void HelloWorld::handleWin32RightClick(Vec2 screenPos) {
  if (gameMode == GAME_MODE_PVP6 || gameMode == GAME_MODE_PVP12 ||
  ...
  if (GM->tileState[(int)coordinate.x][(int)coordinate.y] == 0) {
    if (isMultiplay) {
      std::string strIDs = "";
      for (auto unit : selectedArray) {
        strIDs += Value(unit->unitID).asString() + "|";
...
void HelloWorld::handleWin32AttackMoveClick(Vec2 screenPos) {
  if (selectedArray.size() == 0) {
    cancelWin32AttackMoveMode();
    return;
  }
...
#else
bool HelloWorld::isShiftHeldForSelection() { return false; }
bool HelloWorld::isCtrlHeldForSelection() { return false; }
#endif
    ...
    if (GM->nextScene == STAGE_LOBBY) {
      ...
      for (auto unit : heroArray) { // select hero
        if (unit->getBoundingBox().containsPoint(pos) &&
            unit == BHUD->selectedUnit &&
            unit->unitType != UNIT_TREE_FOR_BATTLE && unit->isBuilding) {
          ...
          this->createBuildingTemplate(unitType,
                                       getSpriteNameForUnit(unitType));
          buildingTemplate->setPosition(
              BHUD->selectedUnit->getPosition() -
...
//        if(touches.size() == 1 && buildingTemplate == nullptr){
  ...
  listener->onTouchesEnded = [&](const std::vector<Touch *> &touches,
    ...
    if (buildingTemplate != nullptr && GM->nextScene == STAGE_LOBBY &&
        isSelectedBuildingTouchStarted) {
      if ((!isBuildingTemplateMoved && isBuildingMovingFirstTry) ||
          isBuildingReadyToBuild) {
        Movable *unit = BHUD->selectedUnit;
        //                setOccupy(getPositionFromTileCoordinate(unit->buildingStartCoordinate.x,
        //                unit->buildingStartCoordinate.y),
        //                unit->buildingOccupySize.width,
        //                unit->buildingOccupySize.height, false);
        updateMiniMapForNonMoving();
        resetPathState();
        Vec2 newPos = buildingTemplate->getPosition() +
                      Vec2(buildingTemplateSize.width * TILE_SIZE / 2,
                           buildingTemplateSize.height * TILE_SIZE / 2);
        if (unit->unitType == UNIT_MINE) {
          std::string name = strmake("newminebutton%d_%d",
                                     (int)unit->getPositionX() / TILE_SIZE,
                                     (int)unit->getPositionY() / TILE_SIZE);
          ...
          std::string newKey =
              strmake("newminebutton%d_%d", (int)newPos.x / TILE_SIZE,
                      (int)newPos.y / TILE_SIZE);
          ...
          //                    strCollectStartTime = BSM->getStrFromTime(now -
          //                    60*60*24*50);
          ...
          std::string name =
              strmake("treeButton%d_%d", (int)unit->getPositionX() / TILE_SIZE,
                      (int)unit->getPositionY() / TILE_SIZE);
          std::string strPreviousTime = UDGetStr(name.c_str(), "");
          Node *ndMineButton = WORLD->getChildByName(name);
          ndMineButton->setPosition(newPos + Vec2(0, 150) / WORLD->getScale());
          std::string newKey =
              strmake("treeButton%d_%d", (int)newPos.x / TILE_SIZE,
                      (int)newPos.y / TILE_SIZE);
          ndMineButton->setName(newKey);
          time_t oldTime = GM->getSavedTime(name.c_str());
          GM->saveTime(newKey.c_str(), oldTime);
        ...
        setOccupy(buildingTemplate->getPosition() +
                      Vec2(0, buildingTemplateSize.height * TILE_SIZE),
                  unit->buildingOccupySize.width,
                  unit->buildingOccupySize.height, true, (EnemyBase *)unit);
        ...
        for (auto movable : heroArray) {
          if (!movable->isBuilding &&
              unit->getBoundingBox().containsPoint(movable->getPosition())) {
            movable->stop();
          }
        }
  ...
  mouseListener->onMouseMove = [this](EventMouse *evt) {
    ...
    if (win32LeftMouseDown && isTouchBeganFromMiniMap) {
      moveScreenToMiniMapGlPos(pos);
      isMapMovingByMiniMap = true;
      return;
    }
  ...
  mouseListener->onMouseUp = [this](EventMouse *evt) {
    ...
    if ((int)evt->getMouseButton() == 0) {
      ...
      if (HUD != nullptr && HUD->tryHandleHudClick(pos)) {
        win32MouseOnWorkerMenu = false;
        return;
      }
...
void HelloWorld::setWorkerToBuild(EnemyBase *worker, int buildingIndex, int x,
                                  int y) {
  ...
  Vec2 pos = this->getPositionFromTileCoordinate(worker->builderCoordinate.x,
...
float HelloWorld::getCommandMarkerScale() const {
  return commandMarkerBaseScale;
}

float HelloWorld::getWorkerMenuScale() const {
  return workerMenuBaseScale * commandMarkerBaseScale;
}

float HelloWorld::getWorkerMenuRadius() const {
  return workerMenuBaseRadius * commandMarkerBaseScale;
}

float HelloWorld::getWorkerMenuCenterOffsetY() const {
  return workerMenuCenterOffsetY * commandMarkerBaseScale;
}

void HelloWorld::setLayerScale(float newScale, const Vec2 &anchorGlPos) {
...
void HelloWorld::moveScreenToMiniMapGlPos(const Vec2 &glPos) {
  if (gameMode == GAME_MODE_PVP6 || gameMode == GAME_MODE_PVP12) {
    return;
  }
          ...
          if (doubleClickTimerForAttackDdaing > 0) {
            ...
            if (tileState == 0) {
              if (BHUD) {
                ...
                moveTo(selectedArray, getPositionFromTileCoordinate(
                ...
                if (isMultiplay) { // test
                  ...
                  this->scheduleOnce(
                      schedule_selector(HelloWorld::checkAttackDdangSent),
...
bool HelloWorld::gatherResourceWithSelectedWorkers(EnemyBase *mutual) {
  if (mutual == nullptr) {
    return false;
  }
...
EnemyBase *HelloWorld::getHeroBuildingAtMapPos(Vec2 mapPos) {
...
bool HelloWorld::tryDeliverCarriedResourcesAt(Vec2 mapPos) {
...
bool HelloWorld::tryClickBuildProgressIcon(const Vec2 &glPos) {
  if (GM->currentStageIndex == STAGE_LOBBY) {
    return false;
  }
...
void HelloWorld::updateBuildingTemplateAtScreenPos(Vec2 glPos,
...
void HelloWorld::confirmBuildingPlacement() {
  if (buildingTemplate == nullptr || GM->currentStageIndex == STAGE_LOBBY) {
    return;
  }
  ...
  for (int j = 0; j < buildingTemplateSize.height; j++) {
    for (int i = 0; i < buildingTemplateSize.width; i++) {
      ...
      if (spt != nullptr) {
        ...
        bool isBlock = isDecoBlock(getTileGIDAt(decoLayer, pos)) ||
...
EnemyBase *HelloWorld::buildTheBuilding(int index, int x, int y, int width,
...
void HelloWorld::selectUnit(EnemyBase *unit) {
  ...
  if (GM->nextScene == STAGE_LOBBY) {
    ...
    const float arrowScale = 0.3f * imageScale * getCommandMarkerScale();
    for (int i = 0; i < 4; i++) {
      Sprite *arrow = (Sprite *)moveArrows->getChildByTag(i);
      if (arrow != nullptr) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (i == 0) {
        spt->setPosition(Vec2(circle->getContentSize().width *
                                  circle->getScaleX() * 1.3f / 2,
                              0));
      } else if (i == 1) {
        spt->setPosition(Vec2(0, -circle->getContentSize().height *
                                     circle->getScaleY() * 1.3f / 2));
      } else if (i == 2) {
        spt->setPosition(Vec2(-circle->getContentSize().width *
                                  circle->getScaleX() * 1.3f / 2,
                              0));
...
// void HelloWorld::TouchesCancelled(const std::vector<Touch*>& touches, Event
// *unused_event)
//{
...
cocos2d::Rect HelloWorld::intersection(cocos2d::Rect source,
                                       cocos2d::Rect rect) {
...
void HelloWorld::onUpdateResultErrorComplete(char const *status,
                                             char const *error) {
...
void HelloWorld::coinWaitDone(Ref *obj) {
  ...
  drop->runAction(
      Sequence::create(FadeOut::create(1),
                       CallFuncN::create(CC_CALLBACK_1(
                           HelloWorld::removeFromParentAndCleanup, drop)),
                       NULL));
}
...
void HelloWorld::blinkingPlayer(float dt) { blinkingTime -= dt; }
...
void HelloWorld::showTalkText(std::string text, int whoseTalk,
                              std::string npcName) {
  std::string str = "" + text;
  talkText = str;
  talkIndex = 0;
      ...
      for (auto unit : npcArray) {
        if (unit->getName().compare("undead") == 0) {
          ...
          unit->runAction(Sequence::create(
              ...
              CallFunc::create(
                  CC_CALLBACK_0(HelloWorld::onUndeadMoveDone, this)),
              nullptr));
      ...
      for (auto unit : unitsToCreateArray) {
        if (unit->alliSide == WHICH_SIDE_HERO) {
          ...
          unit->runAction(Sequence::create(
              ...
              CallFunc::create(CC_CALLBACK_0(HelloWorld::onNPCMoveDone, this)),
              nullptr));
        }
      }
      for (auto unit : npcArray) {
        if (unit->getName().compare("orc") != 0) {
          if (unit->getName().compare("undead") == 0) {
            ...
            //                        CallFunc::create(CC_CALLBACK_0(HelloWorld::onNPCMoveDone,
            //                        this)), nullptr));
          }
        }
      }
...
std::string HelloWorld::getItemStat(int itemType, std::string name,
                                    std::string stat) {
  ValueMap valueMap;
...
void HelloWorld::getSupportFromVideo(int video) { finishedVideo = video; }
