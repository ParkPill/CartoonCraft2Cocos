...
void Movable::init(int unit, int eng, float extraSpd, const char *sptName) {
  ...
  if (WORLD) {
    setAnchorPoint(
        Vec2(0.5, (14 / getBoundingBox().size.height) * WORLD->imageScale));
  } else {
...
void Movable::showTalk(std::string text, float time, float y, float delay) {
  ...
  imgTalkBalloon->runAction(
      Sequence::create(DelayTime::create(delay), FadeIn::create(0), NULL));

  lblTalk = Label::createWithTTF(
      text, GameManager::getInstance()->getFont(FONT_VISITOR), 36);
  ...
  float gap = 3;
  lblTalk->setPosition(
      Vec2(lblTalk->getBoundingBox().size.width / 2 + gap,
           imgTalkBalloon->getBoundingBox().size.height / 2 + 1.5));
  ...
  imgTalkBalloon->setContentSize(
      cocos2d::Size(width, imgTalkBalloon->getContentSize().height));
  ...
  this->schedule(schedule_selector(Movable::updateTalkBalloon), 0.07f,
                 CC_REPEAT_FOREVER, delay);
}
...
void Movable::dontHurt() { isHurting = false; }
float Movable::getCurrentY() { return getPositionY(); }
Movable *Movable::createMovable(int unit, int eng, float extraSpd,
                                const char *sptName) {
...
Movable *Movable::createMovable(int unit, int eng, float extraSpd,
  ...
  return Movable::createMovable(unit, eng, extraSpd, sptName, animationName,
                                false);
}

Movable *Movable::createMovable(int unit, int eng, float extraSpd,
                                const char *sptName, const char *animationName,
                                bool repeat) {
...
void Movable::updatePosition(float dt) {
  ...
  if (this->freezed) {
    vlct =
        Vec2(inWater ? vlct.x * 0.9 : vlct.x * (freezed ? 0.95 : 0.30), vlct.y);
  } else {
    //        if (wantsToJump){
    //            if(inWater || (!jumped && (onGround || jumpAllowed))) {
    //                Vec2 jumpForce = Vec2(0.0,
    //                inWater?(onGround?waterAccelYP+10:waterAccelYP):1850.0);
    //                vlct = vlct + jumpForce;
    //                jumped = true;
    //                onGround = false;
    //                wantsToJump = false;
    //                onJumped();
    //
    //            }else if (doubleJump && !tumbled && (jumped || !onGround) &&
    //            !iWasInWater) {
    //                Vec2 jumpForce = Vec2(0.0,
    //                inWater?(onGround?waterAccelYP+60:waterAccelYP):1850.0);
  ...
  //    Vec2 minMovement = Vec2(inWater?-waterMaxX:-50.0-extraSpeed,
  //    inWater?-waterMinY:-280.0 - extraJump); Vec2 maxMovement =
  //    Vec2(inWater?waterMaxX:50.0+extraSpeed, 280.0 + extraJump);
  Vec2 minMovement = Vec2(inWater ? -waterMaxX : -280.0,
                          inWater ? -waterMinY : -280.0 - extraJump);
...
void Movable::freezeForAWhile(float dur) {
  ...
  this->runAction(Sequence::create(
      ...
      CallFunc::create(CC_CALLBACK_0(Movable::releaseFreezing, this)),
      nullptr));
}
void Movable::releaseFreezing() {
  freezed = false;
  if (fixAngleToTargetWhenReleaseFreezing) {
    movingAngle = -(-GameManager::getInstance()->getAngle(
                        this->getPosition(),
                        this->target->getPosition() + Vec2(0, TILE_SIZE / 2)) +
                    180);
  }
}
void Movable::onDropping() { isDropping = true; }
void Movable::banJump(float dt) { jumpAllowed = false; }
...
void Movable::updatePositionToFlyToFollowTarget(float dt) {
  ...
  float diff = sqrtf((tPos.x - pos.x) * (tPos.x - pos.x) +
                     (tPos.y - pos.y) * (tPos.y - pos.y));
...
cocos2d::Rect Movable::collectBoundingBox() {
  return cocos2d::Rect(desiredPosition.x - TILE_SIZE / 2,
                       desiredPosition.y - getContentSize().height / 2,
...
cocos2d::Rect Movable::collisionBoundingBox() {
  ...
  //    return cocos2d::Rect(desiredPosition.x - TILE_SIZE/2, desiredPosition.y
  //    - getContentSize().height/2, width, height);//TILE_SIZE, TILE_SIZE);
  return cocos2d::Rect(desiredPosition.x - width / 2,
                       desiredPosition.y - height / 2, width, TILE_SIZE / 2);
  cocos2d::Rect bounding = boundingBox();

  if (bounding.size.width < TILE_SIZE * 2 &&
      bounding.size.height < TILE_SIZE * 2) {
    ...
    Vec2 pos = getPosition();
    cocos2d::Rect rect = cocos2d::Rect(pos.x - TILE_SIZE / 2,
                                       pos.y - getContentSize().height / 2,
                                       TILE_SIZE, getContentSize().height);
...
cocos2d::Rect Movable::damageBoundingBox() {
  ...
  //    cocos2d::Rect returnBoundingBox = cocos2d::RectOffset(collisionBox,
  //    diff.x, diff.y); return returnBoundingBox;
  return collisionBox;
}

Vec2 Movable::getGravityPosition() {
  return Vec2(getPosition().x,
              getPosition().y - getContentSize().height / 2 + TILE_SIZE / 2);
}
...
/*cocos2d::Rect Movable::currentBoundingBox()
{
    ...
    if (vehicleType == VEHICLE_GOLIATH) {
        ...
        Vec2 pos = getPosition();
        cocos2d::Rect collisionBox = cocos2d::Rect(pos.x - TILE_SIZE, pos.y -
28, TILE_SIZE*2, TILE_SIZE*2); return collisionBox;
    }
...
//        cocos2d::Rect returnBoundingBox = cocos2d::RectOffset(collisionBox,
diff.x, diff.y); return collisionBox; }else{ Vec2 pos = getPosition();
        cocos2d::Rect rect = cocos2d::Rect(pos.x - TILE_SIZE/2, pos.y -
TILE_SIZE + 1, TILE_SIZE, TILE_SIZE*2 - 2); cocos2d::Rect collisionBox =
cocos2d::RectInset(rect, 0, 0);
//        Vec2 diff = ccpSub(desiredPosition, getPosition());
//        cocos2d::Rect returnBoundingBox = cocos2d::RectOffset(collisionBox,
diff.x, diff.y); return collisionBox;
    }
}*/

cocos2d::Rect Movable::RectOffset(cocos2d::Rect rect, float x, float y) {
  return cocos2d::Rect(rect.origin.x + x, rect.origin.y + y, rect.size.width,
                       rect.size.height);
}
    ...
    if (currentAction != orderAction) {
        if (orderAction == ENEMY_ACTION_MOVE_TO_LEFT) {
            ...
            if(unitType == UNIT_SNAIL_BALL){
                this->stopAllActions();
                this->runAction(RepeatForever::create(RotateBy::create(1,
isTowardLeft?-360:360)));
            }
...
void Movable::setTarget(Movable *t) {
  if (t == nullptr)
    return;
  target = t;
  targetMoveTilePos = WORLD->getCoordinateFromPosition(
      t->getApproachingPoint(getPosition()), WORLD->theMap);
}
float Movable::findTarget(Movable *tar, float dist) {
  Vec2 pos = this->getPosition();
  Vec2 tPos = tar->getPosition();
  float distance = sqrtf((pos.x - tPos.x) * (pos.x - tPos.x) +
...
void Movable::stop(bool search) {
  ...
  //        Movable* unit = WORLD->buildTheBuilding(builderBuildingIndex,
  //        builderCoordinate.x, builderCoordinate.y, builderSize.width,
  //        builderSize.height, builderSpriteName, alliSide); if(unit ==
  //        nullptr){ // condition not met
  //            return;
  //        }
  //        // progress
  //        Sprite* spt = Sprite::create("uiBox.png");
  //        spt->setPosition(unit->getPosition() + Vec2(50,
  //        unit->getContentSize().height/2 + 10)); WORLD->addChild(spt, 1000);
  //        auto timer =
  //        ProgressTimer::create(WORLD->getSpriteForUnit(unit->unitType));
  //        processTimer = timer;
  ...
  //        timer->runAction(Sequence::create(ProgressTo::create(WORLD->getUnitCreateTime(unit->unitType),
  //        100), CallFunc::create(CC_CALLBACK_0(Movable::resetProcessTimer,
  //        this)), CallFunc::create(CC_CALLBACK_0(Sprite::removeFromParent,
  //        spt)), nullptr));
  //        // progress end
  //
  //        if(WORLD->selectedArray.find((EnemyBase*)this) !=
  //        WORLD->selectedArray.end()){
  //            WORLD->deselect(this);
  //        }
  //        builderBuilding = unit;
  //        unit->setRotation(0);
  //        unit->buildingCompleteTime =
  //        WORLD->getUnitCreateTime(builderBuildingIndex);
  //        unit->setSpriteFrame("underConstrunction0.png");
  //        GM->runAnimation(unit, "underConstrunction", true);
  //        this->runAction(Sequence::create(CallFunc::create(CC_CALLBACK_0(Movable::playTreeSound,
  //        this)),
  //                                         DelayTime::create(0.2f),
  //                                         CallFunc::create(CC_CALLBACK_0(Movable::playTreeSound,
  //                                         this)), DelayTime::create(0.2f),
  //                                         CallFunc::create(CC_CALLBACK_0(Movable::playTreeSound,
  //                                         this)), DelayTime::create(0.2f),
  //                                         CallFunc::create(CC_CALLBACK_0(Movable::playTreeSound,
  //                                         this)), DelayTime::create(0.2f),
  ...
  //                                         this)), DelayTime::create(0.2f),
  //                                         NULL));
  //        this->runAction(Sequence::create(DelayTime::create(unit->buildingCompleteTime),
  //        CallFunc::create(CC_CALLBACK_0(Movable::onBuildComplete, this)),
  //        NULL)); this->setVisible(false); // test this->attackType =
  //        ATTACK_TYPE_NONE; this->untouchable = true;
  //    }
  ...
  //    }else if (!isGatheringGold && !WORLD->isThisSpotAvailable(this) &&
  //    canMove && !isFlying) {
  //        isTemporaryFlying = true;
  //        Vec2 coordinate = WORLD->getCoordinateFromPosition(getPosition());
  //        coordinate = Vec2(coordinate.x + rand()%3 - 1, coordinate.y +
  //        rand()%3 - 1); if(coordinate.x + 1 >= WORLD->mapSize.width){
  ...
  //            coordinate.y = 0;
  //        }
  //        Vec2 newPos = WORLD->getPositionFromTileCoordinate(coordinate.x,
  //        coordinate.y);
...
void Movable::resetProcessTimer() { processTimer = nullptr; }
...
void Movable::onBuildComplete() { // building(float dt){
  ...
  // An OilShip that just finished its own OilExtractor immediately starts
  // the gather loop (enter extractor -> haul to depot), like a worker sent
  // back to a gold mine. Skip the footprint-relocation below: gatherOil()
  // walks the ship onto the extractor anyway.
  if ((unitType == UNIT_HUMAN_OIL_SHIP || unitType == UNIT_ORC_OIL_SHIP) &&
      (builderBuilding->unitType == UNIT_HUMAN_OIL_EXTRACTOR ||
       builderBuilding->unitType == UNIT_ORC_OIL_EXTRACTOR)) {
    WORLD->setOilShipToExtractor((EnemyBase *)this,
                                 (EnemyBase *)builderBuilding);
    return;
  }
...
void Movable::runAnimation(int aniType) {
  ...
  std::string name =
      this->unitName; // this->getName();//getCharacterName(unitType);
  if (spine) {
...
std::string Movable::getCharacterName(int unitType) { return ""; }
void Movable::attackUpdate(float dt) {
  //    if(isBuilding) return; // test
  if (!target || target->energy <= 0 || target->untouchable ||
      (target->alliSide == WHICH_SIDE_MUTUAL && unitType != UNIT_WIZARD)) {
    target = nullptr;
    //        stopNew();
    return;
  }
  ...
  if (isSkillOn) {
    if (unitType == UNIT_HERO_ORC) {
      ...
      Sequence *seq = Sequence::create(
          DelayTime::create(25.0f / 30),
          CallFunc::create(CC_CALLBACK_0(Movable::attack, this)),
          DelayTime::create(4.0f / 30),
          CallFunc::create(CC_CALLBACK_0(Movable::attack, this)),
          DelayTime::create(4.0f / 30),
          CallFunc::create(CC_CALLBACK_0(Movable::attack, this)), nullptr);
      ...
      happenTime = 19.0f / 30;
      Sequence *seq = Sequence::create(
          DelayTime::create(30.0f / 30),
          CallFunc::create(CC_CALLBACK_0(Movable::attack, this)),
          DelayTime::create(11.0f / 30),
          CallFunc::create(CC_CALLBACK_0(Movable::attack, this)),
          DelayTime::create(0),
          CallFunc::create(CC_CALLBACK_0(Movable::attack, this)), nullptr);
      seq->setTag(attackTag);
...
void Movable::startSkillAuraAndSkillEffect(int aura) {
  ...
  if (unitType == UNIT_HERO_ORC) {
    this->runAction(Sequence::create(
        DelayTime::create(39.0f / 30 - 0.06f * 6),
        CallFunc::create(CC_CALLBACK_0(Movable::showSkillEffect, this)),
        nullptr));
    ...
    this->runAction(Sequence::create(
        DelayTime::create(33.0f / 30 - 0.1f),
        CallFunc::create(CC_CALLBACK_0(Movable::showSkillEffect, this)),
        nullptr));
    skillEffectIndex = EFFECT_BLUE_TEETH;
  } else if (unitType == UNIT_HERO_SPEARMAN) {
    this->runAction(Sequence::create(
        DelayTime::create(20.0f / 30),
        CallFunc::create(CC_CALLBACK_0(Movable::showSkillEffect, this)),
        DelayTime::create(4.0f / 30),
        CallFunc::create(CC_CALLBACK_0(Movable::showSkillEffect, this)),
        DelayTime::create(4.0f / 30),
    ...
    this->runAction(Sequence::create(
        DelayTime::create(48.0f / 30),
        CallFunc::create(CC_CALLBACK_0(Movable::showSkillEffect, this)),
        DelayTime::create(2.0f / 30),
        CallFunc::create(CC_CALLBACK_0(Movable::showSkillEffect, this)),
        DelayTime::create(2.0f / 30),
        CallFunc::create(CC_CALLBACK_0(Movable::showSkillEffect, this)),
        DelayTime::create(2.0f / 30),
    ...
    if (skillTarget) {
      for (int i = 0; i < 4; i++) {
        float x = rand() % 100 - 50;
        WORLD->createMissile("heroArrow.png", "arrowBuried.png",
                             skillTarget->getPosition() + Vec2(x, 200),
                             skillTarget->getPosition() + Vec2(x, 0), 0.1f, ap,
                             isEnemy, -90, this, 2.8 + i * 0.06f);
      }
    }
  } else if (unitType == UNIT_HERO_MONK) {
    this->runAction(Sequence::create(
        DelayTime::create(27.0f / 30 - 0.1f),
        CallFunc::create(CC_CALLBACK_0(Movable::showSkillEffect, this)),
        nullptr));
    skillEffectIndex = EFFECT_HIT_WITH_CIRCLE;
  } else if (unitType == UNIT_HERO_FIGHTER || unitType == UNIT_HERO_MOLE) {
    this->runAction(Sequence::create(
        DelayTime::create(29.0f / 30),
        CallFunc::create(CC_CALLBACK_0(Movable::showSkillEffect, this)),
        nullptr));
    skillEffectIndex = EFFECT_HIT_WITH_CIRCLE;
  } else if (unitType == UNIT_HERO_BEAR) {
    this->runAction(Sequence::create(
        DelayTime::create(32.0f / 30),
        CallFunc::create(CC_CALLBACK_0(Movable::showSkillEffect, this)),
        nullptr));
    skillEffectIndex = EFFECT_RED_TEETH;
  } else if (unitType == UNIT_HERO_HEALER) {
    this->runAction(Sequence::create(
        DelayTime::create(35.0f / 30),
        CallFunc::create(CC_CALLBACK_0(Movable::showSkillEffect, this)),
        nullptr));
    skillEffectIndex = EFFECT_HEAL;
  } else if (unitType == UNIT_HERO_KNIGHT) {
    this->runAction(Sequence::create(
        DelayTime::create(19.0f / 30),
        CallFunc::create(CC_CALLBACK_0(Movable::showSkillEffect, this)),
        DelayTime::create(4.0f / 30),
        CallFunc::create(CC_CALLBACK_0(Movable::showSkillEffect, this)),
        DelayTime::create(4.0f / 30),
    ...
    this->runAction(Sequence::create(
        ...
        nullptr));
    skillEffectIndex = EFFECT_HIT_WITH_CIRCLE;
  } else if (unitType == UNIT_HERO_TANKER || unitType == UNIT_HERO_SANTADOG ||
    ...
    this->runAction(Sequence::create(
        DelayTime::create(34.0f / 30 - 0.1f),
        CallFunc::create(CC_CALLBACK_0(Movable::showSkillEffect, this)),
        nullptr));
    skillEffectIndex = EFFECT_FIREBOMB;
  } else if (unitType == UNIT_HERO_LADY_LION) {
    this->runAction(Sequence::create(
        DelayTime::create(20.0f / 30 - 0.1f),
        CallFunc::create(CC_CALLBACK_0(Movable::showSkillEffect, this)),
        nullptr));
    skillEffectIndex = EFFECT_FIREBALL_EXPLOSION;
  } else if (unitType == UNIT_HERO_TOYMOUSE) {
    this->runAction(Sequence::create(
        DelayTime::create(57.0f / 30 - 0.1f),
        CallFunc::create(CC_CALLBACK_0(Movable::showSkillEffect, this)),
        nullptr));
    skillEffectIndex = EFFECT_FIREBALL_EXPLOSION;
  } else if (unitType == UNIT_HERO_BATMONSTER) {
    this->runAction(Sequence::create(
        DelayTime::create(45.0f / 30 - 0.1f),
        CallFunc::create(CC_CALLBACK_0(Movable::showSkillEffect, this)),
        nullptr));
    skillEffectIndex = EFFECT_FIREBALL_EXPLOSION;
  } else if (unitType == UNIT_HERO_SAVAGEARCHER) {
    ...
    this->runAction(Sequence::create(
        DelayTime::create(22.0f / 30),
        CallFunc::create(CC_CALLBACK_0(Movable::showSkillEffect, this)),
        nullptr));
    skillEffectIndex = EFFECT_BLUE_SLASH;
  } else if (unitType == UNIT_HERO_MINO) {
    this->runAction(Sequence::create(
        DelayTime::create(39.0f / 30),
        CallFunc::create(CC_CALLBACK_0(Movable::showSkillEffect, this)),
        nullptr));
    skillEffectIndex = EFFECT_FIREBALL_EXPLOSION;
  } else if (unitType == UNIT_HERO_KERBEROS) {
    this->runAction(Sequence::create(
        DelayTime::create(24.0f / 30),
        CallFunc::create(CC_CALLBACK_0(Movable::showSkillEffect, this)),
        nullptr));
    skillEffectIndex = EFFECT_FIREBOMB;
  } else if (unitType == UNIT_HERO_LAMIA) {
    ...
    this->runAction(Sequence::create(
        DelayTime::create(30.0f / 30 - 0.1f),
        CallFunc::create(CC_CALLBACK_0(Movable::showSkillEffect, this)),
        nullptr));
    skillEffectIndex = EFFECT_HIT_WITH_CIRCLE;
  } else if (unitType == UNIT_HERO_GOLEM) {
    this->runAction(Sequence::create(
        DelayTime::create(46.0f / 30 - 0.1f),
        CallFunc::create(CC_CALLBACK_0(Movable::showSkillEffect, this)),
        nullptr));
    skillEffectIndex = EFFECT_FIREBALL_EXPLOSION;
  }
  ...
  if (selectedSkillAura == AURA_LIGHTNING) {
    ...
    if (spine) {
      spt->setPosition(spine->getPosition() +
                       Vec2(diff - rand() % (diff * 2),
                            spine->getBoundingBox().size.height / 2 + diff -
                                rand() % (diff * 2)));
    } else {
      spt->setPosition(
          this->getPosition() +
          Vec2(getContentSize().width / 2 - diff + rand() % (diff * 2),
               getContentSize().height / 2 - diff + rand() % (diff * 2)));
    }
    ...
    for (int i = 0; i < 2; i++) {
      ...
      spt->runAction(
          Sequence::create(TintTo::create(0.02, Color3B::BLACK),
                           TintTo::create(0.02, Color3B::WHITE),
                           TintTo::create(0.02, Color3B(255, 233, 0)),
                           TintTo::create(0.08, Color3B::RED),
                           TintTo::create(0.08, Color3B::GRAY),
                           TintTo::create(0.08, Color3B::BLACK), NULL));
      spt->runAction(Sequence::create(
          DelayTime::create(0.18f), FadeOut::create(0.14f),
      ...
      spt->runAction(Sequence::create(
          DelayTime::create(0.2f),
          ScaleTo::create(0.1f, spt->getScale() * 1.4f), NULL));
      WORLD->batch->addChild(spt, 2);
      if (spine) {
        spt->setPosition(spine->getPosition() +
                         Vec2(diff - rand() % (diff * 2), rand() % 30));
      } else {
        spt->setPosition(
            this->getPosition() +
            Vec2(getContentSize().width / 2 - diff + rand() % (diff * 2),
    ...
    for (int i = 0; i < 2; i++) {
      ...
      spt->runAction(
          Sequence::create(TintTo::create(3, Color3B(12, 175, 239)), NULL));
      spt->runAction(Sequence::create(
          DelayTime::create(2), FadeOut::create(1),
          CallFunc::create(CC_CALLBACK_0(Sprite::removeFromParent, spt)),
          NULL));
      float x = rand() % 100 - 50;
      spt->setOpacity(150);
      ...
      spt->runAction(
          Sequence::create(DelayTime::create(2),
                           ScaleTo::create(1, spt->getScale() * 1.4f), NULL));
      WORLD->batch->addChild(spt, 2);
      if (spine) {
        spt->setPosition(spine->getPosition() +
                         Vec2(diff - rand() % (diff * 2), rand() % 10));
      } else {
        spt->setPosition(
            this->getPosition() +
            Vec2(getContentSize().width / 2 - diff + rand() % (diff * 2),
      ...
      spt = Sprite::createWithSpriteFrameName(
          strmake("iceCrystalPart%d.png", rand() % 2));
      WORLD->batch->addChild(spt);
      spt->setPosition(this->getPosition() + getPosition() +
                       Vec2(diff - rand() % (diff * 2), rand() % 10));
      spt->runAction(Sequence::create(
          EaseOut::create(
              MoveBy::create(0.3f, Vec2(rand() % 60 - 30, rand() % 60 - 30)),
              2),
          CallFunc::create(CC_CALLBACK_0(Sprite::removeFromParent, spt)),
          NULL));
      spt->runAction(Sequence::create(DelayTime::create(0.2f),
    ...
    for (int i = 0; i < 3; i++) {
      ...
      spt->setBlendFunc(BlendFunc::ADDITIVE);
      spt->runAction(
          Sequence::create(TintTo::create(0.3, Color3B(121, 255, 137)), NULL));
      spt->runAction(Sequence::create(
          DelayTime::create(0.2f), FadeOut::create(0.1f),
          CallFunc::create(CC_CALLBACK_0(Sprite::removeFromParent, spt)),
          NULL));
    ...
    WORLD->batch->addChild(spt);
    spt->runAction(Sequence::create(
        FadeIn::create(0.18f), FadeOut::create(0.14f), SPT_REMOVE_FUNC, NULL));
    float widthScale =
        getContentSize().width * 1.3f / spt->getContentSize().width;
...
void Movable::showSkillEffect() {
  ...
  if (skillEffectIndex == EFFECT_LIGHTNING) {
    ...
    WORLD->runEffect(skillEffectIndex,
                     this->getPosition() +
                         Vec2(isFlippedX() ? 100 : -100, 100) *
                             WORLD->imageScale,
                     180);
    ...
    for (int i = 0; i < 6; i++) {
      ...
      if (i == 0) {
        ...
        extraPos = Vec2(cos(theta) * 100 * WORLD->imageScale,
                        sin(theta) * 50 / WORLD->getScaleY());
      }
      for (int j = 0; j < 2; j++) {
        spt = Sprite::createWithSpriteFrameName(
            strmake("iceCrystalPart%d.png", rand() % 2));
        ...
        spt->runAction(Sequence::create(
            EaseOut::create(MoveBy::create(0.3f, Vec2(rand() % 100 - 50,
                                                      rand() % 100 - 50) *
                                                     WORLD->imageScale),
                            2),
      ...
      spt->runAction(Sequence::create(
          EaseOut::create(
              MoveBy::create(0.4f + (rand() % 10) * 0.01f, Vec2(0, 150)), 2),
          FadeOut::create(0.3f), SPT_REMOVE_FUNC, NULL));
      WORLD->batch->addChild(spt);
      spt->setRotation(-43);
      spt->setPosition(skillTarget->getPosition() + extraPos);
    }
    ...
    WORLD->runEffect(
        skillEffectIndex,
        skillTarget->getPosition() + Vec2(0, 50) * WORLD->imageScale, 180);
  } else if (skillEffectIndex == EFFECT_BLUE_SLASH) {
    if (unitType == UNIT_HERO_ARCHER) {
      WORLD->runEffect(skillEffectIndex,
                       getPosition() +
                           Vec2(rand() % 100 - 50, 100) * WORLD->imageScale,
                       -90 + rand() % 10 - 5);
    } else {
      WORLD->runEffect(skillEffectIndex,
                       skillTarget->getPosition() +
                           Vec2(0, rand() % 100 - 50) * WORLD->imageScale,
                       (isFlippedX() ? 0 : 180) + rand() % 30 - 15);
    }
  } else if (skillEffectIndex == EFFECT_EXPLODE_MIDDLE && skillTarget) {
    WORLD->runEffect(
        skillEffectIndex,
        skillTarget->getPosition() + Vec2(0, 30) * WORLD->imageScale, 180);
  } else if (skillEffectIndex == EFFECT_FIRE_SLASH && skillTarget) {
    WORLD->runEffect(skillEffectIndex,
                     skillTarget->getPosition() +
                         Vec2(0, 30) * WORLD->imageScale,
                     30 + rand() % 30);
...
void Movable::attack() {
  ...
  if (!this || !target || target == nullptr || target->untouchable ||
      target->energy <= 0 || target->isDead) {
    target = nullptr;
  ...
  // skip attack
  if (isSkillOn &&
      (unitType == UNIT_HERO_ARCHER || unitType == UNIT_HERO_HEALER)) {
    stopSkillAura();
    isSkillOn = false;
  ...
  if (isSkillOn) {
    ...
    if (skillTarget) {
      if (unitType == UNIT_HERO_ORC || unitType == UNIT_HERO_GOBLIN ||
          unitType == UNIT_HERO_LIZARDMAN || unitType == UNIT_HERO_RUDOLPH ||
          unitType == UNIT_HERO_SANTADOG || unitType == UNIT_HERO_ENT ||
          unitType == UNIT_HERO_GOLEM) {
        WORLD->splashDamage(skillTarget->getPosition(), 10500, ap * 1.5f,
                            isEnemy, this);
      } else if (unitType == UNIT_HERO_SALAMANDER ||
                 unitType == UNIT_HERO_TANKER || unitType == UNIT_HERO_MOLE ||
                 unitType == UNIT_HERO_PENGUIN) {
        WORLD->splashDamage(skillTarget->getPosition(), 10500, ap * 1.5f,
                            isEnemy, this);
      } else if (unitType == UNIT_HERO_LADY_LION ||
        ...
        WORLD->splashDamage(skillTarget->getPosition(), 14500, ap * 2.0f,
                            isEnemy, this);
      } else if (unitType == UNIT_HERO_WEREWOLF || unitType == UNIT_HERO_BEAR ||
                 unitType == UNIT_HERO_MONK ||
                 unitType == UNIT_HERO_ELF_SWORDMAN) {
        WORLD->splashDamage(skillTarget->getPosition(), 100, ap * 3, isEnemy,
                            this);
      } else if (unitType == UNIT_HERO_ASSASSIN ||
  ...
  if (unitType == UNIT_HERO_REAPER) {
    if (skillTarget) {
      WORLD->runEffect(skillEffectIndex,
                       skillTarget->getPosition() + Vec2(0, 30), 180);
    } else if (target) {
      WORLD->runEffect(skillEffectIndex, target->getPosition() + Vec2(0, 30),
                       180);
    }
  }
  if (attackType == ATTACK_TYPE_NEAR) {
    ...
    if (unitType == UNIT_ARCHER || unitType == UNIT_WATCHERTOWER ||
      ...
      missile =
          Movable::createMovable(UNIT_MISSILE_STRAIGHT, ap, 0, "arrow.png");
      if (unitType == UNIT_WATCHERTOWER) {
        speed = 1500;
      }
    } else if (unitType == UNIT_ORC_BUNKER || unitType == UNIT_ORC_HQ ||
               unitType == UNIT_ORC_SPEAR) {
      missile =
          Movable::createMovable(UNIT_MISSILE_STRAIGHT, ap, 0, "spear.png");
      speed = 1500;
      ...
      missile =
          Movable::createMovable(UNIT_MISSILE_STRAIGHT, ap, 0, "boulder.png");
      missile->runAction(RotateBy::create(2, 720));
      missile->runAction(ScaleTo::create(0.5f, 1.5f));
    } else if (unitType == UNIT_WIZARD) {
      missile =
          Movable::createMovable(UNIT_MISSILE_STRAIGHT, ap, 0, "fireBall0.png");
      speed = 300;
    } else if (unitType == UNIT_HERO_WIZARD) {
      if (isSkillOn) {
        apValue *= 2;
      }
      missile = Movable::createMovable(UNIT_MISSILE_STRAIGHT, apValue, 0,
                                       "fireBall0.png");
      if (isSkillOn) {
        missile->setScale(2);
      }
      ...
      //            missile = Movable::createMovable(UNIT_MISSILE_STRAIGHT, ap,
      //            0, "yellowBall.png"); speed = 300;
      healNearFriend();
      return;
    } else if (unitType == UNIT_HERO_REAPER) {
      if (target->getHitAndIsDead(ap, this)) {
    ...
    if (isSkillOn) {
      if (unitType == UNIT_HERO_LADY_WEREWOLF ||
          unitType == UNIT_HERO_LADY_BEAR || unitType == UNIT_HERO_PARASITE ||
          unitType == UNIT_HERO_BABYMINO || unitType == UNIT_HERO_GOBLIN ||
    ...
    if (unitType == UNIT_ARCHER || unitType == UNIT_ORC_SPEAR ||
      ...
      missile->runAction(Sequence::create(
          EaseOut::create(
              MoveBy::create(dur / 2,
                             Vec2(height * (isTowardLeft ? -1 : 1), 0)),
              2),
          EaseIn::create(
              MoveBy::create(dur / 2,
                             Vec2(-height * (isTowardLeft ? -1 : 1), 0)),
    ...
    if (unitType == UNIT_WATCHERTOWER || unitType == UNIT_ORC_BUNKER ||
        unitType == UNIT_ORC_HQ) { // straight
      if (unitType == UNIT_WATCHERTOWER || unitType == UNIT_HELICOPTER) {
        extraPos = Vec2(0, 100);
      }
      missile->setRotation(-GM->getAngle(getPosition(), target->getPosition()) -
                           90);
      ...
      missile->runAction(Sequence::create(
          EaseOut::create(MoveBy::create(dur / 2, Vec2(0, height)), 2),
          EaseIn::create(MoveBy::create(dur / 2, Vec2(0, -height)), 2), NULL));
    ...
    missile->runAction(Sequence::create(
        MoveTo::create(dur, target->getPosition()),
        CallFunc::create(CC_CALLBACK_0(Movable::onMissileMoveDone, missile)),
        NULL));
  }
...
void Movable::onMissileMoveDone() {
  ...
  if (attackTargetType == ATTACK_TARGET_TYPE_SPLASH) {
    if (this->shooter && (this->shooter->unitType == UNIT_WIZARD ||
                          this->shooter->unitType == UNIT_HERO_WIZARD ||
                          this->shooter->unitType == UNIT_HERO_SANTA)) {
      WORLD->runEffect(EFFECT_FIREBALL_EXPLOSION, getPosition());
...
void Movable::refreshApproachPoints() {
  ...
  if (isBuilding || unitType == UNIT_TREE) {
    int offset = 2;
    Vec2 startPos = WORLD->getPositionFromTileCoordinate(
                        buildingStartCoordinate.x,
                        buildingStartCoordinate.y + buildingOccupySize.height) +
                    Vec2(-TILE_SIZE / 2, TILE_SIZE / 2);
    approachingPoints[0] = startPos + Vec2(-offset, -offset);
    approachingPoints[1] =
        startPos + Vec2(buildingOccupySize.width * TILE_SIZE + offset, -offset);
    approachingPoints[2] =
        startPos +
        Vec2(-offset, buildingOccupySize.height * TILE_SIZE + offset);
    approachingPoints[3] =
        startPos + Vec2(buildingOccupySize.width * TILE_SIZE + offset,
                        buildingOccupySize.height * TILE_SIZE + offset);
    index = 4;
    for (int i = 0; i < buildingOccupySize.width; i++) {
      approachingPoints[index] =
          startPos + Vec2(TILE_SIZE / 2 + TILE_SIZE * i, -offset);
      approachingPoints[index + 1] =
          startPos + Vec2(TILE_SIZE / 2 + TILE_SIZE * i,
                          buildingOccupySize.height * TILE_SIZE + offset);
      index += 2;
    }
    for (int i = 0; i < buildingOccupySize.height; i++) {
      approachingPoints[index] =
          startPos + Vec2(-offset, TILE_SIZE / 2 + TILE_SIZE * i);
      approachingPoints[index + 1] =
          startPos + Vec2(buildingOccupySize.width * TILE_SIZE + offset,
    ...
    for (int x = -1; x < 2; x++) {
      for (int y = 0; y < 3; y++) {
        ...
        approachingPoints[index] =
            Vec2(currentPos.x + x * 50, bottomY + y * 50);
        index++;
      }
    }
  }
  ...
  for (int i = 0; i < approachPointCount; i++) {
    ...
    //        Sprite* spt =
    //        Sprite::createWithSpriteFrameName("workerAxeStand1.png");
    //        WORLD->spriteBatchBuilding->addChild(spt);
    //        spt->setPosition(approachingPoints[i]);
  }
}
// bug - AUBase.cpp:832:DispatchSetProperty:  ca_require: ValidFormat(inScope,
// inElement, newDesc) InvalidFormat bug - Thread 1: EXC_BAD_ACCESS (code=1,
// address=0x0)
cocos2d::Vec2 Movable::getApproachingPoint(cocos2d::Vec2 from) {
  //    if (lastApprochingPointCheckPosition != getPosition()) { // calculate
  //    points
  ...
  for (int i = 0; i < approachPointCount; i++) {
    ...
    destCoordinate =
        WORLD->getCoordinateFromPosition(approachingPoints[i], WORLD->theMap);
    //        if(!WORLD->isCoordinateValid(destCoordinate) ||
    //        WORLD->isDecoBlock(WORLD->decoLayer->getTileGIDAt(destCoordinate))
    //        || !WORLD->isCoordinateValid(coordinate) ||
    //        WORLD->isDecoBlock(WORLD->decoLayer->getTileGIDAt(coordinate))){
    ...
    if (minLength > abs(from.x - approachingPoints[i].x) +
                        abs(from.y - approachingPoints[i].y)) {
      minPos = approachingPoints[i];
      minLength = abs(from.x - approachingPoints[i].x) +
                  abs(from.y - approachingPoints[i].y);
    }
    //        }
    //        if( i < 4){
...
bool Movable::getHitAndIsDead(int ap, Movable *attacker) {
  ...
  if (attacker != nullptr && !attacker->isEnemy &&
      isShipType(attacker->unitType)) {
  ...
  if (!isEnemy && isShipType(unitType)) {
    int armorLevel = WORLD->isOrcTypeUnit(unitType) ? WORLD->orcDefenseLevel
                                                    : WORLD->humanDefenseLevel;
  ...
  if (WORLD->isMultiplay) {
    if (isEnemy) {
      ...
      //            }
      //            MultiplayManager::getInstance()->attackUnit(Value(attacker->unitID).asString(),
      //            this->unitID, ap);
    } else {
      //            if (energy <= 0) {
      //                energy = 1;
  ...
  if (isBuilding && energy < maxEnergy / 2 && !isOnFire) {
    ...
    WORLD->batch->addChild(spt);
    spt->setName("sptFire");
    spt->setPosition(getPosition() + Vec2(getContentSize().width / 6,
                                          getContentSize().height / 5));
  ...
  if (!WORLD->isMultiplay && isBuilding && isEnemy && attacker != nullptr &&
      !attacker->isEnemy) {
    WORLD->enemyAINotifyBuildingHit(attacker->getPosition());
  }
  if (attacker != target && energy > 0 && attacker != nullptr &&
      !attacker->untouchable) {
    if (!target || target == nullptr || canRevengeAttack ||
        WORLD->getAttackPriority(attacker) > WORLD->getAttackPriority(target)) {
  ...
  if (WORLD->isMultiplay && energy <= 0) {
    if (isEnemy) {
      if (attacker == nullptr) {
        ...
        MM->killedUnit(unitID, attacker->unitID, attacker->getPositionX(),
      ...
      if (attacker == nullptr) {
        ...
        MM->deadUnit(unitID, attacker->unitID, attacker->getPositionX(),
                     attacker->getPositionY());
      }
    }
  }
...
bool Movable::isTargetLockedOn() {
  if (target == nullptr)
    return false;
  int range = 3;
  if (this->getPositionX() - range <= target->getPositionX() &&
      this->getPositionX() + range >= target->getPositionX()) {
    return true;
  }

  if (this->getPositionY() - range <= target->getPositionY() &&
      this->getPositionY() + range >= target->getPositionY()) {
    return true;
  }
  return false;
}
void Movable::showEffect(int effect, float delay) {
  if (effect == EFFECT_WING) {
    for (int i = 0; i < 2; i++) {
      ...
      sptWing->setPosition(Vec2(i == 0 ? this->getContentSize().width + 3 : -3,
                                -this->getContentSize().height));
      float fadeInTime = 0.1f;
      float moveTime = 1;
      float fadeOutTime = 0.3f;
      sptWing->runAction(Sequence::create(
          DelayTime::create(delay),
          MoveBy::create(0.2f, Vec2(i == 0 ? 5 : -5, 0)), nullptr));
      sptWing->runAction(Sequence::create(
          DelayTime::create(delay),
          EaseInOut::create(ScaleTo::create(0.2f, 1, 1), 2), nullptr));
      sptWing->runAction(Sequence::create(
          DelayTime::create(delay),
          EaseInOut::create(MoveBy::create(0.2f, Vec2(0, 15)), 2), nullptr));
      sptWing->runAction(Sequence::create(
          DelayTime::create(delay), FadeIn::create(0.1f),
          DelayTime::create(moveTime - fadeInTime - fadeOutTime),
          FadeOut::create(fadeOutTime),
          CallFunc::create(CC_CALLBACK_0(Sprite::removeFromParent, sptWing)),
          nullptr));
    }
    ...
    spt->setPosition(
        Vec2(getContentSize().width / 2, getContentSize().height / 2));
    spt->setOpacity(0);
    spt->setScale(1);
    spt->runAction(Sequence::create(DelayTime::create(delay),
                                    ScaleTo::create(0.2f, 2.5), nullptr));
    spt->runAction(Sequence::create(
        DelayTime::create(delay), FadeIn::create(0.1f), DelayTime::create(1),
        FadeOut::create(0.3f),
    ...
    for (int i = 0; i < 6; i++) {   // sequence
      for (int j = 0; j < 3; j++) { // particle at once
        ...
        spt->setPosition(
            Vec2(rand() % (int)this->getContentSize().width,
                 rand() % (int)getContentSize().height + (rand() % 10) * 0.1f));
        spt->runAction(Sequence::create(
            DelayTime::create(delay + extraDelay), FadeIn::create(fadeInTime),
            DelayTime::create(moveTime - fadeInTime - fadeOutTime),
            FadeOut::create(fadeOutTime),
            CallFunc::create(CC_CALLBACK_0(Sprite::removeFromParent, spt)),
            NULL));
        spt->runAction(Sequence::create(DelayTime::create(delay + extraDelay),
                                        MoveBy::create(moveTime, Vec2(0, 10)),
                                        nullptr));
      }
      extraDelay += 0.2f;
    }
    ...
    for (int i = 0; i < 5; i++) { // sequence
      ...
      for (int j = 0; j < 16; j++) { // particle at once
        ...
        //                spt->setScale(0.5f);
        spt->runAction(Sequence::create(
            DelayTime::create(delay + extraDelay), FadeIn::create(0.05f),
            DelayTime::create(0.1f), FadeOut::create(0.05f),
            CallFunc::create(CC_CALLBACK_0(Sprite::removeFromParent, spt)),
            NULL));
        spt->runAction(Sequence::create(
            DelayTime::create(delay + extraDelay),
            MoveBy::create(0.2f, Vec2(this->getContentSize().width + 6, 0)),
            NULL));
        spt->runAction(Sequence::create(
            DelayTime::create(delay + extraDelay),
            EaseInOut::create(MoveBy::create(0.2f, Vec2(0, 5)), 2), nullptr));
        extraDelay += 0.08f;
      }
      //            extraDelay += 0.1f;
    }
...
void Movable::moveToTarget(Movable *unit) {
  ...
  log("target to unit moveFlagPos: %f, %f, %d, %d", moveFlagPos.x,
      moveFlagPos.y, unit->unitType, unit->unitID);
  moveToPos = Vec2::ZERO;
  unitActDetail = UNIT_ACT_DETAIL_IDLE;
...
void Movable::moveToTarget(cocos2d::Vec2 targetPos) {
  ...
  if (WORLD->isDecoBlock(WORLD->decoLayer->getTileGIDAt(targetMoveTilePos)) ||
      WORLD->isDecoBlock(WORLD->decoLayer->getTileGIDAt(selfPos))) {
    return;
  }
  ...
  if (isFlying || isTemporaryFlying) {
    ...
    if (failedFindPathStart == selfPos &&
    ...
    if (routePositionArray != nullptr && routePositionArray->count() > 0) {
      if (!WORLD->isDecoBlock(WORLD->decoLayer->getTileGIDAt(
              WORLD->getCoordinateFromPosition(targetPos)))) {
        routePositionArray->removeControlPointAtIndex(
      ...
      if (target != nullptr && attackType == ATTACK_TYPE_RANGE &&
          attackRange > getPosition().distanceSquared(target->getPosition())) {
...
void Movable::moveToTarget() {
  ...
  if (newTargetPos == selfPos) {
    if (!WORLD->isDecoBlock(
            WORLD->decoLayer->getTileGIDAt(WORLD->getCoordinateFromPosition(
                target->getPosition(), WORLD->theMap)))) {
      addRoute(target->getPosition());
    } else if (!WORLD->isDecoBlock(WORLD->decoLayer->getTileGIDAt(
                   WORLD->getCoordinateFromPosition(
                       target->getApproachingPoint(getPosition()),
                       WORLD->theMap)))) {
    ...
    if (isFlying || isTemporaryFlying) {
      ...
      if (array == nullptr || array->count() == 0) {
        ...
        //                if(!WORLD->isDecoBlock(WORLD->decoLayer->getTileGIDAt(WORLD->getCoordinateFromPosition(approachPos,
        //                WORLD->theMap)))){
        //                    nextPos =
        //                    WORLD->getPositionFromTileCoordinate(array->getControlPointAtIndex(array->count()-1).x,
        //                    array->getControlPointAtIndex(array->count()-1).y);
        //                }
      ...
      if (routePositionArray != nullptr && routePositionArray->count() > 0) {
        if (!WORLD->isDecoBlock(
                WORLD->decoLayer->getTileGIDAt(WORLD->getCoordinateFromPosition(
                    approachPos, WORLD->theMap)))) {
          //                    if(target->isBuilding){
          //                        lastValidPoint =
          //                        routePositionArray->getControlPointAtIndex(routePositionArray->count()
          //                        - 1);
          //                    }else{
          ...
          if (!WORLD->isDecoBlock(WORLD->decoLayer->getTileGIDAt(
                  WORLD->getCoordinateFromPosition(approachPos,
                                                   WORLD->theMap)))) {
            routePositionArray->removeControlPointAtIndex(
                routePositionArray->count() - 1);
        ...
        if (attackType == ATTACK_TYPE_RANGE &&
            attackRange >
                getPosition().distanceSquared(target->getPosition())) {
...
void Movable::gatherTree(Movable *tree) {
  ...
  this->target = tree;
  this->moveFlagPos =
      tree->getApproachingPoint(getPosition()); // tree->getPosition();
  //    this->moveToTarget(mine); // old
}
void Movable::attackTree() {
  if (currentTree != nullptr) {
    ...
    if (tree->energy <= 0) {
      Vec2 treePos = WORLD->getCoordinateFromPosition(
          tree->getPosition(),
          WORLD->theMap); // Vec2((int)currentTree->getPositionX()/TILE_SIZE,
                          // (int)currentTree->getPositionY()/TILE_SIZE);
      ...
      tree->runAction(Sequence::create(
          FadeOut::create(5),
          CallFunc::create(CC_CALLBACK_0(Node::removeFromParent, tree)), NULL));
      //            }
      //            tree->setLocalZOrder(-10000);
      ...
      if (WORLD->isMultiplay) {
        ...
        MM->cutTree(tree->unitID, unitID, getPositionX(), getPositionY(),
                    tankID);
      }
      //            Vec2 pos;
      //            for(auto unit: WORLD->mutualArray){
      //                pos = Vec2((int)unit->getPositionX()/TILE_SIZE,
      //                (int)unit->getPositionY()/TILE_SIZE); if(pos ==
      //                treePos){
      //                    spt = (Sprite*)unit;
...
void Movable::queueUnit(int index) {
  ...
  if (btns.size() == 1) {
    ...
    //        btns.at(0)->runAction(RotateBy::create(productCompleteTimer,
    //        360));
  ...
  if (index == UNIT_SWORDMAN) {
    ...
    //            Vec2(size.width/2 - 750 - 100, 330 + 50)),
    //            MoveBy::create(0.5f, Vec2(150, -200)), DelayTime::create(1),
    //            NULL)));
    //        }
...
void Movable::productUpdate(float dt) {
  ...
  if (!isResearching && btns.size() > 0) {
    ...
    if (productCompleteTimer <= 0 &&
        WORLD->foodMax >=
            WORLD->foodInUse + GM->getFoodUseForUnit(btns.at(0)->getTag())) {
      Button *btn = btns.at(0);
      int index = btn->getTag();
      std::string spriteName = WORLD->getSpriteNameForUnit(index);

      bool isShipUnit =
          (index == UNIT_HUMAN_SHUTTLE || index == UNIT_ORC_SHUTTLE ||
      ...
      EnemyBase *unit = WORLD->createUnit(
          index, isEnemy ? WHICH_SIDE_ENEMY : WHICH_SIDE_HERO, ITS_NOT_BUILDING,
          spawnPos, GM->getUnitName(index), 1, spriteName.c_str());
      ...
      if (btns.size() > 0) {
        ...
        //                360));
        ProgressTimer *timer =
            (ProgressTimer *)btns.at(0)->getChildByName("timer");
        timer->runAction(ProgressTo::create(productCompleteTimer, 100));
      }
      ...
      updateProductButtons();
      if (WORLD->isMultiplay && !isEnemy) {
        MM->createdUnit(unit->unitID, unit->unitType, (int)spawnPos.x,
                        (int)spawnPos.y, WORLD->gameFrameTimer + LATANCY);
        unit->setVisible(false);
        unit->runAction(Sequence::create(
            DelayTime::create(LATANCY),
            CallFunc::create(CC_CALLBACK_0(Movable::visibleLater, unit)),
            NULL));
      }
    }
  }
}
void Movable::visibleLater() { setVisible(true); }
...
void Movable::updateProductButtons() {
  ...
  for (auto btn : btns) {
    //        int index = btn->getTag();
    btn->setPosition(this->getPosition() +
                     Vec2(-buildingOccupySize.width * TILE_SIZE / 2 +
                              counter * 80 * WORLD->imageScale,
                          TILE_SIZE));
    counter++;
  }
}
void Movable::stopNew() {
  resetRoute();
  shipSidestepTarget = Vec2::ZERO;
  shipStallDest = Vec2::ZERO;
  shipStallBestDistSq = -1.0f;
  shipStallTimer = 0.0f;
  shipBlockedTimer = 0.0f;
  stopSkillAura();
  canRevengeAttack = true;
  ...
  bool isShipUnit2 =
      (unitType == UNIT_HUMAN_SHUTTLE || unitType == UNIT_ORC_SHUTTLE ||
       ...
       unitType == UNIT_HUMAN_BATTLE_SHIP || unitType == UNIT_ORC_BATTLE_SHIP ||
       unitType == UNIT_HUMAN_OIL_SHIP || unitType == UNIT_ORC_OIL_SHIP);
  if (!isGatheringGold && !WORLD->isThisSpotAvailable(this) && canMove &&
      !isFlying && !isShipUnit2) {
    Vec2 coordinate = WORLD->getCoordinateFromPosition(getPosition());
    ...
    if (true || !isTemporaryFlying) {
      ...
      do {
        ...
        if (originalCoordinate.y + x < 0)
          x = 0;
        if (originalCoordinate.y + y < 0)
          y = 0;
        ...
        if (GM->tileState[(int)pos.x][(int)pos.y] == 0 && counter > 1) {
          //                if(!WORLD->isOccupied(originalCoordinate + Vec2(x,
          //                y)) && counter > 1){
          PointArray *array =
              PointArray::create(1); // GM->getPath(originalCoordinate,
                                     // originalCoordinate + Vec2(x, y));
    ...
    if (goRandom) {
      coordinate =
          Vec2(coordinate.x + rand() % 3 - 1, coordinate.y + rand() % 3 - 1);
    }
    ...
    Vec2 newPos =
        WORLD->getPositionFromTileCoordinate(coordinate.x, coordinate.y);
    //        WORLD->moveTo((EnemyBase*)this, newPos);
    ...
    //                moveToPos = Vec2::ZERO;
    //                moveFlagPos = newPos;
    //                MM->moveUnit(Value(unitID).asString(), (int)moveFlagPos.x,
    //                (int)moveFlagPos.y);
    //            }
    //        }
    //        else
  ...
  if (isGoingToBuild) {
    ...
    Movable *unit = WORLD->buildTheBuilding(
        builderBuildingIndex, builderCoordinate.x, builderCoordinate.y,
        builderSize.width, builderSize.height, builderSpriteName, alliSide,
        isFree);
    ...
    if (!isEnemy) {
      Button *btn = Button::create("uiBox.png");
      btn->setPosition(unit->getPosition() +
                       Vec2(50, unit->getContentSize().height / 2 + 10));
      btn->setScale(0.3f);
      btn->setTag(unit->unitType);
      btn->addClickEventListener(
          CC_CALLBACK_1(Movable::cancelBuildingConstruction, this));
      ...
      timer->runAction(Sequence::create(
          ProgressTo::create(dur, 100),
          CallFunc::create(CC_CALLBACK_0(Movable::resetProcessTimer, this)),
          CallFunc::create(
              CC_CALLBACK_0(Movable::removeBuildProgressIcon, this)),
          nullptr));
    } // progress end
    ...
    this->runAction(Sequence::create(
        ...
        DelayTime::create(0.2f),
        CallFunc::create(CC_CALLBACK_0(Movable::playTreeSound, this)),
        DelayTime::create(0.2f), NULL));
    Action *buildCompleteAction = Sequence::create(
        DelayTime::create(unit->buildingCompleteTime),
        CallFunc::create(CC_CALLBACK_0(Movable::onBuildComplete, this)), NULL);
    ...
    //        if (WORLD->isMultiplay && !isEnemy) {
    //            MM->workerBuilding(unitID, builderBuildingIndex,
    //            builderCoordinate.x, builderCoordinate.y);
    //        }
  }
}
...
void Movable::moveNew(float dt) { // movenew start
  ...
  // gathering gold
  if (isGatheringGold) {
    if (returningPlace != nullptr && isCarryingGold &&
        returningPlace->getBoundingBoxForIntersect().intersectsRect(
    ...
    if (!isCarryingGold &&
        currentMine->getBoundingBoxForIntersect().intersectsRect(
            getBoundingBoxForIntersect())) {
      ...
      if (gatheringTimer < 0) {
        if (currentMine->energy > 0) {
          ...
          if (castle != nullptr && currentMine != nullptr) {
            ...
            moveToPos = Vec2::ZERO;
            moveFlagPos = target->getApproachingPoint(
            ...
            //                        MM->returnLumber(Value(unitID).asString(),
            //                        returningPlace->unitID);
          }
  ...
  // gathering tree
  if (isGatheringTree) {
    if (!isCarryingTree && currentTree != nullptr &&
        currentTree->getBoundingBoxForIntersect().intersectsRect(
            this->getBoundingBoxForIntersect())) {
      unitName = getName();
      attackCoolTime -= dt;
      ...
      this->runAction(Sequence::create(
          DelayTime::create(attackHappenTime),
          CallFunc::create(CC_CALLBACK_0(Movable::attackTree, this)), nullptr));
      this->runAction(Sequence::create(
          DelayTime::create(0.26f),
          CallFunc::create(CC_CALLBACK_0(Movable::playTreeSound, this)),
          nullptr));
      return;
    } else if (returningPlace != nullptr && isGatheringTree && isCarryingTree &&
               returningPlace->getBoundingBoxForIntersect().intersectsRect(
                   getBoundingBoxForIntersect())) {
  ...
  if (isGatheringOil) {
    if (isCarryingOil) {
      ...
      if (returningPlace != nullptr) {
        if (returningPlace->getBoundingBoxForIntersect().intersectsRect(
                getBoundingBoxForIntersect())) {
          atDepot = true;
        } else {
          Vec2 approach = returningPlace->getApproachingPoint(getPosition());
          if (approach.distanceSquared(getPosition()) <
      ...
      if (currentOilExtractor->getBoundingBoxForIntersect().intersectsRect(
        ...
        if (oilGatheringTimer < 0) {
          ...
          this->untouchable = false;
          this->attackType = ATTACK_TYPE_NEAR;
          // The extractor's footprint tiles are occupied in the
          // water A* (resetWaterPathState), so a ship standing
          // inside it can't path anywhere. Pop it out to the
          // nearest free water tile, like a worker reappearing
          // beside a gold mine.
          {
            ...
            if ((int)tile.x >= (int)st.x &&
                (int)tile.x < (int)(st.x + sz.width) &&
                (int)tile.y >= (int)st.y &&
                (int)tile.y < (int)(st.y + sz.height)) {
              Vec2 freePos = WORLD->findEmptyWaterSpawnTile(
                  (int)st.x, (int)st.y, (int)sz.width, (int)sz.height);
              if (freePos.x >= 0) {
                setPosition(freePos);
          ...
          if (depot != nullptr) {
            ...
            // Navigate like workers returning to castle. Aim at a water tile
            // beside the depot (not its generic land approach point) so the
            // ship's water A* can route around any land between here and there.
            target = depot;
            moveFlagPos = WORLD->getOilShipReturnPoint(depot, getPosition());
            unitAct = UNIT_ACT_MOVE;
            unitActDetail = UNIT_ACT_DETAIL_IDLE;
            moveToPos = Vec2::ZERO;
          }
  ...
  // handle move
  if (!isBuilding &&
      (unitAct == UNIT_ACT_MOVE || unitAct == UNIT_ACT_ATTACK_DDANG ||
       unitAct == UNIT_ACT_ATTACK || unitAct == UNIT_ACT_GATHER_GOLD ||
       unitAct == UNIT_ACT_GATHER_TREE) &&
      unitActDetail != UNIT_ACT_DETAIL_ATTACK) {
    ...
    if (moveToPos == Vec2::ZERO && unitActDetail != UNIT_ACT_DETAIL_ATTACK) {
      ...
      //                if(target->isBuilding){
      //                    for (int j = 0; j <
      //                    target->buildingOccupySize.height; j++) {
      //                        for (int i = 0; i <
      //                        target->buildingOccupySize.width; i++) {
      //                            GM->setPathState(target->buildingStartCoordinate.x
      //                            + i, target->buildingStartCoordinate.y + j,
      //                            false);
      //                        }
      //                    }
      //                }else if(target->unitType == UNIT_TREE ||
      //                target->unitType == UNIT_TREE_FOR_BATTLE){
      //                    Vec2 targetPos =
      //                    WORLD->getCoordinateFromPosition(target->getPosition());
      //                    GM->setPathState(targetPos.x, targetPos.y, false);
      //                }
      //            }
      // check ground under the feet occupied
      //            bool isGroundUnderTheFeetOccupied = false;
      //            Vec2 pos = WORLD->getCoordinateFromPosition(getPosition(),
      //            WORLD->theMap); Movable* theObjectThisIsIn = nullptr; if
      //            (GM->tileState[(int)pos.x][(int)pos.y] > 0) {
      //                theObjectThisIsIn =
      //                WORLD->getGroundOwner(getPosition()); if
      //                (theObjectThisIsIn == nullptr) {
      //                    stopNew();
      //                }else{
      //                    isGroundUnderTheFeetOccupied = true;
      //                    if(theObjectThisIsIn->isBuilding){
      //                        for (int j = 0; j <
      //                        theObjectThisIsIn->buildingOccupySize.height;
      //                        j++) {
      //                            for (int i = 0; i <
      //                            theObjectThisIsIn->buildingOccupySize.width;
      //                            i++) {
      //                                GM->setPathState(theObjectThisIsIn->buildingStartCoordinate.x
      //                                + i,
      //                                theObjectThisIsIn->buildingStartCoordinate.y
      //                                + j, false);
      ////                                log("unlock block: %f, %f",
      ///theObjectThisIsIn->buildingStartCoordinate.x + i,
      ///theObjectThisIsIn->buildingStartCoordinate.y + j);
      //                            }
      //                        }
      //                    }else if(theObjectThisIsIn->unitType == UNIT_TREE ||
      //                    theObjectThisIsIn->unitType ==
      //                    UNIT_TREE_FOR_BATTLE){
      //                        Vec2 targetPos =
      //                        WORLD->getCoordinateFromPosition(theObjectThisIsIn->getPosition());
      //                        GM->setPathState(targetPos.x, targetPos.y,
      //                        false);
      //                    }
      //                }
      //            }
      ...
      if (isFlying || isTemporaryFlying) {
        ...
        // Ships use A* over water tiles to navigate around land and water
        // buildings.
      ...
      //                }
      //            }
      //            if(isGroundUnderTheFeetOccupied){ // putback
      //            theBuildingThisIsIn occupied ground
      //                if(theObjectThisIsIn->isBuilding){
      //                    for (int j = 0; j <
      //                    theObjectThisIsIn->buildingOccupySize.height; j++) {
      //                        for (int i = 0; i <
      //                        theObjectThisIsIn->buildingOccupySize.width;
      //                        i++) {
      //                            GM->setPathState(theObjectThisIsIn->buildingStartCoordinate.x
      ...
      //                    }
      //                }else if(theObjectThisIsIn->unitType == UNIT_TREE ||
      //                theObjectThisIsIn->unitType == UNIT_TREE_FOR_BATTLE){
      //                    Vec2 targetPos =
      //                    WORLD->getCoordinateFromPosition(theObjectThisIsIn->getPosition());
      //                    GM->setPathState(targetPos.x, targetPos.y, true);
      //                }
      ...
      if (isCarryingTree && returningPlace != nullptr) {
        array->addControlPoint(
            WORLD->getCoordinateFromPosition(returningPlace->getPosition()));
      }
      Vec2 pos;
      for (int i = startI; i >= 0; i--) {
        pos = array->getControlPointAtIndex(i);
    ...
    if (unitAct == UNIT_ACT_ATTACK) {
      if (!target || target->energy <= 0) {
        ...
        if (moveFlagPos != target->getPosition()) {
          ...
          moveToPos = moveFlagPos;
          //                    MM->moveUnit(Value(target->unitID).asString(),
          //                    moveFlagPos.x, moveFlagPos.y);
        }
      }
    }
    ...
    if (!isAlreadyArrived && isArrived &&
        unitActDetail != UNIT_ACT_DETAIL_ATTACK &&
        unitAct != UNIT_ACT_GATHER_GOLD &&
        unitAct != UNIT_ACT_GATHER_TREE) { // arrived
      if (shipSidestepTarget != Vec2::ZERO) {
        // Finished a deadlock-recovery slide - don't stop, re-path to the real
        // goal (still held in moveFlagPos) by clearing moveToPos.
        shipSidestepTarget = Vec2::ZERO;
        moveToPos = Vec2::ZERO;
      } else {
        if (attackDdangPos != Vec2::ZERO) {
      ...
      //                if(isCarryingTree){
      //                    EnemyBase* tank =
      //                    WORLD->getNearestLumberTank(getPosition()); unitName
      //                    = strmake("%sWood", getName().c_str()); if(tank !=
      //                    nullptr){
      //                        returningPlace = tank;
      //                        moveToTarget(tank);
      ////                        runMoveAnimation(DIRECTION_E);
      ...
      //                }else{
      //                    EnemyBase* tree =
      //                    WORLD->getNearestTree(getPosition()); if(tree !=
      //                    nullptr){
      //                        gatherTree(tree);
      //                    }
      //                }
      ...
      //                        Vec2 pos =
      //                        WORLD->heroArray.at(0)->getPosition();
      //                        if(failedAttackFlagPos != pos){
      //                            attackFlagPos = pos;
      //                            attackFlagTilePos =
      //                            WORLD->getCoordinateFromPosition(attackFlagPos,
      //                            WORLD->theMap);
      //                        }
      //                    }
      //                }
      ...
      // Naval units - water-only movement. isShip gates that; ship-vs-ship
      // collision is a separate opt-in (collidesWithShips) below.
      bool isShip =
          (unitType == UNIT_HUMAN_SHUTTLE || unitType == UNIT_ORC_SHUTTLE ||
           unitType == UNIT_HUMAN_SHIP || unitType == UNIT_ORC_SHIP ||
           unitType == UNIT_HUMAN_BATTLE_SHIP ||
           unitType == UNIT_ORC_BATTLE_SHIP ||
           unitType == UNIT_HUMAN_OIL_SHIP || unitType == UNIT_ORC_OIL_SHIP);
      // Oil ships move like land workers: routed around terrain/buildings by
      // their water A* but never blocked by (or jammed against) other ships.
      // They shuttle constantly through the tight water between a shipyard and
      // an oil extractor, where mutual blocking is the main source of deadlock,
      // so they neither register in shipOccupiedTiles nor test it. Other ship
      // types still collide normally.
      bool isOilShip =
          (unitType == UNIT_HUMAN_OIL_SHIP || unitType == UNIT_ORC_OIL_SHIP);
      bool collidesWithShips = isShip && !isOilShip;
      Vec2 tileBeforeThisFrame;
      if (collidesWithShips) {
        tileBeforeThisFrame = WORLD->getCoordinateFromPosition(getPosition());
        WORLD->shipOccupiedTiles.insert(std::make_pair(
            (int)tileBeforeThisFrame.x, (int)tileBeforeThisFrame.y));
      }
      if ((unitActDetail == UNIT_ACT_DETAIL_WALK_ROUTE ||
           unitActDetail == UNIT_ACT_DETAIL_WALK_STRAIGHT_TO_TARGET)) {
        Vec2 dest, current;
        ...
        float xWillMove = extraSpeed * 0.01f *
                          (xDistanceToMove / totalDistance) *
                          (xToMove < 0 ? -1 : 1);
        float yWillMove = extraSpeed * 0.01f *
                          (yDistanceToMove / totalDistance) *
                          (yToMove < 0 ? -1 : 1);
        bool shipBlockedThisFrame = false;
        ...
        if (collidesWithShips) {
          ...
          // buildings). So several oil ships shuttling through the narrow water
          // between a shipyard and an oil extractor block each other's next
          // tile and all freeze. When a ship stays blocked past a threshold,
          // sidestep it to the free neighbor tile nearest the destination and
          // re-path from there - that shuffles the jam apart. The threshold is
          // staggered by unitID so two ships facing off don't jump on the same
          // frame and just swap tiles forever.
          // No-progress watchdog (fallback to the next-tile-blocked check
          // above). Track the closest the ship has come to its current waypoint
          // (dest); while that best keeps improving the ship is making real
          // progress. When it fails to improve for kStallThreshold seconds the
          // ship is stuck even though it may still be jittering, so run the same
          // escape. dest is the immediate waypoint, so detouring around land -
          // where straight-line distance to the final goal can grow - is not
          // mistaken for a stall. A sidestep slide sets dest to its free
          // neighbor and moves toward it, so the timer resets and won't cut the
          // slide short.
          if (shipStallDest != dest) {
            shipStallDest = dest;
            shipStallBestDistSq = -1.0f;
            shipStallTimer = 0.0f;
          }
          float stallDistSq = getPosition().distanceSquared(dest);
          if (shipStallBestDistSq < 0.0f ||
              stallDistSq < shipStallBestDistSq - 4.0f) { // ~2px of progress
            shipStallBestDistSq = stallDistSq;
            shipStallTimer = 0.0f;
          } else {
            shipStallTimer += dt;
          }
          const float kStallThreshold = 1.5f;
          bool stalled = shipStallTimer > kStallThreshold;

          if (shipBlockedThisFrame) {
            shipBlockedTimer += dt;
          } else {
            shipBlockedTimer = 0.0f;
          }
          float threshold = 0.4f + (float)(unitID % 3) * 0.12f;
          bool blockedLongEnough =
              shipBlockedThisFrame && shipBlockedTimer > threshold;
          if (blockedLongEnough || stalled) {
            {
              Vec2 curTile = WORLD->getCoordinateFromPosition(getPosition());
              // Measure against the true goal (moveFlagPos), not moveToPos -
              // during a recovery slide moveToPos is just the intermediate tile.
              Vec2 finalDest =
                  (moveFlagPos != Vec2::ZERO) ? moveFlagPos : moveToPos;
              Vec2 destTile = WORLD->getCoordinateFromPosition(finalDest);
              float ax = curTile.x - destTile.x;
              if (ax < 0)
                ax = -ax;
              float ay = curTile.y - destTile.y;
              if (ay < 0)
                ay = -ay;
              float curDistSq = ax * ax + ay * ay;
              int chebyshev = (int)(ax > ay ? ax : ay);
              static const int ndx[8] = {1, -1, 0, 0, 1, 1, -1, -1};
              static const int ndy[8] = {0, 0, 1, -1, 1, -1, 1, -1};
              int bestK = -1;
              float bestDistSq = 1e18f;
              for (int k = 0; k < 8; k++) {
                int nx = (int)curTile.x + ndx[k];
              ...
              // How to react to being blocked depends on where we are:
              //  - A free neighbor gets us strictly closer to the goal: take it
              //    (route around the jam - real progress, never oscillates).
              //  - No closer tile, but we're already clustered near the goal:
              //    settle here and stop. A whole group ordered to one tile can't
              //    all stand on it, so the ones that can't get closer just park
              //    around it instead of teleporting out and pathing back forever.
              //  - No closer tile and still far from the goal: genuine mid-route
              //    jam (e.g. a narrow channel) - sidestep to any free tile to
              //    break the deadlock, even if it isn't closer.
              const int kSettleRadius = 3;
              bool doSidestep = false;
              if (bestK >= 0 && bestDistSq < curDistSq) {
                // A neighbor is closer to the goal - slide there to route around.
                doSidestep = true;
              } else if (chebyshev <= kSettleRadius) {
                // Clustered at the goal, nowhere closer - park here and stop.
                attackFlagPos = Vec2::ZERO;
                stopNew();
              } else if (bestK >= 0) {
                // Mid-route jam far from goal - slide aside to break it.
                doSidestep = true;
              }
              if (doSidestep) {
                // Slide to the free neighbor as an ordinary straight move so it
                // travels at the ship's normal speed (no teleport) and reuses
                // the shared move animation/visuals. Its arrival is intercepted
                // (see the "arrived" branch) to re-path to the real goal instead
                // of stopping. moveFlagPos still holds the true destination.
                shipSidestepTarget = WORLD->getPositionFromTileCoordinate(
                    (int)curTile.x + ndx[bestK], (int)curTile.y + ndy[bestK]);
                resetRoute();
                moveToPos = shipSidestepTarget;
                unitActDetail = UNIT_ACT_DETAIL_WALK_STRAIGHT_TO_TARGET;
              }
            }
            shipBlockedTimer = 0.0f;
        ...
        if (collidesWithShips) {
          ...
          if ((int)tileAfter.x != (int)tileBeforeThisFrame.x ||
              (int)tileAfter.y != (int)tileBeforeThisFrame.y) {
            WORLD->shipOccupiedTiles.erase(std::make_pair(
                (int)tileBeforeThisFrame.x, (int)tileBeforeThisFrame.y));
            WORLD->shipOccupiedTiles.insert(
                std::make_pair((int)tileAfter.x, (int)tileAfter.y));
          }
        }
        if (unitAct == UNIT_ACT_ATTACK) {
          checkAttackTargetReturnSuccess(dt);
  ...
  //    WORLD->gameTimer > 240 && WORLD->gameTimer < 2400) {
  if ((scheduledProductTime >= 0 || scheduledAttackTime >= 0)) {
      ...
      if (scheduledProductTime < timeCollapse) {
        ...
        for (int i = 0; i < scheduledProductUnitCount; i++) {
          ...
          if (unitType == UNIT_ZOMBIE_SWORDSMAN) {
            unitType =
                rand() % 2 == 0 ? UNIT_ZOMBIE_SWORDSMAN : UNIT_ZOMBIE_ORC_AXE;
          } else if (unitType == UNIT_ORC_AXE) {
            unitType = rand() % 2 == 0 ? UNIT_ORC_AXE : UNIT_ORC_SPEAR;
          }
          EnemyBase *createdUnit = WORLD->createUnit(
              unitType, isEnemy, false,
              this->getPosition() +
                  Vec2(i * 100 * WORLD->imageScale, -200 * WORLD->imageScale),
  ...
  //                    MM->moveFlagChanged(unitID, moveFlagPos.x,
  //                    moveFlagPos.y);
  //                }
  //            }
  //        }
...
bool Movable::checkAttackTargetReturnSuccess(float dt) {
  ...
  if (target != nullptr &&
      (isEnemy != target->isEnemy || forceAttackTarget ||
       target->unitType == UNIT_TREE_FOR_BATTLE) &&
    ...
    if (attackType == ATTACK_TYPE_RANGE) {
      ...
      if (unitType == UNIT_GOBLIN_BOMB) {
        ...
        if (this->getBoundingBoxForIntersect().intersectsRect(
                target->getBoundingBoxForIntersect())) {
          attackUpdate(dt);
          return true;
        } else {
          if (unitActDetail == UNIT_ACT_DETAIL_ATTACK) {
...
void Movable::move(float dt) {
  ...
  if (oneSec < oneSecTimeChecker) {
    ...
    if (isZombie) {
      if (target == nullptr &&
          (routePositionArray == nullptr || routePositionArray->count() == 0)) {
        Vec2 minPos;
        ...
        for (int i = -1; i < 2; i++) {
          for (int j = -1; j < 2; j++) {
            ...
            Vec2 newPos = Vec2(getPositionX() + i * TILE_SIZE,
                               getPositionY() + j * TILE_SIZE);
            ;
            if (minLength >
                    abs(getPositionX() - attackFlagPos.x + i * TILE_SIZE) +
                        abs(getPositionY() - attackFlagPos.y + j * TILE_SIZE) &&
                !WORLD->isDecoBlock(WORLD->decoLayer->getTileGIDAt(
                    WORLD->getCoordinateFromPosition(newPos))) &&
                WORLD->isThisSpotAvailable(newPos)) {
              minPos = newPos;
              minLength =
                  abs(getPositionX() - attackFlagPos.x + i * TILE_SIZE) +
                  abs(getPositionY() - attackFlagPos.y + j * TILE_SIZE);
  ...
  if ((scheduledProductTime >= 0 || scheduledAttackTime >= 0) &&
      WORLD->gameTimer > 240 && WORLD->gameTimer < 2400) {
    //    if ((scheduledProductTime >= 0 || scheduledAttackTime >= 0) &&
    //    WORLD->gameTimer > 0 && WORLD->gameTimer < 2400) { // test
    timeCollapse += dt; // test
    if (scheduledAttackTime >= 0) {
      if (scheduledAttackTime < timeCollapse) {
        scheduledAttackTime = -1;
        ...
        //                        WORLD->moveAndAttackTo((EnemyBase*)this,
        //                        unit->getApproachingPoint(getPosition()));
        //                        break;
        //                    }
        //                }
        WORLD->attackNearHero((EnemyBase *)this);
      ...
      if (scheduledProductTime < timeCollapse) {
        ...
        for (int i = 0; i < scheduledProductUnitCount; i++) {
          ...
          EnemyBase *createdUnit =
              WORLD->createUnit(unitType, isEnemy, false,
                                this->getPosition() + Vec2(i * 50, -300),
                                GM->getUnitName(unitType));
          //                            WORLD->moveAndAttackTo((EnemyBase*)createdUnit,
          //                            unit->getApproachingPoint(createdUnit->getPosition()));
          createdUnit->wantToEli = true;
        }
  ...
  if (isGatheringGold) {
    if (returningPlace != nullptr && isCarryingGold &&
        returningPlace->getApproachingPoint(getPosition()) ==
            getPosition()) { // returningPlace->getBoundingBox().containsPoint(this->getPosition()))
                             // {//returningPlace->getBoundingBox().intersectsRect(this->getBoundingBox()))
                             // {
      if (!isEnemy) {
        WORLD->addGold(40);
      }
    ...
    } else if (!isCarryingGold && currentMine->getApproachingPoint(
  ...
  if (isGatheringTree) {
    if (!isCarryingTree && currentTree != nullptr &&
        currentTree->getBoundingBox().intersectsRect(
            this->getBoundingBoxForIntersect())) {
      //            this->setName("worker");
      //            this->setName(getName().substr(0, getName().size() - 4));
      unitName = getName();
      ...
      return;
               ...
               returningPlace->getBoundingBoxForIntersect().intersectsRect(
                   getBoundingBox())) { //&&
                                        //returningPlace->getBoundingBox().containsPoint(getPosition()))
                                        //{
      isCarryingTree = false;
      stop();
      //            this->setName("worker");
      ...
      ////                while((tree = WORLD->getNearestTree(getPosition())) !=
      ///nullptr){
      ////
      ////                }
      //            }
  ...
  if (targetPositionCheckTime < 0 && target != nullptr && target->canMove &&
      !target->untouchable && canMove && !target->isGatheringGold &&
      !target->isGatheringTree) {
    targetPositionCheckTime = 1;

    //        Vec2 newTargetPos =
    //        WORLD->getCoordinateFromPosition(target->getBoundingBox().origin +
    //        Vec2(target->getBoundingBox().size.width/2,
    //        target->collisionBoundingBox().size.height/2), WORLD->theMap);
    Vec2 newTargetPos;
    //        if(isFlying || isTemporaryFlying){
    //            newTargetPos = target->getPosition();
    //        }else{
    newTargetPos = WORLD->getCoordinateFromPosition(
        target->getApproachingPoint(getPosition()), WORLD->theMap);
    //        }
    if (newTargetPos != targetCoordinate) {
      stop();
      moveToTarget();
  ...
  if (target != nullptr && (isEnemy != target->isEnemy || forceAttackTarget) &&
      attackType != ATTACK_TYPE_NONE) {
    if (attackType == ATTACK_TYPE_RANGE) {
      if (target->getPosition().distanceSquared(getPosition()) < attackRange) {
        // attack
  ...
  if (movePointCounter >= routePositionArray->count()) { // arrived
    ...
    if (isEnemy && WORLD->heroArray.size() > 0) {
      ...
      if (targetEnemy != nullptr) {
        ...
        if (failedAttackFlagPos != pos) {
          attackFlagPos = pos;
          attackFlagTilePos =
              WORLD->getCoordinateFromPosition(attackFlagPos, WORLD->theMap);
        }
      }
    }
    //        log("what?");
    ...
    //            slowFrameTime--;
    //            slowRate = 0.5f;
    //            GameManager::getInstance()->makeAdditive(this, Color3B(100,
    //            100, 230));
    //        }
    //        else{
    //            GameManager::getInstance()->clearAdditive(this);
    //        }
    float xWillMove = extraSpeed * 0.01f * slowRate *
                      (xDistanceToMove / totalDistance) *
                      (xToMove < 0 ? -1 : 1);
    float yWillMove = extraSpeed * 0.01f * slowRate *
                      (yDistanceToMove / totalDistance) *
                      (yToMove < 0 ? -1 : 1);
...
Vec2 Movable::getEffectStartPosition() {
  if (isBuilding) {
    if (unitType == UNIT_WATCHERTOWER) {
      ...
      return getPosition() +
             Vec2((-10) * WORLD->imageScale, (-54) * WORLD->imageScale);
    } else {
      return getPosition() +
             Vec2(0, (-getContentSize().height / 2 + 25) * WORLD->imageScale);
    }

    //        return getPosition();
  } else {
    //        return getPosition() + Vec2(0, 14*WORLD->imageScale);//Vec2(0,
...
void Movable::updateGroundEffect(float dt) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    spt->setPosition(this->getPosition() +
                     Vec2(-7 + rand() % 16, 4 - rand() % 9));
    spt->runAction(Sequence::create(MoveBy::create(1.5f, Vec2(0, 32)),
                                    SPT_REMOVE_FUNC, NULL));
    spt->runAction(
        Sequence::create(DelayTime::create(1.0f), FadeOut::create(0.5f), NULL));
  }
}
...
void Movable::updateEnergy() {
  if (energyBar == nullptr) {
    ...
    if (this->isBuilding) {
      batch = WORLD->batch;
      energyBarBack =
          Sprite::createWithSpriteFrameName("energyBarContentForBuilding.png");
      energyBarContent =
          Sprite::createWithSpriteFrameName("energyBarContentForBuilding.png");
      energyBar =
          Sprite::createWithSpriteFrameName("energyBarFrame0ForBuilding.png");
      ...
      energyBarContent =
          Sprite::createWithSpriteFrameName("energyBarContent.png");
  ...
  energyBarContent->setTextureRect(
      cocos2d::Rect(textureRect.origin.x, textureRect.origin.y, 37 * rate, 12));
  if (rate > 0.7f) {
...
void Movable::makingSmoke(float dt) {
  ...
  spt->runAction(Sequence::create(ScaleTo::create(0.1f, 1),
                                  DelayTime::create(1),
                                  ScaleTo::create(3, 1.5f), nullptr));
  spt->runAction(Sequence::create(MoveBy::create(dur, Vec2(200, 0)),
                                  SPT_REMOVE_FUNC, NULL));
  spt->runAction(Sequence::create(
      EaseOut::create(MoveBy::create(dur * 2 / 3, Vec2(0, 200)), 2),
      MoveBy::create(dur / 3, Vec2(0, 10)), nullptr));
  spt->runAction(Sequence::create(DelayTime::create(dur * 3 / 4),
                                  FadeOut::create(dur / 4), NULL));
  this->getParent()->addChild(spt);
}
...
void Movable::showSelectedCircle(bool show) {
  if (show) {
    if (sptSelectedCircle == nullptr) {
      if (isBuilding) {
        sptSelectedCircle =
            Sprite::createWithSpriteFrameName("selectedCircleBuilding.png");
        WORLD->spriteBatchBuilding->addChild(sptSelectedCircle, -20000);
      } else {
        sptSelectedCircle =
            Sprite::createWithSpriteFrameName("selectedCircle.png");
        WORLD->spriteBatchBuilding->addChild(sptSelectedCircle, -20000);
      }
      if (spine) {
        sptSelectedCircle->setScale((this->getContentSize().width) * 1.5f /
                                    sptSelectedCircle->getContentSize().width);
      } else {
        if (isBuilding) {
          sptSelectedCircle->setScale(
              (this->getContentSize().width) * 1.3f /
              sptSelectedCircle->getContentSize().width);
        } else {
          sptSelectedCircle->setScale(
