...
BuggyServerManager *BuggyServerManager::getInstance() {
  if (NULL == m_mySingleton) {
    ...
    //        m_mySingleton->HUDLayer =
    //        (HUDScene*)GameManager::sharedGameManager()->getHUDLayer();
  }
...
//    serverUrl = "http://192.168.0.2:8103"; // cartoon-new server test
  ...
  isOffline = false;
  establishLocalTimeBaseline();
#endif
  //    serverUrl = "http://222.120.115123213.95:8103"; //  test

  ////////////////////////////    serverUrl = "http://52.78.139.153:8103"; //
  ///aws test - now use anymore
...
void BuggyServerManager::setServerUrl() {
  ...
  if (server == 0) {
    serverUrl = "http://1506games.com:8103"; // yangpyung  alpha
    // serverUrl = "http://3.35.68.88:8103"; // AWS Lightsail Vsisters alpha
    //        serverUrl = "http://3.36.133.207:8103"; // AWS Lightsail
    //        rightguy22 alpha
  } else if (server == 1) {
    serverUrl = "http://1506games.com:8109"; // bungee
    //        serverUrl = "http://3.35.68.88:8109"; // AWS Lightsail Vsisters
    //        bungee serverUrl = "http://3.36.133.207:8109"; // AWS Lightsail
    //        rightguy22 beta
  } else {
    serverUrl = "http://1506games.com:8103"; // yangpyung  alpha
    //        serverUrl = "http://3.35.68.88:8103"; // AWS Lightsail Vsisters
    //        alpha serverUrl = "http://3.36.133.207:8103"; // AWS Lightsail
    //        rightguy22 alpha
  }
}
void BuggyServerManager::establishLocalTimeBaseline() {
  time_t currentLocalTime = time(nullptr);
  receivedWebTimeT = currentLocalTime;
  startLocalTime = currentLocalTime;
  startLocalTimeDouble = (double)currentLocalTime;
  webTime = (double)currentLocalTime;
  timeEstablished = true;

  time_t yesterdayNow = currentLocalTime - 60 * 60 * 24;
  struct tm *yesterdayInfo = localtime(&yesterdayNow);
  if (yesterdayInfo != nullptr) {
    receivedYesterdayTime = *yesterdayInfo;
  }
}
...
void BuggyServerManager::getHttpTime() {
  ...
  if (!timeEstablished || startLocalTime == 0) {
    establishLocalTimeBaseline();
  }
  isTrying = true;
  ...
  request->setResponseCallback(
      this, httpresponse_selector(BuggyServerManager::onHttpRequestCompleted));
...
void BuggyServerManager::onHttpRequestCompleted(Node *sender, void *data) {
  ...
  std::string str = std::string(response->getResponseHeader()->begin(),
                                response->getResponseHeader()->end());
  //    int size = (int)response->getResponseHeader()->size();
  std::string responseData = std::string(response->getResponseData()->begin(),
                                         response->getResponseData()->end());
  ...
  if (responseData.length() < 10) {
    log("server time response too short, using local time");
    establishLocalTimeBaseline();
    return;
  }
...
void BuggyServerManager::getVer() {
  ...
  request->setResponseCallback(
      this, httpresponse_selector(BuggyServerManager::onGetVerCompleted));
...
std::string BuggyServerManager::getStrFromTime(time_t timet) {
  if (!timeEstablished || timet < 0) {
    establishLocalTimeBaseline();
    timet = receivedWebTimeT;
  }

  struct tm *timeinfo = gmtime(&timet);
  if (timeinfo == nullptr) {
    establishLocalTimeBaseline();
    timeinfo = gmtime(&receivedWebTimeT);
    if (timeinfo == nullptr) {
  ...
  char buffer[32] = {0};
  if (strftime(buffer, sizeof(buffer), "%F %X", timeinfo) == 0) {
    return "1970-01-01 00:00:00";
  }
...
//     time_t what = getTimeTFromStr(strTime);//mktime(&theTime); // UTC time -
//     1 hour can diff for summertime double tempWebTime = what; tempWebTime +=
//     tz_offset(); // + local timezone diff return tempWebTime;
// }return p;
// }
void BuggyServerManager::setTime(std::string strTime) {
  ...
  //    receivedTime = *localtime(&now); // 지역표준시로 변환한다 (대한민국은
  //    KST)
  ...
  //    log("http time-> %d/%d/%d %d:%d:%d",
  //    receivedTime->tm_year,receivedTime->tm_mon, receivedTime->tm_mday,
  //    receivedTime->tm_hour, receivedTime->tm_min, receivedTime->tm_sec);
  timeEstablished = true;
...
time_t BuggyServerManager::getCurrentTimeT() {
  ...
  if (startLocalTime == 0) {
    startLocalTime = time(nullptr);
    receivedWebTimeT = startLocalTime;
  }
  time_t currentLocalTime = time(nullptr);
  double timeDiff = difftime(currentLocalTime, startLocalTime);

  return receivedWebTimeT + (time_t)timeDiff;
}
...
//     }
////        log("** get current time web: %lf, startLocalTime: %lf,
///currentLocalTime: %lf", webTime, startLocalTimeDouble,
///currentLocalTimeDouble);
//    return webTime + (currentLocalTimeDouble - startLocalTimeDouble);
//    //    return GameUserInfo::getInstance()->serverTime + (currentLocalTime -
//    startLocalTime);
//}
int BuggyServerManager::getDay() {
  ...
  std::string strTime = getStrFromTime(now);
  if (strTime.length() >= 10) {
    day = Value(strTime.substr(8, 2)).asInt();
  }
  return day > 0 ? day : 1;
...
int BuggyServerManager::getYear() {
  ...
  std::string strTime = getStrFromTime(now);
  if (strTime.length() >= 4) {
    return Value(strTime.substr(0, 4)).asInt();
  }
  return 1970;
}
...
rapidjson::Document BuggyServerManager::getDocument(cocos2d::Node *sender,
                                                    void *data) {
  ...
  std::string str = std::string(response->getResponseHeader()->begin(),
                                response->getResponseHeader()->end());
  //    int size = (int)response->getResponseHeader()->size();
  std::string resposeData = std::string(response->getResponseData()->begin(),
                                        response->getResponseData()->end());
  if (!response->isSucceed()) {
  ...
  if (buffer->size() > 0) {
    ...
    // cocos2d::MessageBox(__String::createWithFormat("** res %s",
    // res.c_str())->getCString(), "result");
...
void BuggyServerManager::sendPost(std::string method, std::string requestData,
                                  SEL_HttpResponse pSelector) {
  cocos2d::network::HttpRequest *request = new cocos2d::network::HttpRequest();
  request->setUrl(
      strmake("%s/%s/%s/", serverUrl.c_str(), apiName.c_str(), method.c_str()));
  std::vector<std::string> headers;
...
void BuggyServerManager::sendGet(std::string method,
                                 SEL_HttpResponse pSelector) {
  cocos2d::network::HttpRequest *request = new cocos2d::network::HttpRequest();
  request->setUrl(
      strmake("%s/%s/%s/", serverUrl.c_str(), apiName.c_str(), method.c_str()));
...
void BuggyServerManager::getPostBoxItem() {
  ...
  sendPost("getuserdata", strRID + "&" + "post=1",
           httpresponse_selector(BuggyServerManager::onGetPostBoxItem));
}
...
void BuggyServerManager::rename(std::string strName) {
  ...
  if (savedRID.length() > 1) {
    sendPost("rename",
             "_id=" + savedRID + "&" + "id=" + requestedID + "&name=" + strName,
             httpresponse_selector(BuggyServerManager::onRenameComplete));
  } else {
    sendPost("rename", "id=" + requestedID + "&name=" + strName,
             httpresponse_selector(BuggyServerManager::onRenameComplete));
  }
}
...
void BuggyServerManager::saveUserData(std::string data) {
  //    if (GM->getGem() > gemLimit || GM->getCoin() > goldLimit ||
  //    GM->getTree() > treeLimit) {
  ////        isBannedUser = true;
  ////        banMe();
  //    }else{
  std::string savedRID = UDGetStr(KEY_RID, "");
  if (savedRID.length() > 1) {
    sendPost("saveuser",
             "_id=" + savedRID + "&" + "id=" + requestedID + "&" + data,
             httpresponse_selector(BuggyServerManager::onSaveUserData));
  } else {
    sendPost("saveuser", "id=" + requestedID + "&" + data,
             httpresponse_selector(BuggyServerManager::onSaveUserData));
  }
  //    }
}
...
void BuggyServerManager::findMatch(int trophy) {
  sendPost("findmatch",
           "id=" + requestedID + "&" + strmake("trophy=%d", trophy),
           httpresponse_selector(BuggyServerManager::onFindMatches));
}
...
void BuggyServerManager::onActionNotNeededComplete(cocos2d::Node *sender,
                                                   void *data) {
  //    SET_DOCUMENT_AND_CHECK_ERROR
}
void BuggyServerManager::setPostBoxItem(std::string items) {
  sendPost("saveuser", "id=" + requestedID + "&" + "post=" + items,
           httpresponse_selector(BuggyServerManager::onGetPostBoxItem));
}
...
void BuggyServerManager::registerName(std::string name) {
  sendPost("saveuser", "id=" + requestedID + "&" + "name=" + name,
           httpresponse_selector(BuggyServerManager::onRegisterNameComplete));
}
void BuggyServerManager::onRegisterNameComplete(cocos2d::Node *sender,
                                                void *data) {
  ...
  if (document.HasMember("result")) {
    ...
    rapidjson::Value::ConstMemberIterator result =
        document.FindMember("result");
    if (result !=
        document
            .MemberEnd()) // if level is not found it will be ignored, not crash
    {
  ...
  if (TITLE != nullptr) {
    if (Title::usesAccountSystem() || GM->deferredAccountRegistrationActive) {
      TITLE->nameHandleState = NETWORK_HANDLE_STATE_ARRIVED;
    }
  } else if (GM->deferredAccountRegistrationActive) {
    GM->deferredNameHandleState = NETWORK_HANDLE_STATE_ARRIVED;
  }
}
void BuggyServerManager::changeAccount(std::string name, std::string password) {
  sendPost("userlogin",
           strmake("name=%s&password=%s", name.c_str(), password.c_str()),
           httpresponse_selector(BuggyServerManager::onChangeAccountComplete));
}
void BuggyServerManager::onChangeAccountComplete(cocos2d::Node *sender,
                                                 void *data) {
  rapidjson::Document document = getDocument(sender, data);
  if (document.IsNull()) {
    this->isServerFailed = true;
...
void BuggyServerManager::changeName(std::string name) {
  sendPost("changename",
           strmake("id=%s&name=%s", requestedID.c_str(), name.c_str()),
           httpresponse_selector(BuggyServerManager::onChangeNameComplete));
}
void BuggyServerManager::onChangeNameComplete(cocos2d::Node *sender,
                                              void *data) {
  rapidjson::Document document = getDocument(sender, data);
  if (document.IsNull()) {
    this->isServerFailed = true;
...
void BuggyServerManager::getOtherUserData(std::string data) {
  sendPost(
      "getuserdata", data,
      httpresponse_selector(BuggyServerManager::onGetOtherUserDataComplete));
}
void BuggyServerManager::onGetOtherUserDataComplete(cocos2d::Node *sender,
                                                    void *data) {
  rapidjson::Document document = getDocument(sender, data);
  if (document.IsNull()) {
    this->isServerFailed = true;
...
void BuggyServerManager::getTopPlayers(int count) {
  ...
  sendPost("findtopplayers", strmake("count=%d", count),
           httpresponse_selector(BuggyServerManager::onGetTopPlayersComplete));
}
void BuggyServerManager::onGetTopPlayersComplete(cocos2d::Node *sender,
                                                 void *data) {
  rapidjson::Document document = getDocument(sender, data);
  if (document.IsNull()) {
    this->isServerFailed = true;
...
void BuggyServerManager::getAllUserData() {
  ...
  sendPost("getuserdata",
           strRID + "&name=1" + "&trophy=1" + "&arena_clr_cnt=1" +
               "&stage_clr_idx=1" + "&buildings=1" + "&inventory=1" +
               "&deck=1" + "&hdck=1" + "&hivt=1" + "&gem=1" + "&gold=1" +
               "&tree=1" +
               //             "&arena_score=1"+
               //             "&arena_reward=1"+
               "&spc_units=1" +
               //             "&under_attack=1"+
               //             "&golden_ticket=1"+
               "&defence_record=1" + "&search_state=1" + "&search_items=1" +
               "&attend=1" + "&gift_day=1" + "&iap_list=1" + "&iap_total=1" +
               //             "&mission=1"+
               "&post=1" + "&ban=1" +
...
void BuggyServerManager::getUserData(std::string data) {
  std::string savedRID = UDGetStr(KEY_RID, "");
  if (savedRID.length() > 1) {
    sendPost("getuserdata", "_id=" + savedRID + "&" + data.c_str(),
             httpresponse_selector(BuggyServerManager::onGetUserDataComplete));
  } else {
    sendPost("getuserdata", "id=" + requestedID + "&" + data.c_str(),
             httpresponse_selector(BuggyServerManager::onGetUserDataComplete));
  }
}
void BuggyServerManager::onGetUserDataComplete(cocos2d::Node *sender,
                                               void *data) {
  rapidjson::Document document = getDocument(sender, data);
  if (document.IsNull()) {
    this->isServerFailed = true;
  ...
  if (document.HasMember("trophy"))
    UDSetInt(KEY_TROPHY, document["trophy"].GetInt());
  ...
  if (document.HasMember("spc_units"))
    UDSetStr(KEY_SPECIAL_UNITS, document["spc_units"].GetString());
  ...
  if (document.HasMember("gold"))
    GM->setCoin(document["gold"].GetInt());
  if (document.HasMember("tree"))
    GM->setTree(document["tree"].GetInt());
  ...
  if (document.HasMember("inventory")) {
    ...
    if (str.length() > 1) {
      ...
      //            ValueVector units = GameManager::getInstance()->split(value,
      //            "_"); std::vector<UnitInfo*>
      //            unitInfoBattleUnitListInventory; for(int i = 0; i <
      //            units.size(); i++){
  ...
  if (document.HasMember("deck")) {
    ...
    if (str.length() > 1) {
      ...
      //            ValueVector units = GameManager::getInstance()->split(value,
      //            "_"); std::vector<UnitInfo*> unitInfoBattleUnitListDeck;
  ...
  std::string str;
  //    if(document.HasMember("attend")) UDSetStr(KEY_ATTEND_RCV_INFO,
  //    document["attend"].GetString());
  if (document.HasMember("access_reward_receive")) {
    str = document["access_reward_receive"].GetString();
    //        for(int i = 0; i < str.length(); i++){
    //            if(i == 0){
    //                UDSetBool(KEY_ACCEESS_REWARD_RECEIVED_AM79,
    //                Value(str.at(i)).asInt() == 1);
    //            }else if(i == 1){
    //                UDSetBool(KEY_ACCEESS_REWARD_RECEIVED_PM1214,
    //                Value(str.at(i)).asInt() == 1);
    //            }else if(i == 2){
    //                UDSetBool(KEY_ACCEESS_REWARD_RECEIVED_PM1720,
    //                Value(str.at(i)).asInt() == 1);
    //            }else if(i == 3){
    //                UDSetBool(KEY_ACCEESS_REWARD_RECEIVED_PM2022,
    //                Value(str.at(i)).asInt() == 1);
    //            }else if(i == 4){
    //                UDSetBool(KEY_ACCEESS_REWARD_RECEIVED_PM2224,
    //                Value(str.at(i)).asInt() == 1);
    //            }
    //        }
  }
  //    if(document.HasMember("gift_day")) UDSetInt(KEY_GIFT_DAY,
  //    document["gift_day"].GetInt());
  ...
  if (document.HasMember("first_launch_date")) {
    ...
    int dayCount = GM->getDayDiff(Value(strFirstDay.substr(0, 4)).asInt(),
                                  Value(strFirstDay.substr(5, 2)).asInt(),
                                  Value(strFirstDay.substr(8, 2)).asInt(),
                                  Value(strNow.substr(0, 4)).asInt(),
                                  Value(strNow.substr(5, 2)).asInt(),
                                  Value(strNow.substr(8, 2)).asInt());
...
void BuggyServerManager::getMyRank(int trophy) {
  sendPost("getrank", strmake("id=%s&trophy=%d", requestedID.c_str(), trophy),
           httpresponse_selector(BuggyServerManager::onGetMyRankComplete));
}
void BuggyServerManager::onGetMyRankComplete(cocos2d::Node *sender,
                                             void *data) {
...
void BuggyServerManager::getGameInfo() {
  ...
  request->setRequestType(cocos2d::network::HttpRequest::Type::GET);
  request->setResponseCallback(
      this, httpresponse_selector(BuggyServerManager::onGetGameInfoComplete));

  // write the post data
  cocos2d::network::HttpClient::getInstance()->send(request);
  request->release();
}
void BuggyServerManager::onGetGameInfoComplete(cocos2d::Node *sender,
                                               void *data) {
...
void BuggyServerManager::getRewardInfo() {
  ...
  request->setRequestType(cocos2d::network::HttpRequest::Type::GET);
  request->setResponseCallback(
      this, httpresponse_selector(BuggyServerManager::onGetRewardInfoComplete));

  // write the post data
  cocos2d::network::HttpClient::getInstance()->send(request);
  request->release();
}
void BuggyServerManager::onGetRewardInfoComplete(cocos2d::Node *sender,
                                                 void *data) {
...
void BuggyServerManager::isThisAppFree() {
  cocos2d::network::HttpRequest *request = new cocos2d::network::HttpRequest();
  std::string isThisAppFreeAddress =
      strmake("%s/%s/", serverUrl.c_str(), "isCartoonCraftFree");
  ;
#if CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID
  isThisAppFreeAddress =
      strmake("%s/%s/", serverUrl.c_str(), "isCartoonCraftFree");
  ...
  isThisAppFreeAddress =
      strmake("%s/%s/", serverUrl.c_str(), "isCartoonCraftIOSFree");
#endif
  ...
  request->setRequestType(cocos2d::network::HttpRequest::Type::GET);
  request->setResponseCallback(
      this, httpresponse_selector(BuggyServerManager::onIsThisAppFree));

  // write the post data
  //    std::string requestData = strmake("id=%s", requestedID.c_str());
  //    request->setRequestData(requestData.c_str(),
  //    strlen(requestData.c_str()));
  cocos2d::network::HttpClient::getInstance()->send(request);
  request->release();
...
void BuggyServerManager::getArenaRank() {
  sendPost("getarnrank", "_id=" + UDGetStr(KEY_RID),
           httpresponse_selector(BuggyServerManager::onGetArenaComplete));
}
...
void BuggyServerManager::sendArenaScore(int score) {
  std::string savedRID = UDGetStr(KEY_RID, "");
  std::string data =
      "score=" + Value(score).asString() + "&name=" + UDGetStr(KEY_NAME);
  std::string arenaRID = UDGetStr(KEY_ARENA_RID, "");
  if (arenaRID.length() > 0) {
    data += "&arenarid=" + arenaRID;
  }
  BSM->sendPost("arnsv", "_id=" + savedRID + "&" + data,
                httpresponse_selector(BuggyServerManager::onSaveUserData));

  //    saveUserData("arena_score="+Value(score).asString() + "&arena_clr_cnt="
  //    + Value(clearCount).asString());
}
void BuggyServerManager::onSENDArenaScoreComplete(cocos2d::Node *sender,
                                                  void *data) {
...
void BuggyServerManager::getArenaReward() {
  ...
  if (savedRID.length() > 0) {
    sendPost(
        "getuserdata", "_id=" + savedRID + "&arena_reward=1",
        httpresponse_selector(BuggyServerManager::onGetArenaRewardComplete));
  } else {
    sendPost(
        "getuserdata", "id=" + requestedID + "&arena_reward=1",
        httpresponse_selector(BuggyServerManager::onGetArenaRewardComplete));
  }
}
void BuggyServerManager::onGetArenaRewardComplete(cocos2d::Node *sender,
                                                  void *data) {
  rapidjson::Document document = getDocument(sender, data);
  if (document.IsNull()) {
    this->isServerFailed = true;
...
void BuggyServerManager::addShield(int hour) {
  ...
  sendPost("addshield",
           "id=" + requestedID + "&hour=" + Value(hour).asString() +
               "&gem=" + Value(GM->getGem()).asString(),
           httpresponse_selector(BuggyServerManager::onAddShieldComplete));
}
void BuggyServerManager::onAddShieldComplete(cocos2d::Node *sender,
                                             void *data) {
...
void BuggyServerManager::verifyReceipt(std::string strSignedData,
  ...
  request->setRequestType(cocos2d::network::HttpRequest::Type::POST);
  request->setResponseCallback(
      this, httpresponse_selector(BuggyServerManager::onVerifyReceiptComplete));

  // write the post data
  std::string requestData = strmake(
      "id=%s&signedData=%s&signature=%s&platform=%s", requestedID.c_str(),
      strSignedData.c_str(), strSignature.c_str(), strPlatform.c_str());
...
void BuggyServerManager::onVerifyReceiptComplete(cocos2d::Node *sender,
                                                 void *data) {
...
void BuggyServerManager::banMe() {
  sendPost(
      "saveuser", "id=" + requestedID + "&ban=1&trophy=0",
      httpresponse_selector(BuggyServerManager::onActionNotNeededComplete));
}

void BuggyServerManager::checkPlayID(std::string data) {
  sendPost("checkplayid", "playid=" + data,
           httpresponse_selector(BuggyServerManager::onCheckPlayID));
}
...
void BuggyServerManager::saveUserData(std::vector<int> &datas) {
  ...
  for (int i = 0; i < datas.size(); i++) {
    if (datas.at(i) == DATA_TYPE_GEM) {
      ...
      strData = strmake("%s&buildings=%s", strData.c_str(),
                        UDGetStr(KEY_BUILDINGS, "").c_str());
    } else if (datas.at(i) == DATA_TYPE_INVENTORY) {
      strData = strmake("%s&inventory=%s", strData.c_str(),
                        UDGetStr(KEY_UNITS_INVENTORY, "").c_str());
    } else if (datas.at(i) == DATA_TYPE_DECK) {
      strData = strmake("%s&deck=%s", strData.c_str(),
                        UDGetStr(KEY_UNITS_DECK, "").c_str());
    } else if (datas.at(i) == DATA_TYPE_KEYS) {
      //            strData = strmake("%s&keys=%s", strData.c_str(),
      //            UDGetStr(KEY_KEYS, "").c_str());
    } else if (datas.at(i) == DATA_TYPE_KEY_GET_STATE) {
      //            strData = strmake("%s&keygetstate=%s", strData.c_str(),
      //            UDGetStr(KEY_KEY_GET_STATE, "").c_str());
    } else if (datas.at(i) == DATA_TYPE_IAP) {
      strData = strmake("%s&iap_list=%s&iap_total=%d", strData.c_str(),
                        UDGetStr(KEY_IAP_LIST, "").c_str(),
                        UDGetInt(KEY_IAP_TOTAL, 0));
    } else if (datas.at(i) == DATA_TYPE_SEARCH_STATE) {
      strData = strmake("%s&search_state=%s", strData.c_str(),
                        UDGetStr(KEY_SEARCH_STATE, "000").c_str());
    } else if (datas.at(i) == DATA_TYPE_HERO_INVENTORY) {
      strData = strmake("%s&hivt=%s", strData.c_str(),
                        UDGetStr(KEY_UNITS_HERO_INVENTORY, "").c_str());
    } else if (datas.at(i) == DATA_TYPE_HERO_DECK) {
      strData = strmake("%s&hdck=%s", strData.c_str(),
                        UDGetStr(KEY_UNITS_HERO_DECK, "").c_str());
    } else if (datas.at(i) == DATA_TYPE_STAGE_CLEAR) {
      strData = strmake("%s&stage_clr_idx=%d", strData.c_str(),
                        UDGetInt(KEY_LAST_CLEAR_STAGE, -1));
    }
  }
...
void BuggyServerManager::getPvp6ResultAndTicket() {
  if (savedRID.length() > 0) {
    sendPost("getuserdata",
             "_id=" + savedRID +
                 "&pvp6Ticket=1&pvp6_refill_start_time=1&pvp_rwd_6=1",
             httpresponse_selector(
                 BuggyServerManager::onGetPvp6ResultAndTicketComplete));
  } else {
    sendPost("getuserdata",
             "id=" + requestedID +
                 "&pvp6Ticket=1&pvp6_refill_start_time=1&pvp_rwd_6=1",
...
void BuggyServerManager::onGetPvp6ResultAndTicketComplete(cocos2d::Node *sender,
                                                          void *data) {
  ...
  if (document.HasMember("pvp_rwd_6")) {
    ...
    if (HEROPAGE->pvp6Reward.length() > 1) {
      ...
      sendPost("saveuser", strRID + "&pvp_rwd_6=_",
               httpresponse_selector(BuggyServerManager::onSaveUserData));
    }
  }
  if (document.HasMember("pvp6Ticket")) {
    BSM->pvp6TicketCount = document["pvp6Ticket"].GetInt();
    BSM->pvp6TicketRefillStartTime =
        document["pvp6_refill_start_time"].GetString();
    HEROPAGE->pvpNetworkState = NETWORK_HANDLE_STATE_ARRIVED;
  }
}
void BuggyServerManager::getPvp6Rank() {
  sendPost("getp6rank", "_id=" + UDGetStr(KEY_RID, "_"),
           httpresponse_selector(BuggyServerManager::onGetPvp6RankComplete));
}
void BuggyServerManager::onGetPvp6RankComplete(cocos2d::Node *sender,
                                               void *data) {
...
void BuggyServerManager::findMatchForPvp6() {
  sendPost(
      "p6findmatch", "trophy=" + Value(UDGetInt(KEY_PVP6_TROPHY)).asString(),
      httpresponse_selector(BuggyServerManager::onFindMatchForPvp6Complete));
}
void BuggyServerManager::onFindMatchForPvp6Complete(cocos2d::Node *sender,
                                                    void *data) {
  ...
  if (document.HasMember("data")) {
    ...
    //        pvpTargetData =
    //        "_50/100/13/13_62/0/14/14_50/201/17/14_50/200/14/13_48/200/16/13_58/0/18/14_55/0/18/13_54/0/16/15_55/0/21/11_58/0/20/11_49/200/18/15_57/0/22/11_";
    //        // test
...
void BuggyServerManager::sendPvp6Result(int trophy) {
  ...
  sendPost("p6sv",
           "_id=" + UDGetStr(KEY_RID, "_") + "&name=" + strName +
               "&trophy=" + Value(trophy).asString() +
               "&data=" + UDGetStr(KEY_UNITS_HERO_DECK, "_"),
           httpresponse_selector(BuggyServerManager::onSendPvp6ResultComplete));
}
void BuggyServerManager::onSendPvp6ResultComplete(cocos2d::Node *sender,
                                                  void *data) {
...
void BuggyServerManager::getPvp12ResultAndTicket() {
  if (savedRID.length() > 0) {
    sendPost("getuserdata",
             "_id=" + savedRID +
                 "&pvp12Ticket=1&pvp12_refill_start_time=1&pvp_rwd_12=1",
             httpresponse_selector(
                 BuggyServerManager::onGetPvp12ResultAndTicketComplete));
  } else {
    sendPost("getuserdata",
             "id=" + requestedID +
                 "&pvp12Ticket=1&pvp12_refill_start_time=1&pvp_rwd_12=1",
...
void BuggyServerManager::onGetPvp12ResultAndTicketComplete(
    cocos2d::Node *sender, void *data) {
  ...
  if (document.HasMember("pvp_rwd_12")) {
    ...
    if (HEROPAGE->pvp12Reward.length() > 1) {
      ...
      sendPost("saveuser", strRID + "&pvp_rwd_12=_",
               httpresponse_selector(BuggyServerManager::onSaveUserData));
    }
  }
  if (document.HasMember("pvp12Ticket")) {
    BSM->pvp12TicketCount = document["pvp12Ticket"].GetInt();
    BSM->pvp12TicketRefillStartTime =
        document["pvp12_refill_start_time"].GetString();
    HEROPAGE->pvpNetworkState = NETWORK_HANDLE_STATE_ARRIVED;
  }
}
void BuggyServerManager::getPvp12Rank() {
  sendPost("getp12rank", "_id=" + UDGetStr(KEY_RID, "_"),
           httpresponse_selector(BuggyServerManager::onGetPvp12RankComplete));
}
void BuggyServerManager::onGetPvp12RankComplete(cocos2d::Node *sender,
                                                void *data) {
...
void BuggyServerManager::findMatchForPvp12() {
  sendPost(
      "p12findmatch", "trophy=" + Value(UDGetInt(KEY_PVP12_TROPHY)).asString(),
      httpresponse_selector(BuggyServerManager::onFindMatchForPvp12Complete));
}
void BuggyServerManager::onFindMatchForPvp12Complete(cocos2d::Node *sender,
                                                     void *data) {
  ...
  if (document.HasMember("data")) {
    ...
    pvpTargetData = document["data"].GetString();
    //        pvpTargetData =
    //        "_55/203/13/12_62/103/16/15_57/2/15/16_49/202/22/12_63/202/11/13_54/203/10/15_54/102/11/18_55/103/12/18_49/202/21/9_49/102/13/19_57/2/12/19_53/102/-1/-1_";
    //        // test
    pvpTargetName = document["name"].GetString();
    HEROPAGE->pvpNetworkState = NETWORK_HANDLE_STATE_ARRIVED;
  } else {
...
void BuggyServerManager::sendPvp12Result(int trophy) {
  sendPost(
      "p12sv",
      "_id=" + UDGetStr(KEY_RID, "_") +
          "&name=" + UDGetStr(KEY_NAME, "Unknown") +
          "&trophy=" + Value(trophy).asString() +
          "&data=" + UDGetStr(KEY_UNITS_HERO_DECK, "_"),
      httpresponse_selector(BuggyServerManager::onSendPvp12ResultComplete));
}
void BuggyServerManager::onSendPvp12ResultComplete(cocos2d::Node *sender,
                                                   void *data) {
...
int BuggyServerManager::getTimeLeftToSunday() {
  ...
  int timeLeft = 86400 - (tminfo->tm_hour * 60 * 60 + tminfo->tm_min * 60 +
                          tminfo->tm_sec);
...
void BuggyServerManager::checkServer() {
  ...
  request->setRequestType(HttpRequest::Type::GET);
  request->setResponseCallback(
      this, httpresponse_selector(BuggyServerManager::onCheckServerCompleted));
  HttpClient::getInstance()->send(request);
  request->release();
}
void BuggyServerManager::onCheckServerCompleted(Node *sender, void *data) {
  ...
  if (document.HasMember("rewardMsg")) {
    ...
    for (int i = 0; i < list.size(); i++) {
      ...
      if (dataList.size() > 1) {
        if (dataList.at(0).asString().compare("ko") == 0 &&
            LM->getLanguageType() == LanguageType::KOREAN) {
          GM->rewardMsg = dataList.at(1).asString();
          break;
        } else if (dataList.at(0).asString().compare("en") == 0 &&
                   LM->getLanguageType() != LanguageType::KOREAN) {
...
void BuggyServerManager::uploadMap(std::string name, std::string mapData) {
  ...
  BSM->sendPost(
      "usmpupload",
      strmake("userid=%s&username=%s&mapdata=%s&mapname=%s", userid.c_str(),
              username.c_str(), mapData.c_str(), mapname.c_str()),
      httpresponse_selector(BuggyServerManager::onUploadMapCompleted));
}
...
void BuggyServerManager::deleteMap(std::string mapID) {
  ...
  BSM->sendPost(
      "usmpdelete", strmake("_id=%s", mapID.c_str()),
      httpresponse_selector(BuggyServerManager::onDeleteMapCompleted));
}
...
void BuggyServerManager::downloadMapList(int index) {
  ...
  if (index == 0) {
    BSM->sendPost(
        "getusmprecent", "",
        httpresponse_selector(BuggyServerManager::onDownloadMapListCompleted));
  } else if (index == 1) {
    BSM->sendPost(
        "getusmptrophy", "",
        httpresponse_selector(BuggyServerManager::onDownloadMapListCompleted));
...
void BuggyServerManager::downloadMap(std::string mapID) {
  downloadMapState = -1;
  BSM->sendPost(
      "getusmpget", strmake("_id=%s", mapID.c_str()),
      httpresponse_selector(BuggyServerManager::onDownloadMapCompleted));
}
...
void BuggyServerManager::uploadCustomMapResult(std::string mapID, int like,
                                               int dislike, int success,
                                               int fail) {
  downloadMapState = -1;
  BSM->sendPost("setusmpresult",
                strmake("_id=%s&like=%d&dislike=%d&success=%d&fail=%d",
                        mapID.c_str(), like, dislike, success, fail),
                httpresponse_selector(
                    BuggyServerManager::onUploadCustomMapResultCompleted));
}
void BuggyServerManager::onUploadCustomMapResultCompleted(Node *sender,
                                                          void *data) {
  ...
  //    rapidjson::Document document = getDocument(sender, data);
  if (!response->isSucceed()) {
    log("!onUploadCustomMapResultCompleted error %s",
...
void BuggyServerManager::resetMapLike() {
  BSM->sendPost(
      "usmpresetlike", "reset=like",
      httpresponse_selector(BuggyServerManager::onResetMapLikeCompleted));
}
