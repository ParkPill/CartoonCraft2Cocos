...
//    log("PluginSdkboxPlay signin");
  ...
  log("%dx%d", (int)director->getWinSize().width,
...
//      CCLog("retina!!");
  ...
  //        i)->getCString())->getSpriteFrame(),
  //        __String::createWithFormat("laserThinBlueHit%d.png",
  //        i)->getCString());
  //    }
...
#if (CC_TARGET_PLATFORM == CC_PLATFORM_WIN32)
  Win32MouseCursor::setCursor("cursor.png", 0, 0);
#endif
  ...
  if (frameSize.height / frameSize.width >
      designResolutionSize.height / designResolutionSize.width) {
    glview->setDesignResolutionSize(designResolutionSize.width,
                                    designResolutionSize.height,
...
void AppDelegate::applicationWillEnterForeground() {
  ...
  //            WORLD->gameTimer += (float)(current -
  //            backgroundEnterTime)*0.001f; MM->disconnect();
  //            WORLD->endGame(false);
  //        }
  //    }
  // #ifdef SDKBOX_ENABLED
...
//     }
// }
// void AppDelegate::adViewDidFailToReceiveAdWithError ( const std::string &
// name ,
...
void AppDelegate::onSuccess(sdkbox::Product const &p) {
  //    if (Application::getInstance()->getTargetPlatform() ==
  //    Application::Platform::OS_IPHONE ||
  //        Application::getInstance()->getTargetPlatform() ==
  //        Application::Platform::OS_IPAD) { SM->proveReceipt(p.id,
  //        p.receiptCipheredPayload);
  //    }else{
...
void AppDelegate::onProductRequestSuccess(
...
void AppDelegate::unityAdsPlacementStateChanged(
    const std::string &placementId,
    PluginUnityAds::SBUnityAdsPlacementState oldState,
    PluginUnityAds::SBUnityAdsPlacementState newState) {
  CCLOG("unityads state change %s %d->%d", placementId.c_str(), oldState,
        newState);
}
...
void AppDelegate::unityAdsDidError(
    sdkbox::PluginUnityAds::SBUnityAdsError error, const std::string &message) {
...
void AppDelegate::onScoreSubmitted(const std::string &leaderboard_name,
                                   long score, bool maxScoreAllTime,
                                   bool maxScoreWeek, bool maxScoreToday) {}
void AppDelegate::onIncrementalAchievementUnlocked(
    const std::string &achievement_name) {}
void AppDelegate::onIncrementalAchievementStep(
    const std::string &achievement_name, double step) {}
void AppDelegate::onAchievementUnlocked(const std::string &achievement_name,
                                        bool newlyUnlocked) {}
