...
//    }
  ...
  if (GM->nextScene == STAGE_FIELD || isRaid) {
    ...
    // btn->addClickEventListener(CC_CALLBACK_1(HudLayer::onDragSelectClick,
    // this));
...
void HudLayer::showDisassembleButton() {
  ...
  btn->runAction(MoveTo::create(
      0.3f, Vec2(size.width / 2, btn->getContentSize().height / 2 + gap / 2)));
}
...
void HudLayer::showDisassembleConfirmButtons() {
  ...
  float gap = 40;
  btn->runAction(MoveTo::create(
      0.3f, Vec2(size.width / 2 - btn->getContentSize().width / 2 - gap / 2,
                 btn->getContentSize().height / 2 + gap / 2)));
...
void HudLayer::showWinPopup(bool win) {
  ...
  if (WORLD->gameMode == GAME_MODE_PVP6 || WORLD->gameMode == GAME_MODE_PVP12) {
    ...
    if (win && timeLeft <= 0 && stageIndex == chestIndex && !isArena && false) {
...
void HudLayer::onOkFromWinPopup(Ref *ref) {
  ...
  if (GM->playedMapID.length() > 0) {
    ...
    Node *btn = layer->getChildByName("btnLike");
    int tag = btn->getTag();
    BSM->uploadCustomMapResult(GM->playedMapID, tag == 1 ? 1 : 0, 0,
                               isWon ? 1 : 0, isWon ? 0 : 1);
    GM->playedMapID = "";
  }
